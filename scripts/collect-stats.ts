...
function countMigrations(): number {
  return readdirSync(join(ROOT, 'server/db/migrations')).filter((f) => f.endsWith('.ts')).length;
}

function countRouteModules(): number {
  return readdirSync(join(ROOT, 'server/routes')).filter((f) => f.endsWith('.ts')).length;
}
...
function countE2eSpecFiles(): number {
  try {
    return readdirSync(join(ROOT, 'e2e')).filter((f) => f.endsWith('.spec.ts')).length;
  } catch {
    return 0;
  }
}
...
function getTestResults(): TestResults {
  try {
    ...
    return {
      ...
      assertions: assertMatch ? parseInt(assertMatch[1].replace(/,/g, ''), 10) : 0,
    };
  } catch {
...
for (const doc of DOC_STATS) {
  ...
  const documented = parseInt(match[1].replace(/,/g, ''), 10);
...
if (updateMode && drifts.length > 0) {
  ...
  if (collected.spec_files) {
    updateFile('README.md', /Module specs\s*\|\s*\*\*\d+\*\*/, `Module specs | **${collected.spec_files}**`);
  }
  if (collected.mcp_tools) {
    updateFile('README.md', /MCP tools\s*\|\s*\*\*\d+\*\*/, `MCP tools | **${collected.mcp_tools}**`);
  }
  if (collected.migration_files) {
    updateFile('README.md', /DB migrations\s*\|\s*\*\*\d+\*\*/, `DB migrations | **${collected.migration_files}**`);
  }
  ...
  if (collected.spec_files) {
    updateFile('docs/deep-dive.md', /Module specs\s*\|\s*\d+\s/, `Module specs | ${collected.spec_files} `);
  }
  if (collected.db_tables) {
    updateFile('docs/deep-dive.md', /Database tables\s*\|\s*\d+/, `Database tables | ${collected.db_tables}`);
  }
  if (collected.migration_files) {
    updateFile(
      'docs/deep-dive.md',
  ...
  if (collected.mcp_tools) {
    updateFile('docs/deep-dive.md', /MCP tools\s*\|\s*\d+/, `MCP tools | ${collected.mcp_tools}`);
  }
