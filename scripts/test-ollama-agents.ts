...
const filterIds = process.argv
  .slice(2)
  .filter((a) => a !== '--agent' && (agentArgIdx < 0 || a !== process.argv[agentArgIdx + 1]));
const tests = filterIds.length > 0 ? ALL_TESTS.filter((t) => filterIds.includes(t.id)) : ALL_TESTS;
...
async function main() {
    ...
    await Bun.write(
      ...
      JSON.stringify(
        results.map((r) => ({
          ...
          passCriteria: r.test.passCriteria,
          response: r.response,
          errorMsg: r.errorMsg,
  ...
  for (const r of results) {
    ...
    lines.push(
      `| ${r.test.id} | ${r.test.name} | ${icon} ${r.status} | ${(r.durationMs / 1000).toFixed(1)}s | ${r.toolCalls.length} | ${preview}${r.response.length > 60 ? '...' : ''} |`,
    );
  }
  lines.push('');
