...
const MODELS = modelFilter.length > 0 ? DEFAULT_MODELS.filter((m) => modelFilter.includes(m.model)) : DEFAULT_MODELS;
  ...
  corvid_list_work_tasks: {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        status: {
          type: 'string',
          enum: ['active', 'queued', 'completed', 'failed'],
...
const ALGOCHAT_SCENARIOS: TestScenario[] = [
  {
    ...
    evaluate(result) {
      const hasToolCall = result.toolCalls.some(
        (t) => t.toLowerCase().includes('corvid_send_message') || t.toLowerCase().includes('send_message'),
      );
      const textMentionsTool = result.assistantTexts.some(
        (t) => t.includes('corvid_send_message') || t.includes('"to"') || t.includes('"message"'),
      );
      const passed = hasToolCall || textMentionsTool;
      return {
        passed,
        notes: passed ? 'Correctly invoked send_message' : 'Did not call corvid_send_message',
      };
    },
  },
  {
    ...
    evaluate(result) {
      const hasToolCall = result.toolCalls.some(
        (t) => t.toLowerCase().includes('recall_memory') || t.toLowerCase().includes('corvid_recall'),
      );
      const textMentionsTool = result.assistantTexts.some(
        (t) => t.includes('corvid_recall_memory') || t.includes('"key"') || t.includes('team-agents-alpha'),
      );
      const passed = hasToolCall || textMentionsTool;
      return {
        passed,
        notes: passed ? 'Correctly invoked recall_memory with key param' : 'Did not call corvid_recall_memory',
      };
    },
  },
  {
    ...
    evaluate(result) {
      const hasRecall =
        result.toolCalls.some((t) => t.toLowerCase().includes('recall_memory')) ||
        result.assistantTexts.some((t) => t.includes('corvid_recall_memory'));
      const hasSend =
        result.toolCalls.some((t) => t.toLowerCase().includes('send_message')) ||
    ...
    evaluate(result) {
      const hasToolCall =
        result.toolCalls.some(
          (t) => t.toLowerCase().includes('list_work_tasks') || t.toLowerCase().includes('work_tasks'),
        ) || result.assistantTexts.some((t) => t.includes('corvid_list_work_tasks'));
      return {
        passed: hasToolCall,
        notes: hasToolCall ? 'Correctly called list_work_tasks' : 'Did not call corvid_list_work_tasks',
      };
    },
  },
];
...
async function benchmarkModel(modelConfig: ModelConfig, projectId: string): Promise<ModelBenchmarkResult> {
  console.log(`\n${'─'.repeat(60)}`);
  ...
  for (const scenario of ALGOCHAT_SCENARIOS) {
    ...
    try {
      ...
      const prompt = modelConfig.textBasedTools ? `${TOOL_SCHEMA_PROMPT}\n---\n\n${scenario.prompt}` : scenario.prompt;
...
function generateReport(results: ModelBenchmarkResult[], runDate: string): string {
  ...
  for (const r of results) {
    ...
    const rec =
      r.totalPassed === ALGOCHAT_SCENARIOS.length ? 'Ready' : r.totalPassed >= 3 ? 'Conditional' : 'Needs work';
    lines.push(
      `| ${r.model.displayName} | ${r.model.role} | ${r.model.tier} | ${passRate} | ${faRate} | ${avgTime} | ${rec} |`,
    );
  }
  ...
  for (const r of results) {
    ...
    lines.push(
      `- **Tool parsing:** ${r.model.textBasedTools ? 'Text-based (prompt-injected schema)' : 'Native Ollama tools API'}`,
    );
    ...
    if (r.skipped) {
      ...
      for (const sr of r.scenarios) {
        ...
        lines.push(
          `| ${sr.scenario} | ${icon} ${sr.passed ? 'PASS' : 'FAIL'} | ${(sr.durationMs / 1000).toFixed(1)}s | ${sr.notes} |`,
        );
      }
  ...
  if (ranked.length > 0) {
    ...
    lines.push(
      `- Recommended: **${starlingPick.model.displayName}** (${starlingPick.totalPassed}/${ALGOCHAT_SCENARIOS.length} scenarios, ${starlingPick.model.contextK}K context)`,
    );
    lines.push('');
    ...
    lines.push(
      `- Recommended: **${merlinPick.model.displayName}** (${merlinPick.totalPassed}/${ALGOCHAT_SCENARIOS.length} scenarios, tier ${merlinPick.model.tier})`,
    );
    lines.push('');
    ...
    lines.push(
      `- Recommended: **${condorPick.model.displayName}** (${condorPick.totalPassed}/${ALGOCHAT_SCENARIOS.length} scenarios, tier ${condorPick.model.tier}, thinking: ${condorPick.model.supportsThinking ? 'yes' : 'no'})`,
    );
    lines.push('');
    ...
    for (const r of results) {
      ...
      if (r.totalPassed < 3) {
        lines.push(
          `- **${r.model.displayName}**: Not recommended for junior duties without prompt tuning. Pass rate ${r.totalPassed}/${ALGOCHAT_SCENARIOS.length}.`,
        );
      }
    }
    ...
    if (textBasedModels.length > 0) {
      lines.push(
        `- All recommended models use text-based tool parsing — ensure \`TEXT_BASED_TOOL_FAMILIES\` is up to date in \`server/providers/ollama/provider.ts\`.`,
      );
    }
  }
