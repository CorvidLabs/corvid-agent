...
{
  const matches = grepFiles('dispenser\\.testnet\\|testnet.*faucet\\|faucet.*testnet', ['server']).filter(
    (l) => !l.includes('.test.ts') && !l.includes('// '),
  );
  ...
  if (unguarded.length === 0) {
    pass(
      'bypasses',
      'Testnet dispenser URLs are guard-gated',
      `${matches.length} references, all behind network=testnet checks`,
    );
  } else {
...
// Check for testnet USDC ASA IDs (testnet USDC is 10458941 — NOT the mainnet 31566704)
{
  ...
  const matches = grepFiles(String(testnetUsdcAsaId), ['server']).filter((l) => !l.includes('.test.ts'));
...
// Check mainnet USDC ASA ID is correct (31566704)
{
  ...
  if (matches.length > 0) {
    ...
    warn(
      'bypasses',
      `Mainnet USDC ASA ID (${mainnetId}) not found`,
      'May be USDC_ASA_ID env-var only — verify .env.mainnet.example',
    );
  }
}
...

{
  ...
  if (guardFile.length > 0) {
    ...
    fail(
      'security',
      'Injection guard not found in server code',
      'Expected server/lib/injection-guard.ts with checkInjection export',
    );
  }
}
...
if (existsSync(envPath)) {
  ...
  if (!wek || wek === 'CHANGE_ME_GENERATE_256_BIT_KEY') {
    fail(
      'env',
      'WALLET_ENCRYPTION_KEY is not set or is default placeholder',
      'REQUIRED for mainnet — generate with: openssl rand -hex 32',
    );
  } else if (wek.length < 32) {
    fail(
      'env',
      `WALLET_ENCRYPTION_KEY is too short (${wek.length} chars)`,
      'Must be at least 32 chars for AES-256 strength',
    );
  } else {
