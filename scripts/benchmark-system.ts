...
function getTotalMemoryMB(): number {
  ...
  const ps = exec('powershell -Command "(Get-CimInstance Win32_ComputerSystem).TotalPhysicalMemory"');
...
function getFreeMemoryMB(): number {
  ...
  if (platform === 'darwin') {
    ...
    const free = parseInt(vmstat.match(/Pages free:\s+(\d+)/)?.[1] || '0', 10) * pageSize;
    const inactive = parseInt(vmstat.match(/Pages inactive:\s+(\d+)/)?.[1] || '0', 10) * pageSize;
...
function getCPUInfo(): { model: string; cores: number } {
  ...
  if (platform === 'linux') {
    const model = exec("grep 'model name' /proc/cpuinfo | head -1").split(':').pop()?.trim() || 'Unknown';
...
function getTier(totalGB: number): { tier: string; advice: string } {
  if (totalGB < 8)
    return {
      tier: 'Below Minimum',
      advice: 'Not recommended. corvid-agent may run in CLI-only mode with Claude API, but expect frequent swapping.',
    };
  if (totalGB < 16)
    return {
      tier: 'Minimum (8 GB)',
      advice: 'CLI agent only with Claude API. No Docker, no Ollama, lightweight editor. Expect tight margins.',
    };
  if (totalGB < 32)
    return {
      tier: 'Recommended (16 GB)',
      advice: 'Single agent + IDE works well. Use TestNet instead of localnet. Skip Ollama. Close unused browser tabs.',
    };
  if (totalGB < 64)
    return {
      tier: 'Comfortable (32 GB)',
      advice: 'Full stack — localnet + IDE + Docker + browser. Ollama with small models (8B). Room to breathe.',
    };
  return {
    tier: 'Unlimited (64 GB+)',
    advice: 'Everything works — localnet, Ollama (70B+), multi-agent, IDE, browser, all at once.',
  };
}
...
function measureLocalnet(): ComponentMetrics {
  ...
  const containers = exec('docker ps --format "{{.Names}}\t{{.ID}}" --filter "name=algokit" 2>/dev/null');
  if (!containers) {
    return {
...
function measureOllama(): ComponentMetrics {
    ...
    try {
      ...
      if (data.models?.length) {
        modelInfo = data.models
          .map((m: { name: string; size: number }) => `${m.name} (${formatMB(m.size / (1024 * 1024))})`)
          .join(', ');
      }
    } catch {}
  }
  ...
  return {
    ...
    status: 'running',
    memoryMB: Math.round(memoryMB),
    details: modelInfo ? `Loaded models: ${modelInfo}` : `RSS: ${formatMB(memoryMB)}`,
  };
}
...
function run(): BenchmarkResult {
  ...
  const selectedComponent = componentArg >= 0 ? args[componentArg + 1] : undefined;

  const shouldMeasure = (name: string) => !selectedComponent || selectedComponent === name;
  ...
  const totalUsedMB = components.reduce((sum, c) => sum + (c.memoryMB || 0), 0);

  // Generate recommendations
  const recommendations: string[] = [];
  if (totalGB < 16) {
    recommendations.push('Consider using TestNet instead of localnet to save ~3-4 GB of RAM');
    recommendations.push('Use a lightweight editor instead of VS Code with many extensions');
    recommendations.push('Avoid running Ollama locally — use Claude API instead');
  }
  if (totalGB < 32) {
    recommendations.push('Close browser DevTools and unused tabs when running the full stack');
  }
  if (process.platform === 'linux' && exec('grep -q microsoft /proc/version 2>/dev/null && echo yes') === 'yes') {
    recommendations.push('WSL2 detected — set memory limit in ~/.wslconfig to prevent Windows from over-allocating');
    recommendations.push("Consider using Docker Desktop's WSL2 backend memory limit (Settings → Resources)");
  }

  const running = components.filter((c) => c.status === 'running');
  if (running.length > 0 && totalUsedMB > totalMB * 0.7) {
    recommendations.push('Running components use >70% of total RAM — consider stopping unused services');
  }
...
if (jsonMode) {
  ...
  for (const c of result.components) {
    const status = c.status === 'running' ? '✓' : c.status === 'not_running' ? '○' : '✗';
