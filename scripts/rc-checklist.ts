...
check('security', 'Zero critical security test failures', () => {
  const securityGlob =
    'server/__tests__/{bash-security,injection-guard,injection-hardening,prompt-injection,security-audit,security-headers,routes-security-overview}.test.ts';
...
check('payments', 'Escrow auto-release (72h) tested', () => {
  ...
  const hasAutoRelease =
    content.includes('processAutoReleases') || content.includes('auto-release') || content.includes('autoRelease');
  return {
