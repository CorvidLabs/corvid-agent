                    ...
                    @if (group.schedules.length > 0) {
                        <div class="status-group">
                            <button class="status-group__header" (click)="toggleGroup(group.status)" [attr.data-status]="group.status">
                                <span class="status-group__indicator section-chevron" [class.section-chevron--open]="!collapsedGroups()[group.status]">&#9654;</span>
                                        ...
                                        <div class="schedule-card" [attr.data-status]="schedule.status" [class.schedule-card--expanded]="expandedScheduleId() === schedule.id" (click)="toggleSchedule(schedule.id)">
                                            <div class="schedule-card__header">
                                                <div class="schedule-card__title">
                                                    ...
                                                    <h3>{{ schedule.name }}</h3>
                                                    <span class="expand-indicator section-chevron" [class.section-chevron--open]="expandedScheduleId() === schedule.id">&#9654;</span>
                                                </div>
...
.exec-stats__item{display:flex;flex-direction:column;gap:.1rem}.exec-stats__value{font-size:var(--text-base);font-weight:700;color:var(--text-primary)}.exec-stats__label{font-size:var(--text-4xs);color:var(--text-tertiary);text-transform:uppercase;letter-spacing:.05em}
...
.create-btn,.save-btn{padding:.5rem 1rem;background:var(--accent-cyan-dim);color:var(--accent-cyan);border:1px solid var(--accent-cyan);border-radius:var(--radius);font-size:var(--text-xs);font-weight:600;cursor:pointer}
.create-btn:hover,.save-btn:hover:not(:disabled){background:rgba(0,229,255,.2)}.save-btn:disabled{opacity:.5;cursor:not-allowed}
.approvals-banner{background:var(--accent-amber-dim);border:1px solid var(--accent-amber);border-radius:var(--radius-lg);padding:1rem;margin-bottom:1.25rem}.approvals-banner h3{margin:0 0 .75rem;color:var(--accent-amber);font-size:var(--text-caption)}
...
.approval-type{font-size:var(--text-2xs);font-weight:700;text-transform:uppercase;padding:2px 6px;border-radius:var(--radius-sm);background:var(--accent-magenta-dim);color:var(--accent-magenta)}.approval-desc{font-size:var(--text-xs);color:var(--text-secondary)}.approval-actions{display:flex;gap:.35rem}
.approve-btn,.deny-btn{padding:.3rem .75rem;border-radius:var(--radius);font-size:var(--text-xxs);font-weight:600;cursor:pointer}.approve-btn{background:var(--accent-green-dim);color:var(--accent-green);border:1px solid var(--accent-green)}.deny-btn{background:var(--accent-red-dim);color:var(--accent-red);border:1px solid var(--accent-red)}
.create-form{background:var(--bg-surface);border:1px solid var(--border);border-radius:var(--radius-lg);padding:1.25rem;margin-bottom:1.25rem;animation:expandReveal .25s ease-out}.create-form h3{margin:0 0 1rem;color:var(--text-primary);font-size:var(--text-sm)}.create-form h4{margin:1rem 0 .5rem;color:var(--text-secondary);font-size:var(--text-xs)}
.form-grid{display:grid;grid-template-columns:1fr 1fr;gap:.75rem}.span-2{grid-column:span 2}.form-field{display:flex;flex-direction:column;gap:.25rem}.form-field label{font-size:var(--text-2xs);color:var(--text-tertiary);text-transform:uppercase;letter-spacing:.05em}
.form-input,.form-select{padding:.45rem;background:var(--bg-input);border:1px solid var(--border-bright);border-radius:var(--radius);color:var(--text-primary);font-size:var(--text-caption);transition:border-color .1s,box-shadow .15s}.form-input:focus,.form-select:focus{border-color:var(--accent-cyan);box-shadow:var(--glow-cyan);outline:none}
.form-hint{font-size:var(--text-3xs);color:var(--text-tertiary)}.form-hint--active{color:var(--accent-cyan)}.mono{font-family:var(--font-mono, monospace)}.schedule-type-toggle{display:flex;gap:.35rem}
.type-btn{padding:.35rem .65rem;background:var(--bg-raised);border:1px solid var(--border);border-radius:var(--radius-sm);color:var(--text-secondary);font-size:var(--text-xxs);cursor:pointer}.type-btn--active{border-color:var(--accent-cyan);color:var(--accent-cyan);background:var(--accent-cyan-dim)}
.action-row{display:flex;gap:.5rem;align-items:center;margin-bottom:.5rem}.action-type-select{min-width:140px}
.remove-action-btn{padding:.25rem .5rem;background:var(--accent-red-dim);color:var(--accent-red);border:1px solid var(--accent-red);border-radius:var(--radius-sm);cursor:pointer;font-size:var(--text-sm);line-height:1}
.add-action-btn{padding:.35rem .75rem;background:var(--bg-raised);border:1px solid var(--border);border-radius:var(--radius);color:var(--text-secondary);font-size:var(--text-xxs);cursor:pointer;margin-top:.25rem}
.form-checkbox{display:flex;align-items:center;gap:.35rem;font-size:var(--text-xs);color:var(--text-secondary);cursor:pointer;white-space:nowrap}.form-buttons{margin-top:1rem}.save-btn{text-transform:uppercase}
.empty{text-align:center;padding:3rem;color:var(--text-tertiary)}.empty-hint{font-size:var(--text-xs);margin-top:.5rem}
...
.status-group__indicator{font-size:var(--text-4xs);color:var(--text-tertiary);width:.75rem}.status-group__label{font-size:var(--text-xxs);font-weight:700;text-transform:uppercase;letter-spacing:.05em}
...
.status-group__count{font-size:var(--text-2xs);color:var(--text-tertiary);margin-left:auto}
.schedule-list{display:flex;flex-direction:column;gap:.75rem}.schedule-card{background:var(--bg-surface);border:1px solid var(--border);border-radius:var(--radius-lg);padding:1rem;cursor:pointer;transition:border-color .15s,transform .15s,box-shadow .15s}.schedule-card:hover{transform:translateY(-1px);box-shadow:0 4px 16px rgba(0,0,0,.3)}
...
.schedule-card__header{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:.5rem}.schedule-card__title{display:flex;align-items:center;gap:.5rem}.schedule-card__title h3{margin:0;font-size:var(--text-sm);color:var(--text-primary)}.schedule-card__actions{display:flex;gap:.35rem}
.schedule-status{font-size:var(--text-3xs);font-weight:700;text-transform:uppercase;padding:2px 8px;border-radius:var(--radius-sm);border:1px solid}
...
.action-btn{padding:.3rem .6rem;background:var(--bg-raised);border:1px solid var(--border);border-radius:var(--radius-sm);color:var(--text-secondary);font-size:var(--text-2xs);cursor:pointer;transition:border-color .1s,color .1s,background .1s}.action-btn:hover{background:var(--bg-hover);color:var(--text-primary)}
...
.last-result__status{font-size:var(--text-4xs);font-weight:700;text-transform:uppercase;padding:1px 5px;border-radius:var(--radius-sm)}.last-result__status[data-status="completed"]{color:var(--accent-green);background:var(--accent-green-dim)}.last-result__status[data-status="failed"]{color:var(--accent-red);background:var(--accent-red-dim)}
.last-result__text{font-size:var(--text-2xs);color:var(--text-secondary);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
...
.validation-error{font-size:var(--text-xxs);color:var(--accent-red);font-weight:600;padding:.15rem 0}
.validation-error::before{content:'\x26A0  '}
.schedule-desc{margin:0 0 .5rem;font-size:var(--text-xs);color:var(--text-secondary)}.schedule-meta{display:flex;flex-wrap:wrap;gap:1rem;margin-bottom:.5rem}.meta-item{display:flex;flex-direction:column;gap:.1rem}.meta-label{font-size:var(--text-4xs);color:var(--text-tertiary);text-transform:uppercase}.meta-value{font-size:var(--text-xs);color:var(--text-primary);font-weight:600}
...
.action-tag{font-size:var(--text-3xs);padding:2px 6px;border-radius:var(--radius-sm);background:var(--bg-raised);color:var(--text-secondary);border:1px solid var(--border)}.action-tag[data-type="star_repo"]{color:var(--accent-amber);border-color:var(--accent-amber)}.action-tag[data-type="review_prs"]{color:var(--accent-cyan);border-color:var(--accent-cyan)}.action-tag[data-type="work_task"]{color:var(--accent-green);border-color:var(--accent-green)}
.exec-section{margin-top:2rem;background:var(--bg-surface);border:1px solid var(--border);border-radius:var(--radius-lg);padding:1.25rem}.exec-section h3{margin:0 0 .75rem;color:var(--text-primary);font-size:var(--text-sm)}.exec-list{display:flex;flex-direction:column;gap:.35rem}
.exec-row{display:flex;align-items:center;gap:.5rem;padding:.5rem;background:var(--bg-raised);border-radius:var(--radius);font-size:var(--text-xxs)}.exec-type{font-weight:600;color:var(--text-secondary);min-width:100px}.exec-status{font-size:var(--text-3xs);font-weight:700;text-transform:uppercase;padding:1px 6px;border-radius:var(--radius-sm)}
...
.exec-time{color:var(--text-tertiary);font-size:var(--text-2xs)}.exec-duration{font-size:var(--text-3xs);color:var(--text-tertiary);background:var(--bg-base);padding:1px 5px;border-radius:var(--radius-sm);border:1px solid var(--border);font-family:var(--font-mono, monospace)}.exec-result{color:var(--text-secondary);flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.exec-link{font-size:var(--text-2xs);color:var(--accent-cyan);text-decoration:none;border:1px solid var(--accent-cyan);padding:1px 6px;border-radius:var(--radius-sm)}
.expand-indicator{font-size:var(--text-4xs);color:var(--text-tertiary);margin-left:.25rem}.exec-row--clickable{cursor:pointer}.exec-row--clickable:hover{background:var(--bg-hover)}
.schedule-execs{margin-top:.75rem;border-top:1px solid var(--border);padding-top:.75rem;animation:expandReveal .3s ease-out}.execs-heading{margin:0 0 .5rem;color:var(--text-secondary);font-size:var(--text-xxs);text-transform:uppercase;letter-spacing:.04em}.loading-execs,.no-execs{font-size:var(--text-xxs);color:var(--text-tertiary);margin:0}
.exec-detail{padding:.5rem;background:var(--bg-base);border-radius:var(--radius);margin-top:.25rem;margin-bottom:.35rem}.exec-detail__result{margin:0;font-size:var(--text-xxs);color:var(--text-secondary);white-space:pre-wrap;word-break:break-word;max-height:300px;overflow-y:auto}
