...
export class SessionLifecycleManager {
  ...
  getAndClearRestartPendingSessions(): string[] {
    const rows = this.db.query(`SELECT id FROM sessions WHERE restart_pending = 1`).all() as Array<{ id: string }>;

    if (rows.length === 0) return [];

    const ids = rows.map((r) => r.id);
    const placeholders = ids.map(() => '?').join(',');
    this.db.query(`UPDATE sessions SET restart_pending = 0 WHERE id IN (${placeholders})`).run(...ids);
  ...
  private async cleanupOrphanedMessages(): Promise<void> {
    const orphanedMessages = this.db
      .query(`
            DELETE FROM session_messages
  ...
  private async enforceSessionLimits(): Promise<void> {
    ...
    const overLimitProjects = this.db
      .query(`
  ...
  canCreateSession(projectId: string): boolean {
    return (
      queryCount(this.db, 'SELECT COUNT(*) as cnt FROM sessions WHERE project_id = ?', projectId) <
      this.config.maxSessionsPerProject
    );
  }
  ...
  async cleanupSession(sessionId: string): Promise<boolean> {
    try {
      const cleanupTransaction = this.db.transaction(() => {
        ...
        // Delete escalation queue entries
        const escalationsResult = this.db
          .query(`
