...
import { getSession, updateSessionAgent, updateSessionPid, updateSessionStatus } from '../db/sessions';
...
export class SessionResilienceManager {
  ...
  handleApiOutage(sessionId: string): void {
    ...
    this.eventBus.emit(sessionId, {
      type: 'error',
      error: {
        message: `Session paused due to API outage — auto-resume in ${AUTO_RESUME_BASE_MS / 60_000}min`,
        type: 'api_outage',
      },
  ...
  attemptRestart(sessionId: string, restartCount: number): boolean {
    ...
    const backoffMs = BACKOFF_BASE_MS * 3 ** restartCount;
    ...
    const timer = setTimeout(() => {
      ...
      if (session.source === 'algochat' && session.agentId) {
        ...
        if (currentAgent && currentAgent.id !== session.agentId) {
          log.info(
            `Reassigning session ${sessionId} from agent ${session.agentId} to ${currentAgent.id} (${currentAgent.name})`,
          );
      ...
      for (const [sessionId, info] of this.pausedSessions) {
        ...
        if (info.resumeAttempts >= AUTO_RESUME_MAX_ATTEMPTS) {
          ...
          this.eventBus.emit(sessionId, {
            type: 'error',
            error: {
              message: `Auto-resume abandoned after ${info.resumeAttempts} attempts`,
        ...
        .then((healthy) => {
          ...
          for (const sessionId of dueSessionIds) {
            ...
            const backoffMs = Math.min(
              AUTO_RESUME_BASE_MS * AUTO_RESUME_MULTIPLIER ** info.resumeAttempts,
              AUTO_RESUME_CAP_MS,
            );
            info.resumeAttempts++;
   ...
   */
  startOrphanPruner(pruneCallback: () => number): void {
