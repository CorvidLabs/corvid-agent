...
export class ProcessManager {
  ...
  private buildMcpContext(
    ...
    return this.mcpServices.buildContext({
      ...
      emitStatus: sessionId
        ...
        : undefined,
      extendTimeout: sessionId ? (additionalMs: number) => this.extendTimeout(sessionId, additionalMs) : undefined,
  ...
  startProcess(
    ...
    if (routingDecision.fallback) {
      if (routingDecision.reason === 'no_claude_access' && ollamaProvider) {
        ...
        if (effectiveAgent && routingDecision.effectiveModel !== effectiveAgent.model) {
          log.warn(`Agent model "${effectiveAgent.model}" is not an Ollama model -- will use Ollama default`, {
    ...
    const ollamaProxyUrl =
      process.env.OLLAMA_CLAUDE_PROXY_URL ?? `http://localhost:${process.env.PORT ?? '3000'}/api/ollama/claude-proxy`;
    const baseProject =
      routingDecision.reason === 'ollama_via_claude_proxy'
        ? {
    ...
    if (provider && provider.executionMode === 'direct' && !(isOllamaProvider && ollamaProxyEnabled)) {
      this.startDirectProcessWrapped(
        session,
        effectiveProject,
        effectiveAgent,
        resolvedPrompt,
  ...
  private startSdkProcessWrapped(
    session: Session,
    project: import('../../shared/types').Project,
    ...
    if (!provider && !providerType && !hasClaudeAccess()) {
      ...
      if (ollamaFallback) {
        ...
        if (
          ...
          !effectiveAgent.model.includes(':') &&
          !effectiveAgent.model.startsWith('qwen') &&
          !effectiveAgent.model.startsWith('llama')
        ) {
  ...
  resumeProcess(session: Session, prompt?: string): void {
    ...
    if (!this.processes.has(session.id)) {
      const dbState = this.db.query('SELECT status, pid FROM sessions WHERE id = ?').get(session.id) as {
        status: string;
        pid: number | null;
      } | null;
      if (dbState?.status === 'running') {
        log.warn(
          `Session ${session.id} marked as 'running' in DB but has no in-memory process — resetting to idle before resume`,
          {
    ...
    if (recentSystemMsgs.length >= ZERO_TURN_CIRCUIT_BREAKER_THRESHOLD) {
      ...
      const allSystemMsgs = recentMessages.filter(
        (m) => m.role === 'system' && /Session (completed|exited)/.test(m.content),
      );
    ...
    const resumeExternalMcpConfigs = isRestrictedMessage
      ? []
      : session.agentId
        ? getActiveServersForAgent(this.db, session.agentId)
        : [];
    ...
    try {
      if (providerInstance && providerInstance.executionMode === 'direct') {
        ...
        const modelOverrideResume =
          session.councilRole === 'chairman' && councilModelResume ? councilModelResume : undefined;
        sp = startDirectProcess({
          session,
          project: effectiveProject,
  ...
  shutdown(): void {
    ...
    if (activeIds.length > 0) {
      ...
      this.db.query(`UPDATE sessions SET restart_pending = 1 WHERE id IN (${placeholders})`).run(...activeIds);
