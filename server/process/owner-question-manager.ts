...
export class OwnerQuestionManager {
  ...
  private persistResponse(questionId: string, response: OwnerQuestionResponse): void {
    ...
    try {
      this.db
        .query(`UPDATE owner_questions SET status = 'answered', answer = ?, resolved_at = datetime('now') WHERE id = ?`)
        .run(response.answer, questionId);
    } catch (err) {
      log.warn('Failed to persist owner question response', {
  ...
  private persistTimeout(questionId: string): void {
    ...
    try {
      this.db
        .query(`UPDATE owner_questions SET status = 'timeout', resolved_at = datetime('now') WHERE id = ?`)
        .run(questionId);
    } catch (err) {
