...
export type BudgetRejectionReason = 'INVOCATION_LIMIT' | 'UNIQUE_AGENT_LIMIT' | 'COOLDOWN';
...
export class SessionInvocationBudget {
  ...
  check(targetAgentUrl: string): BudgetCheckResult {
    ...
    if (!this.uniqueAgents.has(targetAgentUrl) && this.uniqueAgents.size >= this.config.maxUniqueAgentsPerSession) {
      return {
        allowed: false,
...
export class InboundA2ARateLimiter {
  ...
  check(sourceAgent: string): { allowed: boolean; retryAfterMs?: number } {
    ...
    const timestamps = this.windows.get(sourceAgent);
    if (!timestamps) {
      return { allowed: true };
    }
