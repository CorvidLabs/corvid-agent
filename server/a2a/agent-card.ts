...
import type { A2AAgentCard, A2AAgentSkill, A2AProtocolExtension, Agent } from '../../shared/types';
 ...
 */
export function buildAgentCardForAgent(agent: Agent, baseUrl?: string, db?: Database): A2AAgentCard {
  const port = parseInt(process.env.PORT ?? '3000', 10);
  const host = process.env.BIND_HOST || '127.0.0.1';
  const url = baseUrl ?? `http://${host}:${port}`;
  ...
  const skills = toolNames.filter((name) => TOOL_DESCRIPTIONS[name] || TOOL_TAG_MAP[name]).map(toolToSkill);
