...
import { buildCorsHeaders, checkHttpAuth, timingSafeEqual } from './auth';
 ...
 */
export function roleMiddleware(allowedRoles: string[], pathPredicate: (pathname: string) => boolean): Middleware {
  return {
    name: 'role',
    order: ORDER.ROLE,
    handler: async (ctx: MiddlewareContext, next: NextFn): Promise<void> => {
      ...
      if (!requestContext.authenticated) {
        ctx.response = new Response(JSON.stringify({ error: 'Authentication required' }), {
          status: 401,
