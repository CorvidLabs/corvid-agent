...
export function getEncryptionPassphrase(network?: string, serverMnemonic?: string | null): string {
  ...
  if (envKey && envKey.trim().length > 0) {
    ...
    if (network && network !== 'localnet' && trimmed.length < MIN_KEY_LENGTH) {
      log.warn(
        `WALLET_ENCRYPTION_KEY is only ${trimmed.length} chars — recommend at least ${MIN_KEY_LENGTH}. Generate with: openssl rand -hex 32`,
      );
    }
 ...
 */
export async function encryptMnemonicWithPassphrase(plaintext: string, passphrase: string): Promise<string> {
  const salt = crypto.getRandomValues(new Uint8Array(SALT_LENGTH));
  const key = await deriveKey(passphrase, salt, CURRENT_ITERATIONS);
  const iv = crypto.getRandomValues(new Uint8Array(IV_LENGTH));
  const encoder = new TextEncoder();
 ...
 * @returns The decrypted mnemonic plaintext.
 */
export async function decryptMnemonicWithPassphrase(encrypted: string, passphrase: string): Promise<string> {
  const combined = Uint8Array.from(atob(encrypted), (c) => c.charCodeAt(0));
