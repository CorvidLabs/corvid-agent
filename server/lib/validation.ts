 ...
 */
export async function parseBodyOrThrow<T extends z.ZodType>(req: Request, schema: T): Promise<z.infer<T>> {
  let raw: unknown;
  try {
    raw = await req.json();
  } catch {
...
export const CreateAgentSchema = z.object({
  ...
  displayColor: z
    .string()
    .regex(/^#[0-9a-fA-F]{6}$/, 'displayColor must be a hex color (e.g. #ff00aa)')
    .nullable()
    .optional(),
...
export const FundAgentSchema = z.object({
  microAlgos: z
    .number()
    .min(1000, 'microAlgos must be at least 1000')
    .max(100_000_000, 'microAlgos must be at most 100000000'),
});

export const SetSpendingCapSchema = z.object({
  dailyLimitMicroalgos: z
    .number()
    .min(0, 'dailyLimitMicroalgos must be >= 0')
    .max(1_000_000_000, 'dailyLimitMicroalgos too large'),
  dailyLimitUsdc: z
    .number()
    .min(0, 'dailyLimitUsdc must be >= 0')
    .max(1_000_000_000, 'dailyLimitUsdc too large')
    .optional()
    .default(0),
});
...
const PipelineStepSchema = z.object({
  label: z
    ...
    .min(1)
    .max(64)
    .regex(/^[a-zA-Z0-9_-]+$/, 'Label must be alphanumeric with dashes/underscores'),
...
export const CreateScheduleSchema = z
  ...
  .refine((d) => d.cronExpression || d.intervalMs || (d.triggerEvents && d.triggerEvents.length > 0), {
    message: 'Either cronExpression, intervalMs, or triggerEvents must be provided',
  })
  .refine((d) => d.executionMode !== 'pipeline' || (d.pipelineSteps && d.pipelineSteps.length >= 2), {
...
export const BulkScheduleActionSchema = z.object({
  ...
  ids: z
    ...
    .max(50, 'Maximum 50 IDs per bulk action'),
});

// ─── Webhooks ────────────────────────────────────────────────────────────────

const WebhookEventTypeSchema = z.enum(['issue_comment', 'issues', 'pull_request_review_comment', 'issue_comment_pr']);

export const CreateWebhookRegistrationSchema = z.object({
  agentId: z.string().min(1, 'agentId is required'),
  repo: z
    .string()
    .min(1, 'repo is required (format: owner/name)')
...
const PollingEventFilterSchema = z.enum(['issue_comment', 'issues', 'pull_request_review_comment', 'pull_request']);
...
export const CreateMentionPollingSchema = z.object({
  ...
  repo: z
    ...
    .regex(/^[^/]+(?:\/[^/]+)?$/, 'repo must be owner/name or org name'),
  mentionUsername: z.string().min(1, 'mentionUsername is required'),
  projectId: z.string().min(1).optional(),
  intervalSeconds: z
    .number()
    ...
    .max(3600, 'Maximum polling interval is 1 hour')
    .optional(),
...
const WorkflowNodeConfigSchema = z
  .object({
    agentId: z.string().optional(),
    projectId: z.string().optional(),
    prompt: z.string().optional(),
...
export const CreateWorkflowSchema = z
  ...
  .refine((d) => d.nodes.some((n) => n.type === 'start'), { message: 'Workflow must have at least one start node' });
...
export const CreatePersonaSchema = z.object({
  ...
  exampleMessages: z
    .array(z.string().max(500, 'each example must be 500 chars or less'))
...
export const SendA2ATaskSchema = z
  .object({
    params: z
      .object({
        message: z.string().min(1, 'message is required'),
  ...
  .refine((d) => d.params?.message?.trim() || d.message?.trim(), {
...
export const UpdateCreditConfigSchema = z
  .object({
  ...
  .strict()
  .refine((d) => Object.keys(d).length > 0, { message: 'At least one config key is required' });
...
const PluginCapabilitySchema = z.enum(['db:read', 'network:outbound', 'fs:project-dir', 'agent:read', 'session:read']);
...
export const PermissionRevokeSchema = z
  .object({
  ...
  .refine((d) => d.grant_id !== undefined || d.agent_id !== undefined, { message: 'grant_id or agent_id is required' });
