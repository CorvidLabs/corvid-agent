 ...
 */
export function scoreResponseQuality(text: string, hasToolCalls: boolean): ResponseQualityScore {
  ...
  const hasFileRefs = /(?:\/[\w.-]+){2,}|\.\/[\w.-]+|[\w-]+\.(?:ts|js|py|rs|go|json|yaml|yml|toml|md|sql)\b/.test(
    trimmed,
  );
  if (!hasFileRefs) {
  ...
  const hasConcreteRefs =
    /\b(?:function|class|const|let|var|def|fn|func|type|interface)\s+\w+/.test(trimmed) ||
...
export function buildLoopNudge(toolName: string): string {
  return (
    `STOP. You are calling \`${toolName}\` repeatedly with the same arguments. ` +
...
export class ResponseQualityTracker {
  ...
  constructor(threshold = LOW_QUALITY_THRESHOLD, trigger = CONSECUTIVE_LOW_QUALITY_TRIGGER) {
    this.threshold = threshold;
...
export function buildQualityNudge(): string {
  return (
    'STOP. You are producing filler text, not results. ' +
...
export function buildRepetitionNudge(): string {
  return (
    'CRITICAL: You are repeating yourself. Your last several responses say the same thing in different words. ' +
