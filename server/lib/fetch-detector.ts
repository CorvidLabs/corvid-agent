...
const FETCH_PATTERNS: Array<{ name: string; regex: RegExp }> = [
  ...
  {
    name: 'axios',
    regex: /axios\s*\.\s*(?:get|post|put|delete|patch|head|options|request)\s*\(\s*['"`](https?:\/\/[^'"`\s)]+)/gi,
  },
  // axios('https://...')
  ...
  {
    name: 'import',
    regex: /(?:from|require)\s*\(\s*['"](?:node-fetch|undici|axios|got|ky|superagent|needle|request)['"]/gi,
  },
];
