...
import { getKeystorePath, type KeystoreData, readKeystore } from './wallet-keystore';
...
export async function rotateWalletEncryptionKey(
  ...
  if (oldPassphrase === newPassphrase) {
    return {
      ...
      error: 'New passphrase must differ from old passphrase',
    };
  }
  ...
  if (newPassphrase.length < 32) {
    return {
      ...
      agentsRotated: 0,
      keystoreEntriesRotated: 0,
      error: 'New passphrase must be at least 32 characters',
    };
  }
  ...
  try {
    ...
    const agentRows = db
      .query('SELECT id, name, wallet_mnemonic_encrypted FROM agents WHERE wallet_mnemonic_encrypted IS NOT NULL')
    ...
    const updateStmt = db.prepare('UPDATE agents SET wallet_mnemonic_encrypted = ? WHERE id = ?');
