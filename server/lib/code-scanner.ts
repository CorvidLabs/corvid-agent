 ...
 */
const EXCLUDED_FILES: string[] = ['server/__tests__/', 'server/lib/code-scanner.ts'];
...
function isAllowed(rule: PatternRule, file: string | null): boolean {
  ...
  return rule.allowedFiles.some((allowed) => file === allowed || file.startsWith(allowed));
}
...
export function formatScanReport(result: CodeScanResult): string {
  ...
  if (criticals.length > 0) {
    lines.push('To fix: remove the flagged patterns, or refactor to use safe alternatives.');
  }
