...
export async function validateGitHubTokenOnStartup(
  ...
  try {
    ...
    if (result.missingScopes.length > 0) {
      log.warn(`GH_TOKEN is missing required OAuth scopes: ${result.missingScopes.join(', ')}`, {
        currentScopes: result.scopes,
        missingScopes: result.missingScopes,
      });
      log.warn('Some GitHub features may fail. Regenerate your token with "repo" and "read:org" scopes.');
      return;
    }
