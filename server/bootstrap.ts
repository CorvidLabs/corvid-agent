...
export async function bootstrapServices(db: Database, startTime: number): Promise<ServiceContainer> {
  ...
  if (ollamaReady) {
    ...
    const ollamaHost = process.env.OLLAMA_HOST || 'http://localhost:11434';
    log.warn(
      `Ollama unreachable at ${ollamaHost} — skipping provider registration. Start Ollama and restart to enable.`,
    );
  }
  ...
  workTaskService
    ...
    .then(() => workTaskService.pruneStaleWorktrees())
    .catch((err) =>
      log.error('Failed to recover interrupted work tasks', {
    ...
    discordBridge = new DiscordBridge(
      ...
      processManager,
      {
        ...
        allowedUserIds: process.env.DISCORD_ALLOWED_USER_IDS
          ? process.env.DISCORD_ALLOWED_USER_IDS.split(',')
              .map((s) => s.trim())
              .filter(Boolean)
          : [],
        additionalChannelIds: process.env.DISCORD_ADDITIONAL_CHANNEL_IDS
          ? process.env.DISCORD_ADDITIONAL_CHANNEL_IDS.split(',')
              .map((s) => s.trim())
              .filter(Boolean)
          : undefined,
  ...
  if (process.env.SLACK_BOT_TOKEN && process.env.SLACK_SIGNING_SECRET) {
    slackBridge = new SlackBridge(db, processManager, {
      ...
      allowedUserIds: process.env.SLACK_ALLOWED_USER_IDS
        ? process.env.SLACK_ALLOWED_USER_IDS.split(',')
            .map((s) => s.trim())
            .filter(Boolean)
        : [],
    });
  ...
  shutdownCoordinator.register({
    name: 'MarketplaceFederation',
    priority: 5,
    handler: () => marketplaceFederation.stopPeriodicSync(),
  });
  shutdownCoordinator.register({
    name: 'WorkTaskDrain',
    priority: 10,
  ...
  if (sandboxLifecycleAdapter) {
    shutdownCoordinator.register({
      name: 'SandboxLifecycleAdapter',
      priority: 14,
      handler: () => sandboxLifecycleAdapter!.stop(),
    });
  }
  if (sandboxManager) {
    shutdownCoordinator.register({
      name: 'SandboxManager',
      priority: 15,
      handler: () => sandboxManager.shutdown(),
      timeoutMs: 10_000,
    });
  }
  shutdownCoordinator.register({
    name: 'ProcessManager',
    priority: 30,
    handler: () => processManager.shutdown(),
    timeoutMs: 15_000,
  });
  shutdownCoordinator.register({
    name: 'PerformanceCollector',
    priority: 0,
    handler: () => performanceCollector.stop(),
  });
