...
export class AutoUpdateService {
  ...
  async check(): Promise<void> {
    ...
    try {
      ...
      const activeCount = queryCount(
        this.db,
        "SELECT COUNT(*) as cnt FROM sessions WHERE status = 'running' AND pid IS NOT NULL",
      );
      if (activeCount > 0) {
      ...
      if (changedFiles) {
        ...
        const installResult = Bun.spawnSync(['bun', 'install', '--frozen-lockfile', '--ignore-scripts'], {
          cwd: `${import.meta.dir}/..`,
          stdout: 'pipe',
          stderr: 'pipe',
        });
