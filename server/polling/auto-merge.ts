...
export class AutoMergeService {
  ...
  async checkAll(): Promise<void> {
    ...
    try {
      ...
      const allConfigs = this.db
        .query(`SELECT repo, mention_username FROM mention_polling_configs WHERE status = 'active'`)
  ...
  private async mergeForRepo(repo: string, username: string): Promise<void> {
    ...
    const result = await this.runGh([
      ...
      'GET',
      '-f',
      `q=${searchQualifier} is:pr is:open author:${username}`,
      '-f',
      'per_page=20',
    ]);
    ...
    for (const pr of prs) {
      ...
      const statusResult = await this.runGh([
        ...
        'state',
        '--jq',
        '[.[].state] | if length == 0 then "none" elif all(. == "SUCCESS") then "pass" else "fail" end',
      ]);
      ...
      if (rejection) {
        ...
        if (!this.flaggedPRs.has(prKey)) {
          await this.runGh([
            ...
            prRepo,
            '--body',
            `Auto-merge skipped — security scan flagged issues:\n\n${rejection}\n\nA human must review and merge this PR manually.`,
          ]);
