...
const CONFIG_FILE_NAMES = ['corvid-agent.config.ts', 'corvid-agent.config.js', 'corvid-agent.config.json'];
...
export function configFromEnv(): AgentDeploymentConfig {
  ...
  return {
    ...
    work: {
      maxIterations: env.WORK_MAX_ITERATIONS
        ? parseInt(env.WORK_MAX_ITERATIONS, 10)
        : CONFIG_DEFAULTS.work.maxIterations,
...
export function validateConfig(config: AgentDeploymentConfig): ConfigValidationError[] {
  ...
  if (config.providers.enabledProviders.includes('anthropic') && !config.providers.anthropic?.apiKey) {
    errors.push({
      ...
      message: 'Anthropic API key is required when anthropic provider is enabled',
    });
  }
