...
export function getGrantedCapabilities(db: Database, pluginName: string): PluginCapability[] {
  const rows = db
    .query('SELECT capability FROM plugin_capabilities WHERE plugin_name = ? AND granted = 1')
...
export function revokeCapability(db: Database, pluginName: string, capability: PluginCapability): void {
  db.query('UPDATE plugin_capabilities SET granted = 0 WHERE plugin_name = ? AND capability = ?').run(
    pluginName,
    capability,
  );
...
export function hasCapability(db: Database, pluginName: string, capability: PluginCapability): boolean {
  const row = db
    .query('SELECT granted FROM plugin_capabilities WHERE plugin_name = ? AND capability = ?')
