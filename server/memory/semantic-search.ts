 ...
 */
export function fastSearch(db: Database, agentId: string, query: string, opts: SearchOptions = {}): ScoredMemory[] {
  ...
  for (let i = 0; i < filtered.length; i++) {
    ...
    const score = hasNonZeroTfidf ? 0.4 * fts5Score + 0.6 * tfidfScore : fts5Score;
 ...
 */
export type DeepSearchFn = (query: string, candidates: AgentMemory[], limit: number) => Promise<ScoredMemory[]>;
...
function getCategoryMap(db: Database, memoryIds: string[]): Map<string, string> {
  ...
  try {
    ...
    const rows = db
      .query(`SELECT memory_id, category FROM memory_categories WHERE memory_id IN (${placeholders})`)
