 ...
 *   - Expiring short-term agent_memories past their TTL
 *   - Purging archived short-term memories after 30 days
 */
...
export class MemoryGraduationService {
  ...
  async tick(): Promise<void> {
    ...
    try {
      ...
      // 3. Expire short-term agent memories past their TTL
      ...
      // 4. Purge archived short-term memories older than 30 days
      const purgedMemories = purgeOldArchivedMemories(this.db);
      if (purgedMemories > 0) {
        log.info('Purged old archived memories', { count: purgedMemories });
      }
