 ...
 */
export function summarizeOldMemories(db: Database, agentId: string, olderThanDays: number = 30): number {
  ...
  for (const [category, memories] of groups) {
    ...
    const summaryContent = `Summary of ${memories.length} "${category}" memories (archived ${now}):\n${summaryLines.join('\n')}`;
    ...
    db.query(`UPDATE agent_memories SET archived = 1 WHERE id IN (${placeholders})`).run(...ids);
