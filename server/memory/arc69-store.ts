...
async function encryptContent(content: string, account: ChatAccount): Promise<string> {
  ...
  const envelope = encryptMessage(content, account.encryptionKeys.publicKey, account.encryptionKeys.publicKey);
...
async function decryptContent(envelopeB64: string, account: ChatAccount): Promise<string | null> {
  ...
  const result = decryptMessage(envelope, account.encryptionKeys.privateKey, account.encryptionKeys.publicKey);
...
export async function deleteMemoryAsa(
  ...
  // biome-ignore lint/suspicious/noImplicitAnyLet: algosdk Transaction type from dynamic import
  let txn;
  if (mode === 'hard') {
 ...
 */
export async function readMemoryAsa(ctx: Arc69Context, asaId: number): Promise<Arc69Memory | null> {
  try {
