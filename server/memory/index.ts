...
import { listMemories, recallMemory, saveMemory } from '../db/agent-memories';
import { createLogger } from '../lib/logger';
import type { LRUCacheOptions } from './cache';
import { LRUCache } from './cache';
import type { CategoryResult, MemoryCategory } from './categories';
import { categorize } from './categories';
import { IDFCorpus, tokenize } from './embeddings';
import { ensureMemorySchema } from './schema';
import type { DeepSearchFn, ScoredMemory, SearchMode, SearchOptions } from './semantic-search';
import { fastSearch, search as semanticSearch } from './semantic-search';
...
export class MemoryManager {
   ...
   */
  async search(agentId: string, query: string, opts: SearchOptions = {}): Promise<SearchResult> {
    this.ensureSchema();

    const mode = opts.mode ?? 'fast';

    const memories = await semanticSearch(this.db, agentId, query, opts, this.deepSearchFn);
   ...
   */
  searchFast(agentId: string, query: string, opts: Omit<SearchOptions, 'mode'> = {}): ScoredMemory[] {
    this.ensureSchema();
  ...
  private getCategory(memoryId: string): { category?: MemoryCategory; categoryConfidence?: number } {
    try {
      const row = this.db
        .query('SELECT category, confidence FROM memory_categories WHERE memory_id = ?')
  ...
  private getOrCreateCorpus(agentId: string): IDFCorpus {
    ...
    try {
      const memories = this.db
        .query('SELECT key, content FROM agent_memories WHERE agent_id = ?')
  ...
  private updateEmbedding(memory: AgentMemory): void {
    try {
      ...
      this.db
        ...
        .run(memory.id, JSON.stringify(vectorObj), tokens.slice(0, 50).join(','));
    } catch (err) {
...
export type { CategoryResult, MemoryCategory } from './categories';
export { allCategories, categorize } from './categories';
export { cosineSimilarityDense, cosineSimilaritySparse, IDFCorpus, tokenize } from './embeddings';
export type { DeepSearchFn, ScoredMemory, SearchMode, SearchOptions } from './semantic-search';
export { deepSearch, fastSearch } from './semantic-search';
