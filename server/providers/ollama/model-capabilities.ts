...
const TOOL_INCAPABLE_PATTERNS = [/embed/i, /nomic/i, /mxbai/i, /all-minilm/i, /snowflake/i, /bge/i];

/** Models known to support vision. */
const VISION_PATTERNS = [/llava/i, /bakllava/i, /moondream/i, /llama.*vision/i];
...
export class ModelCapabilityDetector {
  ...
  private parseCapabilities(name: string, data: OllamaShowResponse): ModelCapabilities {
    ...
    const supportsVision =
      VISION_PATTERNS.some((p) => p.test(name)) || allFamilies.some((f) => f === 'clip' || f === 'mllama');

    // Embedding model detection
    const isEmbeddingModel =
      TOOL_INCAPABLE_PATTERNS.some((p) => p.test(name)) || allFamilies.some((f) => f === 'bert' || f === 'nomic-bert');
  ...
  private detectToolSupport(name: string, families: string[], template?: string): boolean {
  ...
  private inferFromName(name: string): ModelCapabilities {
    ...
    else if (lowerName.includes('mistral') || lowerName.includes('devstral'))
      family = lowerName.includes('devstral') ? 'devstral' : 'mistral';
