...
import type { LlmToolCall, LlmToolDefinition } from '../types';
...
export function extractToolCallsFromContent(content: string, tools?: LlmToolDefinition[]): LlmToolCall[] {
  ...
  if (pythonTagMatch) {
    ...
    let match: RegExpExecArray | null = fnPattern.exec(body);
    while (match !== null) {
      const fnName = match[1];
      const argsStr = match[2].trim();
      if (toolNames.has(fnName)) {
        try {
  ...
  if (calls.length === 0) {
    for (const toolName of toolNames) {
      ...
      let jMatch: RegExpExecArray | null = jsonPattern.exec(content);
      while (jMatch !== null) {
        try {
        ...
        jMatch = jsonPattern.exec(content);
      }
    }
  }
  ...
  if (calls.length === 0) {
    for (const toolName of toolNames) {
      const pyPattern = new RegExp(`${toolName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*\\(([\\s\\S]*?)\\)`, 'g');
      let pyMatch: RegExpExecArray | null = pyPattern.exec(content);
      while (pyMatch !== null) {
        ...
        pyMatch = pyPattern.exec(content);
      }
    }
  }
...
export function parsePythonArgs(argsStr: string): Record<string, unknown> {
  ...
  let kwMatch: RegExpExecArray | null = kwargPattern.exec(argsStr);
  while (kwMatch !== null) {
    const key = kwMatch[1];
    ...
    kwMatch = kwargPattern.exec(argsStr);
  }
  return result;
}
 ...
 */
export function normalizeToolArgs(args: Record<string, unknown>, toolDef: LlmToolDefinition): Record<string, unknown> {
