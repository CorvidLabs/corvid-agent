...
import { estimateCost, MODEL_PRICING } from './cost-table';
import { DEFAULT_FALLBACK_CHAINS, type FallbackChain, FallbackManager } from './fallback';
...
export function estimateComplexity(prompt: string): { level: ComplexityLevel; signals: ComplexitySignals } {
  ...
  const requiresThinking =
    complexityKeywords >= 3 || multiStep || prompt.length > 2000 || lower.includes('reason') || lower.includes('think');
...
export class ModelRouter {
  ...
  selectModel(
    ...
    if (candidates.length === 0) {
      ...
      const fallback = MODEL_PRICING.filter((m) => this.registry.get(m.provider)).sort(
        (a, b) => a.outputPricePerMillion - b.outputPricePerMillion,
      )[0];
   ...
   */
  selectModelByTier(tier: ModelTier): { model: string; provider: LlmProviderType } {
