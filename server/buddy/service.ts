...
import { BUDDY_DEFAULT_MCP_TOOLS, BUDDY_DEFAULT_TOOLS } from '../../shared/types/buddy';
...
import { addBuddyMessage, createBuddySession, getBuddySession, updateBuddySessionStatus } from '../db/buddy';
...
export class BuddyService {
  ...
  onSessionUpdate(cb: (session: BuddySession) => void): () => void {
    ...
    return () => {
      this.sessionUpdateListeners.delete(cb);
    };
  }
  ...
  private async runConversationLoop(buddySessionId: string, onRoundComplete?: BuddyRoundCallback): Promise<void> {
    ...
    let lastLeadOutput = await this.runAgentTurn(buddySessionId, session.leadAgentId, 'lead', 1, session.prompt);
    ...
    for (let round = 1; round <= session.maxRounds; round++) {
      ...
      const buddyOutput = await this.runAgentTurn(buddySessionId, session.buddyAgentId, 'buddy', round, buddyPrompt);
      ...
      const leadPrompt = this.buildLeadRevisionPrompt(session.prompt, buddyAgent.name, buddyOutput, round + 1);

      lastLeadOutput = await this.runAgentTurn(buddySessionId, session.leadAgentId, 'lead', round + 1, leadPrompt);
    ...
    return new Promise((resolve) => {
      ...
      const callback = (sid: string, event: ClaudeStreamEvent) => {
        ...
        if (event.type === 'result') {
          ...
          done(text || resultText || null);
        }

        if (event.type === 'session_exited' || event.type === 'session_error') {
  ...
  private async invokeRoundCallback(callback: BuddyRoundCallback | undefined, event: BuddyRoundEvent): Promise<void> {
    if (!callback) return;
    try {
