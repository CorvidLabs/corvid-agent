...
import { SELF_TEST_AGENT, SELF_TEST_PROJECT } from './config';
...
export class SelfTestService {
  ...
  run(testType: 'unit' | 'e2e' | 'all'): { sessionId: string } {
    ...
    switch (testType) {
      ...
      case 'e2e':
        prompt =
          'Run the end-to-end tests with `npx playwright test --config=playwright.config.js`. If any tests fail, analyze the failures, fix the source code, and re-run to verify the fix.';
        break;
      case 'all':
        prompt =
