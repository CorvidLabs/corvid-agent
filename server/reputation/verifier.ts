...
export class ReputationVerifier {
  ...
  constructor(indexerBaseUrl?: string) {
    this.indexerBaseUrl = indexerBaseUrl ?? process.env.ALGORAND_INDEXER_URL ?? 'https://mainnet-idx.algonode.cloud';
  }
    ...
    try {
      ...
      for (const txn of data.transactions ?? []) {
        ...
        attestations.push({
          ...
          timestamp: txn['round-time'] ? new Date(txn['round-time'] * 1000).toISOString() : '',
        });
      }
   ...
   */
  async checkRemoteTrust(walletAddress: string, minTrust: TrustLevel = 'low'): Promise<RemoteTrustResult> {
