...
export type VerificationTier = 'UNVERIFIED' | 'GITHUB_VERIFIED' | 'OWNER_VOUCHED' | 'ESTABLISHED';
...
export class IdentityVerification {
  ...
  getTier(agentId: string): VerificationTier {
    const row = this.db.query('SELECT tier FROM agent_identity WHERE agent_id = ?').get(agentId) as {
      tier: string;
    } | null;
  ...
  getIdentity(agentId: string): AgentIdentity | null {
    const row = this.db
      .query('SELECT * FROM agent_identity WHERE agent_id = ?')
  ...
  evaluateEstablished(agentId: string): VerificationTier {
    ...
    // Check age
    const agent = this.db.query('SELECT created_at FROM agents WHERE id = ?').get(agentId) as {
      created_at: string;
    } | null;
    ...
    const scoreRow = this.db.query('SELECT overall_score FROM agent_reputation WHERE agent_id = ?').get(agentId) as {
  ...
  getAllIdentities(): AgentIdentity[] {
    const rows = this.db.query('SELECT * FROM agent_identity ORDER BY updated_at DESC').all() as AgentIdentityRecord[];
