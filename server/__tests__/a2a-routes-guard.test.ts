...
beforeAll(() => {
    ...
    db.run(
        `INSERT INTO agents (id, name, default_project_id) VALUES (?, ?, ?)`,
        ['agent-1', 'TestAgent', 'proj-1'],
    );
});
    ...
    it('returns 429 when source agent exceeds rate limit', async () => {
        ...
        for (let i = 0; i < 5; i++) {
            const { req, url } = makeRequest(
                { params: { message: `msg-${i}` } },
                { 'x-source-agent': 'spammer-agent' },
            );
