...
import { MarketplaceService, InsufficientCreditsError, RateLimitExceededError } from '../marketplace/service';
...
describe('MarketplaceService — Pricing Tiers', () => {
    ...
    test('createTier throws for nonexistent listing', () => {
        expect(() => svc.createTier('nonexistent', {
            name: 'Basic', priceCredits: 5,
    ...
    test('createTier enforces max 5 tiers per listing', () => {
        ...
        expect(() => svc.createTier(listingId, {
            name: 'Too Many', priceCredits: 100,
    ...
    test('recordTierUse throws InsufficientCreditsError', () => {
        ...
        expect(() => svc.recordTierUse(listingId, tier.id, 'buyer-1'))
    ...
    test('recordTierUse fails for wrong listing', () => {
        const otherListing = svc.createListing({
            agentId: 'agent-2', name: 'Other', description: 'Other', category: 'general',
        });
        const tier = svc.createTier(listingId, { name: 'Basic', priceCredits: 5 });
