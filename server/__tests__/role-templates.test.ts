...
describe('applyRoleTemplate', () => {
    ...
    test('throws for unknown template name', async () => {
        expect(applyRoleTemplate(db, AGENT_ID, 'unknown', 'owner')).rejects.toThrow(
            'Unknown role template: "unknown"',
        );
    });
...
describe('revokeRoleTemplate', () => {
    ...
    test('throws for unknown template name', () => {
        expect(() => revokeRoleTemplate(db, AGENT_ID, 'unknown', 'admin')).toThrow(
            'Unknown role template: "unknown"',
        );
    });
