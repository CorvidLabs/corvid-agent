...
describe('summarizeOldMemories', () => {
  it('archives old memories and creates a summary per category', () => {
    ...
    const originals = db
      .query(`SELECT archived FROM agent_memories WHERE agent_id = ? AND key IN ('project:alpha', 'project:beta')`)
      .all(agent) as { archived: number }[];
    expect(originals.every((r) => r.archived === 1)).toBe(true);

    // A summary memory should have been created
    const summaries = db
      .query(`SELECT key, content FROM agent_memories WHERE agent_id = ? AND key LIKE 'summary:project:%'`)
  ...
  it('skips groups with only 1 memory', () => {
    ...
    const configRow = db
      .query(`SELECT archived FROM agent_memories WHERE agent_id = ? AND key = 'config:only'`)
  ...
  it('does not archive recent memories', () => {
    ...
    const rows = db.query(`SELECT archived FROM agent_memories WHERE agent_id = ?`).all(agent) as {
      archived: number;
    }[];
  ...
  it('skips already-archived memories', () => {
    ...
    const summaries = db
      .query(`SELECT id FROM agent_memories WHERE agent_id = ? AND key LIKE 'summary:%'`)
