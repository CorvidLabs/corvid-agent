...
import { CAPABILITIES, CapabilityRouter } from '../flock-directory/capability-router';
...
describe('repo conflict filtering', () => {
  test('excludes agents that already have claims on the same repo', () => {
    ...
    expect(result.bestCandidate).toBeNull();
    expect(result.exclusions.some((e) => e.agentName === 'Bob' && e.reason.includes('already working'))).toBe(true);
  });
});
