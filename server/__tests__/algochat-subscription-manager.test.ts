...
/** Extract typed LocalChatEvent[] from a mocked LocalChatEventFn's call args. */
function eventCalls(fn: LocalChatEventFn): LocalChatEvent[] {
  return (fn as ReturnType<typeof mock>).mock.calls.map((c: unknown[]) => c[0] as LocalChatEvent);
}
...
describe('subscribeForResponse', () => {
  ...
  describe('ack delay logic', () => {
    ...
    test('emits processing status only once for multiple assistant events', () => {
      ...
      const processingEvents = rf._events.filter((e) => e.direction === 'status' && e.content.includes('processing'));
    ...
    test('sends ack immediately when tool_status event arrives', async () => {
      ...
      const ackResponse = rf._responses.find((r) => r.content.includes('Received your message'));
  ...
  describe('progress tracking', () => {
    ...
    test('emits synthesizing status on result when agent queries were made and enough time elapsed', async () => {
      ...
      const synthEvents = rf._events.filter((e) => e.direction === 'status' && e.content.includes('Synthesizing'));
    ...
    test('flushes pending text block before tool_use starts', () => {
      ...
      const statusEvents = rf._events.filter((e) => e.direction === 'status' && e.content.includes('reasoning text'));
  ...
  describe('tool_status event forwarding', () => {
    test('forwards tool_status messages as status events', () => {
      ...
      const statusEvents = rf._events.filter((e) => e.direction === 'status' && e.content === 'Querying CorvidLabs...');
      expect(statusEvents.length).toBe(1);
    });
  ...
  describe('content block lifecycle', () => {
    ...
    test('ignores content_block_delta when not in a text block', () => {
      ...
      const textEvents = rf._events.filter((e) => e.direction === 'status' && e.content.includes('orphan'));
...
describe('subscribeForLocalResponse', () => {
  ...
  describe('streaming events', () => {
    test('emits thinking events on first assistant event', () => {
      ...
      const events = eventCalls(eventFn);
      expect(events.some((e) => e.type === 'thinking' && e.active === true)).toBe(true);
    });
    ...
    test('emits thinking only once for multiple assistant events', () => {
      ...
      const activeThinking = eventCalls(eventFn).filter((e) => e.type === 'thinking' && e.active === true);
      expect(activeThinking.length).toBe(1);
    });
    ...
    test('emits stream chunks for content_block_delta', () => {
      ...
      const streamEvents = eventCalls(eventFn).filter((e) => e.type === 'stream' && !e.done);
      expect(streamEvents.length).toBe(2);
      expect((streamEvents[0] as Extract<LocalChatEvent, { type: 'stream' }>).chunk).toBe('chunk1');
      expect((streamEvents[1] as Extract<LocalChatEvent, { type: 'stream' }>).chunk).toBe('chunk2');
    });
    ...
    test('emits stream done on result', () => {
      ...
      const doneEvents = eventCalls(eventFn).filter((e) => e.type === 'stream' && e.done);
    ...
    test('emits thinking=false on result', () => {
      ...
      expect((lastThinking as Extract<LocalChatEvent, { type: 'thinking' }>).active).toBe(false);
    });
    ...
    test('emits tool_use events for content_block_start with tool_use', () => {
      ...
      const toolEvents = eventCalls(eventFn).filter((e) => e.type === 'tool_use');
      expect(toolEvents.length).toBe(1);
      expect((toolEvents[0] as Extract<LocalChatEvent, { type: 'tool_use' }>).toolName).toBe('Read');
    });
    ...
    test('emits tool_use for direct-mode tool_status events', () => {
      ...
      const toolEvents = eventCalls(eventFn).filter((e) => e.type === 'tool_use');
      expect(toolEvents.length).toBe(1);
      const toolEvt = toolEvents[0] as Extract<LocalChatEvent, { type: 'tool_use' }>;
      expect(toolEvt.toolName).toBe('Bash');
      expect(toolEvt.input).toBe('Running npm test');
    });
    ...
    test('emits thinking events for direct-mode thinking signal', () => {
      ...
      const thinkingEvents = eventCalls(eventFn).filter((e) => e.type === 'thinking');
      expect(thinkingEvents.some((e) => (e as Extract<LocalChatEvent, { type: 'thinking' }>).active === true)).toBe(
        true,
      );
    });
    ...
    test('emits message event on turn completion with text', () => {
      ...
      const messageEvents = eventCalls(eventFn).filter((e) => e.type === 'message');
      expect(messageEvents.length).toBe(1);
      const msgEvt = messageEvents[0] as Extract<LocalChatEvent, { type: 'message' }>;
      expect(msgEvt.content).toBe('response text');
      expect(msgEvt.direction).toBe('outbound');
    });
    ...
    test('emits message event on session_exited with remaining buffer', () => {
      ...
      const messageEvents = eventCalls(eventFn).filter((e) => e.type === 'message');
      expect(messageEvents.length).toBe(1);
      expect((messageEvents[0] as Extract<LocalChatEvent, { type: 'message' }>).content).toBe('final text');
    });
...
describe('updateLocalEventFn', () => {
  test('updates the event function for an existing session', () => {
    ...
    const fn1Streams = eventCalls(eventFn1).filter(
      (e) => e.type === 'stream' && (e as Extract<LocalChatEvent, { type: 'stream' }>).chunk === 'chunk1',
    );
    const fn2Streams = eventCalls(eventFn2).filter(
      (e) => e.type === 'stream' && (e as Extract<LocalChatEvent, { type: 'stream' }>).chunk === 'chunk2',
    );
    expect(fn1Streams.length).toBe(1);
    expect(fn2Streams.length).toBe(1);
  });
