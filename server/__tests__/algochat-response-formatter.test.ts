...
import { createConversation, createSession } from '../db/sessions';
...
function createMockTransactor(overrides: Partial<Record<string, unknown>> = {}) {
  return {
    sendToAddress: mock(
      (overrides.sendToAddress as (...args: unknown[]) => Promise<unknown>) ?? (() => Promise.resolve({ fee: 1000 })),
    ),
...
describe('ResponseFormatter', () => {
  ...
  describe('sendResponse', () => {
    ...
    test('should split large PSK messages into chunks', async () => {
      ...
      const sentContent = pskManager.sendMessage.mock.calls.map((c: unknown[]) => c[0]).join('');
