...
import type { ResourceLimits, SandboxConfig } from '../sandbox/types';
...
describe('sandbox/container', () => {
  ...
  describe('getContainerStatus', () => {
    ...
    it('sends correct docker inspect command', async () => {
      ...
      expect(spawnCalls).toHaveLength(1);
      expect(spawnCalls[0].cmd).toEqual(['docker', 'inspect', '--format', '{{json .}}', 'my-container-id']);
    });
  });
  ...
  describe('isDockerAvailable', () => {
    it('returns true when docker version succeeds', async () => {
      ...
      expect(spawnCalls[0].cmd).toEqual(['docker', 'version', '--format', '{{.Server.Version}}']);
    });
    ...
    it('returns false when spawn throws', async () => {
      ...
      (
        Bun.spawn as unknown as { mockImplementation: (fn: (...args: unknown[]) => unknown) => void }
      ).mockImplementation(() => {
