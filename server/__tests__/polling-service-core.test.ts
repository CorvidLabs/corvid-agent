...
describe('pollConfig components', () => {
  ...
  test('filterNewMentions filters already-processed IDs', () => {
    ...
    const mentions = [makeMention({ id: 'a' }), makeMention({ id: 'b' }), makeMention({ id: 'c' })];
  ...
  test('updatePollState updates timestamp in DB', () => {
    ...
    const row = db
      .query('SELECT last_poll_at, last_seen_id FROM mention_polling_configs WHERE id = ?')
      .get(config.id) as { last_poll_at: string; last_seen_id: string };
  ...
  test('updateProcessedIds persists ID set to DB', () => {
    ...
    const row = db.query('SELECT processed_ids FROM mention_polling_configs WHERE id = ?').get(config.id) as {
...
describe('processMention', () => {
  ...
  test('increments trigger count on successful trigger', async () => {
    ...
    const row = db.query('SELECT trigger_count FROM mention_polling_configs WHERE id = ?').get(config.id) as {
  ...
  test('returns false when session creation fails', async () => {
    ...
    mockStartProcess.mockImplementation(() => {
...
describe('scheduler integration', () => {
  test('setSchedulerService stores reference', () => {
    ...
    } as unknown as import('../scheduler/service').SchedulerService;
