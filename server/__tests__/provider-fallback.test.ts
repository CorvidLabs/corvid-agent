...
function mockProvider(
  result: { content: string; model: string; usage?: { inputTokens: number; outputTokens: number } } | Error,
) {
  return {
...
describe('FallbackManager', () => {
  ...
  describe('completeWithFallback', () => {
    ...
    test('throws ExternalServiceError when all providers fail', async () => {
      ...
      await expect(fm.completeWithFallback({ messages: [] } as any, TWO_PROVIDER_CHAIN)).rejects.toBeInstanceOf(
  ...
  describe('isProviderAvailable', () => {
    ...
    test('returns false when provider is in cooldown', async () => {
      ...
      for (let i = 0; i < 3; i++) {
        try {
          await fm.completeWithFallback({ messages: [] } as any, singleChain);
        } catch {}
      }
  ...
  describe('Ollama-specific HTTP error classification', () => {
    ...
    test('Ollama HTTP 500 (internal error) is treated as transient — triggers cooldown after 3 failures', async () => {
      ...
      try {
        await localFm.completeWithFallback({ messages: [] } as any, OLLAMA_CHAIN);
      } catch {}
