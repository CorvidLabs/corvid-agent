...
const mockStarRepo = mock(() =>
  Promise.resolve({ ok: true, message: 'Starred test/repo', error: undefined as string | undefined }),
);
const mockUnstarRepo = mock(() =>
  Promise.resolve({ ok: true, message: 'Unstarred test/repo', error: undefined as string | undefined }),
);
const mockForkRepo = mock(() =>
  Promise.resolve({
    ok: true,
    message: 'Forked test/repo',
    forkUrl: 'https://github.com/me/repo',
    error: undefined as string | undefined,
  }),
);
const mockListOpenPrs = mock(() =>
  Promise.resolve({ ok: true, prs: [] as Array<Record<string, unknown>>, error: undefined as string | undefined }),
);
const mockCreatePr = mock(() =>
  Promise.resolve({ ok: true, prUrl: 'https://github.com/test/repo/pull/1', error: undefined as string | undefined }),
);
...
const mockListIssues = mock(() =>
  Promise.resolve({ ok: true, issues: [] as Array<Record<string, unknown>>, error: undefined as string | undefined }),
);
const mockGetRepoInfo = mock(() =>
  Promise.resolve({ ok: true, info: { name: 'repo', stargazerCount: 42 }, error: undefined as string | undefined }),
);
const mockGetPrDiff = mock(() =>
  Promise.resolve({
    ok: true,
...
beforeEach(() => {
  ...
  mockForkRepo.mockReset().mockResolvedValue({
    ok: true,
    message: 'Forked test/repo',
    forkUrl: 'https://github.com/me/repo',
    error: undefined,
  ...
  mockCreatePr
    .mockReset()
    .mockResolvedValue({ ok: true, prUrl: 'https://github.com/test/repo/pull/1', error: undefined });
  mockAddPrReview.mockReset().mockResolvedValue({ ok: true, error: undefined });
  mockCreateIssue
    .mockReset()
    .mockResolvedValue({ ok: true, issueUrl: 'https://github.com/test/repo/issues/1', error: undefined });
  mockListIssues.mockReset().mockResolvedValue({ ok: true, issues: [], error: undefined });
  mockGetRepoInfo
    .mockReset()
    .mockResolvedValue({ ok: true, info: { name: 'repo', stargazerCount: 42 }, error: undefined });
  mockGetPrDiff
    .mockReset()
...
describe('formatCoAuthoredBy (#1576)', () => {
  test('formats Co-Authored-By with agent name and friendly model', () => {
    expect(formatCoAuthoredBy({ name: 'Rook', model: 'claude-sonnet-4-6' })).toBe('Co-Authored-By: Rook (Sonnet 4.6)');
  });

  test('formats Co-Authored-By with non-Claude model', () => {
    expect(formatCoAuthoredBy({ name: 'Condor', model: 'nemotron' })).toBe('Co-Authored-By: Condor (nemotron)');
  });
