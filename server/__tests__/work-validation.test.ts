...
describe('runValidation', () => {
  ...
  test('passes when all checks succeed in a valid project', async () => {
    ...
    await git.exited;
    const gitConfig1 = Bun.spawn(['git', 'config', 'user.email', 'test@test.com'], {
      cwd: tempDir,
      stdout: 'pipe',
      stderr: 'pipe',
    });
    await gitConfig1.exited;
    const gitConfig2 = Bun.spawn(['git', 'config', 'user.name', 'Test'], {
      cwd: tempDir,
      stdout: 'pipe',
      stderr: 'pipe',
    });
    ...
    await gitAdd.exited;
    const gitCommit = Bun.spawn(['git', 'commit', '-m', 'init', '--allow-empty'], {
      cwd: tempDir,
      stdout: 'pipe',
      stderr: 'pipe',
    });
