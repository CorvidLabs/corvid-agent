...
describe('verifyWebhookSignature', () => {
  ...
  it('rejects invalid signature format (missing t=)', async () => {
    await expect(verifyWebhookSignature('{}', 'v1=abc123', secret)).rejects.toThrow('Invalid webhook signature format');
  });

  it('rejects invalid signature format (missing v1=)', async () => {
    await expect(verifyWebhookSignature('{}', 't=12345', secret)).rejects.toThrow('Invalid webhook signature format');
  });
  ...
  it('rejects wrong signature', async () => {
    ...
    await expect(verifyWebhookSignature('{"type":"test"}', header, secret)).rejects.toThrow(
  ...
  it('rejects stale timestamp (older than 5 minutes)', async () => {
    ...
    await expect(verifyWebhookSignature(payload, header, secret)).rejects.toThrow('Webhook timestamp too old');
  });
  ...
  it('rejects payload tampered after signing', async () => {
    ...
    await expect(verifyWebhookSignature(tamperedPayload, header, secret)).rejects.toThrow(
      'Webhook signature verification failed',
    );
  });
});
