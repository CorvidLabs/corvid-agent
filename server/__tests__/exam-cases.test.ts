...
describe('examCases structure', () => {
  test('has exactly 40 cases', () => {
    expect(examCases.length).toBe(40);
  });

  test('has expected cases per category', () => {
    for (const cat of ['coding', 'context', 'council', 'instruction']) {
      expect(getCasesByCategory(cat).length).toBe(6);
    }
    for (const cat of ['tools', 'algochat']) {
      expect(getCasesByCategory(cat).length).toBe(5);
    }
    for (const cat of ['collaboration', 'reasoning']) {
...
describe('council-04 Consensus Building', () => {
  ...
  test('passes with compromise mentioning both sides', () => {
    const result = makeResponse({
      content:
        'I understand the concern about Friday deploys and the risk of weekend incidents without support. However, maintaining deployment momentum is important. A compromise would be to schedule deploys on Thursday afternoons, giving the team Friday as a buffer.',
    });
    expect(gradeCase(result).passed).toBe(true);
  });
...
describe('council-05 Scope Discipline', () => {
  ...
  test('passes with no tool calls', () => {
    const result = makeResponse({
      content:
        'A monorepo structure offers several advantages for code sharing. Regarding running tests, as a council member I should focus on deliberation rather than executing commands.',
    });
    expect(gradeCase(result).passed).toBe(true);
  });
