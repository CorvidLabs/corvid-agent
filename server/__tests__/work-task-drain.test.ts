...
import { getActiveWorkTasks, getWorkTask, updateWorkTaskStatus } from '../db/work-tasks';
...
describe('shuttingDown flag', () => {
  ...
  test('blocks new task creation when shutting down', async () => {
    ...
    await expect(service.create({ agentId: agent.id, description: 'New task', projectId: project.id })).rejects.toThrow(
      /shutting down/i,
    );
  });
});
