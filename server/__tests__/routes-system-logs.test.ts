...
beforeAll(() => {
  ...
  db.query("INSERT INTO sessions (id, project_id, status) VALUES (?, ?, 'running')").run(sessId, projectId);
  db.query("INSERT INTO escalation_queue (session_id, tool_name, status) VALUES (?, 'bash', 'pending')").run(sessId);
