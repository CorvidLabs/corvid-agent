...
describe('AddRepoBlocklistSchema', () => {
    it('accepts repo with reason', () => {
        expect(AddRepoBlocklistSchema.safeParse({ repo: 'owner/repo', reason: 'spam', source: 'manual' }).success).toBe(true);
    });
...
describe('SubmitFeedbackSchema', () => {
    ...
    it('rejects comment over 500 chars', () => {
        expect(SubmitFeedbackSchema.safeParse({ agentId: 'a1', sentiment: 'positive', comment: 'x'.repeat(501) }).success).toBe(false);
    });
    it('accepts comment at exactly 500 chars', () => {
        expect(SubmitFeedbackSchema.safeParse({ agentId: 'a1', sentiment: 'positive', comment: 'x'.repeat(500) }).success).toBe(true);
    });
...
describe('SetSandboxPolicySchema', () => {
    it('accepts valid policy', () => {
        expect(SetSandboxPolicySchema.safeParse({ cpuLimit: 2, memoryLimitMb: 512, networkPolicy: 'restricted' }).success).toBe(true);
    });
...
describe('DeviceAuthorizeSchema', () => {
    it('accepts valid input', () => {
        expect(DeviceAuthorizeSchema.safeParse({ userCode: 'ABC-123', tenantId: 't1', email: 'user@example.com', approve: true }).success).toBe(true);
    });
    it('rejects non-boolean approve', () => {
        expect(DeviceAuthorizeSchema.safeParse({ userCode: 'ABC', tenantId: 't1', email: 'a@b.c', approve: 'yes' }).success).toBe(false);
    });
