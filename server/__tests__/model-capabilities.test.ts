...
describe('findBestModel', () => {
  ...
  it('finds first tool-capable model', async () => {
    const result = await detector.findBestModel(['nomic-embed-text', 'phi3:mini', 'llama3.1:8b'], { tools: true });
    expect(result).toBe('llama3.1:8b');
  });

  it('returns null when no model matches', async () => {
    const result = await detector.findBestModel(['nomic-embed-text', 'mxbai-embed-large'], { tools: true });
    expect(result).toBeNull();
  });

  it('filters by vision requirement', async () => {
    const result = await detector.findBestModel(['llama3.1:8b', 'llava:13b'], { vision: true });
    expect(result).toBe('llava:13b');
  });

  it('filters by minimum context', async () => {
    const result = await detector.findBestModel(['nomic-embed-text', 'llama3.1:8b'], { minContext: 2048 });
    // nomic has 512, llama has 4096
    expect(result).toBe('llama3.1:8b');
  });

  it('returns first model when no requirements', async () => {
    const result = await detector.findBestModel(['model-a', 'model-b'], {});
