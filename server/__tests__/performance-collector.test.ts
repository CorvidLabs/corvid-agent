...
describe('PerformanceCollector', () => {
  ...
  describe('collect', () => {
    ...
    it('persists expected metric names', async () => {
      await collector.collect();
      const metrics = db.query('SELECT DISTINCT metric FROM performance_metrics ORDER BY metric').all() as {
        metric: string;
      }[];
    ...
    it('stores correct units', async () => {
      await collector.collect();
      const memRow = db.query("SELECT unit FROM performance_metrics WHERE metric = 'memory_rss' LIMIT 1").get() as {
        unit: string;
      };
      expect(memRow.unit).toBe('bytes');

      const latRow = db.query("SELECT unit FROM performance_metrics WHERE metric = 'db_latency' LIMIT 1").get() as {
        unit: string;
      };
      expect(latRow.unit).toBe('ms');

      const upRow = db.query("SELECT unit FROM performance_metrics WHERE metric = 'uptime' LIMIT 1").get() as {
        unit: string;
      };
  ...
  describe('recordSlowQuery', () => {
    it('records queries above the threshold', () => {
      ...
      const rows = db.query("SELECT * FROM performance_metrics WHERE metric = 'db_slow_query'").all() as {
        labels: string;
        value: number;
      }[];
    ...
    it('ignores queries below the threshold', () => {
      ...
      const count = (
        db.query("SELECT COUNT(*) as c FROM performance_metrics WHERE metric = 'db_slow_query'").get() as { c: number }
      ).c;
      expect(count).toBe(0);
    });
