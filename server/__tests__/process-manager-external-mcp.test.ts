...
const MANAGER_SOURCE = readFileSync(join(import.meta.dir, '..', 'process', 'manager.ts'), 'utf-8');
...
describe('resumeProcess external MCP runtime coverage', () => {
  ...
  beforeEach(() => {
    ...
    runMigrations(db);
    db.query(`INSERT INTO agents (id, name, model, system_prompt) VALUES (?, 'McpTestAgent', 'test', 'test')`).run(
      AGENT_ID,
    );
