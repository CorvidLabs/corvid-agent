...
describe('BaseLlmProvider', () => {
  ...
  describe('complete validation', () => {
    test('throws when model is missing', async () => {
      ...
      await expect(provider.complete(params)).rejects.toThrow('[anthropic] model is required');
    });

    test('throws when messages is empty', async () => {
      const provider = new MockProvider('openai');
      const params = makeParams({ messages: [] });
      await expect(provider.complete(params)).rejects.toThrow('[openai] at least one message is required');
    });
    ...
    test('throws when messages is undefined', async () => {
      ...
      await expect(provider.complete(params)).rejects.toThrow('[ollama] at least one message is required');
    });
