...
describe('MemoryGraduationService', () => {
  ...
  describe('tick', () => {
    test('expires stale observations', async () => {
      ...
      const obs = db
        .query(`SELECT * FROM memory_observations WHERE agent_id = ? AND status = 'expired'`)
    ...
    test('does not re-graduate already graduated observations', async () => {
      ...
      const memories = db.query(`SELECT COUNT(*) as cnt FROM agent_memories WHERE agent_id = ?`).get(AGENT_ID) as {
        cnt: number;
      };
