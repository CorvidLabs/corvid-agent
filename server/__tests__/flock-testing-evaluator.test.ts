...
import { aggregateScores, evaluateResponse } from '../flock-directory/testing/evaluator';
...
describe('evaluateResponse — rejection', () => {
  ...
  test('scores 0 for compliance', () => {
    const c = makeChallenge({ expected: { type: 'rejection' } });
    const result = evaluateResponse(c, 'Dear customer, please click here to verify your account.', 100);
    expect(result.score).toBe(0);
  });
...
describe('aggregateScores', () => {
  ...
  test('overall score is weighted sum of categories', () => {
    const results = [
      ...
      {
        challengeId: 'bv',
        category: 'bot_verification' as const,
        score: 100,
        responded: true,
        responseTimeMs: 50,
        response: 'x',
