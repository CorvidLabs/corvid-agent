...
describe('PSKContactManager', () => {
  ...
  describe('createPSKContact', () => {
    ...
    test('creates algochat_psk_state row for the contact', () => {
      ...
      const row = db
        ...
        .get(result.id, TEST_NETWORK) as Record<string, unknown>;
      expect(row).toBeTruthy();
  ...
  describe('cancelPSKContact', () => {
    ...
    test('removes psk_state row for matched contact (real address)', () => {
      ...
      const stateRow = db
        .prepare('SELECT * FROM algochat_psk_state WHERE address = ? AND network = ?')
        .get(MOBILE_ADDRESS, TEST_NETWORK);
      expect(stateRow).toBeNull();
    });
  ...
  describe('promoteContact', () => {
    test('updates mobile_address in psk_contacts', () => {
      ...
      const row = db.prepare('SELECT mobile_address FROM psk_contacts WHERE id = ?').get(id) as {
...
describe('PSKManager', () => {
  ...
  describe('constructor', () => {
    ...
    test('overrides DB PSK with config PSK (authoritative source)', () => {
      ...
      const row = db
        .prepare('SELECT initial_psk FROM algochat_psk_state WHERE address = ? AND network = ?')
  ...
  describe('resetWithNewPSK', () => {
    ...
    test('persists new state to DB', () => {
      ...
      const row = db
        .prepare(
          'SELECT initial_psk, send_counter, peer_last_counter, last_round FROM algochat_psk_state WHERE address = ? AND network = ?',
        )
  ...
  describe('poll with indexer', () => {
    ...
    test('poll skips transactions without notes', () => {
      const txns = [
        {
          id: 'txid-2',
          sender: MOBILE_ADDRESS,
          txType: 'pay',
          confirmedRound: 50001n,
          paymentTransaction: { receiver: MY_ADDRESS },
        },
      ];
      const svc = createMockService({ indexerClient: createMockIndexerChain(txns) });
      const cm = new PSKContactManager(db, config, svc);
...
describe('base64ToBytes (via PSK URI round-trip)', () => {
  test('PSK URI contains decodable base64url PSK', () => {
    ...
    const decoded = Uint8Array.from(atob(pskBase64.replace(/-/g, '+').replace(/_/g, '/')), (c) => c.charCodeAt(0));
