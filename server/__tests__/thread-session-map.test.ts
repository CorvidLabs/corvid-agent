...
import { formatDuration, normalizeTimestamp, tryRecoverThread } from '../discord/thread-session-map';
...
describe('tryRecoverThread', () => {
  ...
  test('recovers session from DB and populates map', () => {
    ...
    db.run(
      `INSERT INTO sessions (id, agent_id, name, source) VALUES ('sess-1', 'agent-1', 'Discord thread:999000999000999000', 'discord')`,
    );
  ...
  test('returns null for non-discord session with matching thread name pattern', () => {
    ...
    db.run(
      `INSERT INTO sessions (id, agent_id, name, source) VALUES ('sess-2', 'agent-2', 'Discord thread:111222333444555666', 'telegram')`,
    );
  ...
  test('uses most-recent session when multiple exist for same thread', () => {
    ...
    db.run(
      `INSERT INTO sessions (id, agent_id, name, source, created_at) VALUES ('sess-old', 'agent-3', 'Discord thread:777888999000111222', 'discord', '2026-01-01 00:00:00')`,
    );
    db.run(
      `INSERT INTO sessions (id, agent_id, name, source, created_at) VALUES ('sess-new', 'agent-3', 'Discord thread:777888999000111222', 'discord', '2026-02-01 00:00:00')`,
    );
  ...
  test('falls back to default agent name when agent row is missing', () => {
    db.run(
      `INSERT INTO sessions (id, agent_id, name, source) VALUES ('sess-3', NULL, 'Discord thread:100200300400500600', 'discord')`,
    );
  ...
  test('recovers optional project name when present', () => {
    ...
    db.run(
      `INSERT INTO sessions (id, agent_id, project_id, name, source) VALUES ('sess-4', 'agent-4', 'proj-1', 'Discord thread:200300400500600700', 'discord')`,
    );
