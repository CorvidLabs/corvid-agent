...
describe('tokenizeBashCommand', () => {
  ...
  test('handles single-quoted paths', () => {
    expect(tokenizeBashCommand("cat '/path with spaces/file.ts'")).toEqual(['cat', '/path with spaces/file.ts']);
  });

  test('handles double-quoted paths', () => {
    expect(tokenizeBashCommand('rm "/path with spaces/file.ts"')).toEqual(['rm', '/path with spaces/file.ts']);
  });
  ...
  test('splits shell operators as separate tokens', () => {
    expect(tokenizeBashCommand('cat foo | grep bar')).toEqual(['cat', 'foo', '|', 'grep', 'bar']);
  });
  ...
  test('handles redirection operators', () => {
    expect(tokenizeBashCommand('echo hello > output.txt')).toEqual(['echo', 'hello', '>', 'output.txt']);
  });

  test('handles >> append', () => {
    expect(tokenizeBashCommand('echo data >> log.txt')).toEqual(['echo', 'data', '>>', 'log.txt']);
  });
  ...
  test('handles multiple operators in sequence', () => {
    expect(tokenizeBashCommand('a && b || c ; d')).toEqual(['a', '&&', 'b', '||', 'c', ';', 'd']);
  });
...
describe('detectDangerousPatterns', () => {
  ...
  // biome-ignore lint/suspicious/noTemplateCurlyInString: intentional test of template literal detection
  test('detects ${} variable expansion', () => {
    // biome-ignore lint/suspicious/noTemplateCurlyInString: intentional test of template literal detection
    const result = detectDangerousPatterns('rm ${TARGET_FILE}');
    expect(result.isDangerous).toBe(true);
    // biome-ignore lint/suspicious/noTemplateCurlyInString: intentional test string containing ${}
    expect(result.reason).toContain('${}');
  });
