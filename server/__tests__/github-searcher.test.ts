...
describe('resolveFullRepo', () => {
  test('returns configRepo when it contains a slash', () => {
    expect(resolveFullRepo('CorvidLabs/corvid-agent', 'https://github.com/CorvidLabs/other/issues/1')).toBe(
      'CorvidLabs/corvid-agent',
    );
  });

  test('extracts owner/repo from HTML URL when config is org-only', () => {
    expect(resolveFullRepo('CorvidLabs', 'https://github.com/CorvidLabs/site/pull/22')).toBe('CorvidLabs/site');
  });

  test('handles deep URL paths', () => {
    expect(
      resolveFullRepo('CorvidLabs', 'https://github.com/CorvidLabs/corvid-agent/issues/400#issuecomment-123'),
...
describe('escapeRegex', () => {
  ...
  test('escapes all special regex characters', () => {
    // biome-ignore lint/suspicious/noTemplateCurlyInString: intentional test of regex special character escaping
    const specials = '.*+?^${}()|[]\\';
...
describe('GitHubSearcher', () => {
  ...
  describe('fetchRecentComments', () => {
    ...
    test('sets isPullRequest flag correctly', async () => {
      const commentsResponse = ghOk(
        JSON.stringify([{ id: 3, body: '@user hi', user: { login: 'alice' }, created_at: '2026-03-01T14:00:00Z' }]),
      );
      const searcher = createSearcher(new Map([['repos/', commentsResponse]]));

      const result = await searcher.fetchRecentComments('CorvidLabs/repo', 5, 'user', '2026-03-01T00:00:00Z', true, {
        title: 'PR title',
      });
  ...
  describe('fetchPRReviews', () => {
    ...
    test('finds non-self reviews after since date', async () => {
      ...
      const result = await searcher.fetchPRReviews(
        'CorvidLabs/repo',
        5,
        'author',
        '2026-03-01T00:00:00Z',
  ...
  describe('fetchPRReviewComments', () => {
    test('returns empty on gh failure', async () => {
      const searcher = createSearcher(new Map([['repos/', ghFail()]]));
      const result = await searcher.fetchPRReviewComments(
        'CorvidLabs/repo',
        1,
        'user',
        '2026-03-01T00:00:00Z',
    ...
    test('finds inline review comments from other users', async () => {
      const response = ghOk(
        JSON.stringify([
          {
            id: 101,
            body: 'Suggestion: use const here',
            user: { login: 'reviewer' },
            html_url: 'url-101',
    ...
    test('case-insensitive self-comment detection', async () => {
      const response = ghOk(
        JSON.stringify([{ id: 103, body: 'test', user: { login: 'AUTHOR' }, created_at: '2026-03-01T17:00:00Z' }]),
      );
      const searcher = createSearcher(new Map([['repos/', response]]));
  ...
  describe('searchGlobalAuthoredPRReviews', () => {
    ...
    test('caches results per username+date to avoid redundant API calls', async () => {
      ...
      const globalCalls = capturedArgs.filter((args) => {
        ...
        return qArg?.includes('author:') && !qArg.includes('repo:') && !qArg.includes('org:');
      });
        ...
        JSON.stringify({
          items: [
            ...
            },
            {
              ...
              body: '',
              user: { login: 'c' },
              html_url: 'https://github.com/CorvidLabs/repo/issues/3',
              title: 'T3',
              created_at: '2026-03-01T12:00:00Z',
            },
          ],
