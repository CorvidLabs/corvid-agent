...
describe('priority-rules', () => {
  ...
  describe('evaluateAction', () => {
    it('returns run when healthy', () => {
      expect(evaluateAction('work_task', ['healthy']).decision).toBe('run');
    });
    it('skips feature work when CI broken', () => {
      expect(evaluateAction('work_task', ['ci_broken']).decision).toBe('skip');
    });
    it('boosts maintenance when CI broken', () => {
      expect(evaluateAction('codebase_review', ['ci_broken']).decision).toBe('boost');
    });
    it('boosts review when CI broken', () => {
      expect(evaluateAction('review_prs', ['ci_broken']).decision).toBe('boost');
    });
    it('allows lightweight when CI broken', () => {
      expect(evaluateAction('star_repo', ['ci_broken']).decision).toBe('run');
    });
    ...
    it('boosts lightweight when server degraded', () => {
      expect(evaluateAction('star_repo', ['server_degraded']).decision).toBe('boost');
    });
    it('skips feature work when P0 open', () => {
      expect(evaluateAction('work_task', ['p0_open']).decision).toBe('skip');
    });
    ...
    it('skips feature work under disk pressure', () => {
      expect(evaluateAction('work_task', ['disk_pressure']).decision).toBe('skip');
    });
    it('boosts maintenance under disk pressure', () => {
      expect(evaluateAction('memory_maintenance', ['disk_pressure']).decision).toBe('boost');
    });
    it('skip wins over boost in conflict', () => {
      expect(evaluateAction('codebase_review', ['ci_broken', 'server_degraded']).decision).toBe('skip');
    });
    it('handles empty states', () => {
      expect(evaluateAction('work_task', []).decision).toBe('run');
    });
  });
  ...
  describe('getAllRules', () => {
    ...
    it('healthy has no skips', () => {
      expect(getRulesForState('healthy').skip).toHaveLength(0);
    });
  });
});
...
describe('all action types covered', () => {
  const types: ScheduleActionType[] = [
    'star_repo',
    'fork_repo',
    'review_prs',
    'work_task',
