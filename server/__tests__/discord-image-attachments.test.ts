...
import {
  ...
  extractImageBlocks,
  isImageAttachment,
} from '../discord/image-attachments';
...
describe('extractImageBlocks', () => {
  ...
  test('enforces max 5 images per message', async () => {
    const attachments = Array.from({ length: 7 }, (_, i) => makeAttachment({ id: String(i), filename: `img${i}.png` }));
    const result = await extractImageBlocks(attachments);
...
describe('appendAttachmentUrls', () => {
  ...
  test('falls back to url when proxy_url is empty', () => {
    const result = appendAttachmentUrls('hi', [
      makeAttachment({ proxy_url: '', url: 'https://cdn.discordapp.com/direct.png' }),
    ]);
