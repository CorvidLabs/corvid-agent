...
describe('compose', () => {
  it('executes middleware in order (downstream) and reverse (upstream)', async () => {
    ...
    expect(log).toEqual(['a:down', 'b:down', 'c:down', 'c:up', 'b:up', 'a:up']);
  });
  ...
  it('preserves registration order for equal order values', async () => {
    ...
    expect(log).toEqual(['first:down', 'second:down', 'third:down', 'third:up', 'second:up', 'first:up']);
  });
...
describe('abort semantics', () => {
  ...
  it('stops downstream execution when ctx.aborted is set', async () => {
    ...
    expect(log).toEqual(['first:down', 'aborter:abort', 'first:up']);
  });
...
describe('MiddlewarePipeline', () => {
  it('registers and executes middleware', async () => {
    ...
    pipeline.use(trackingMiddleware('second', 20, log)).use(trackingMiddleware('first', 10, log));
  ...
  it('does not override response if middleware already set one during error', async () => {
    ...
    pipeline.use(
      makeMiddleware('handler', 10, async (ctx, next) => {
        try {
          await next();
        } catch {
...
describe('errorHandlerMiddleware', () => {
  it('catches downstream errors and sets 500 response', async () => {
    ...
    pipeline.use(
      makeMiddleware('thrower', 100, async () => {
        throw new Error('test error');
      }),
    );
  ...
  it('does not override existing response on error', async () => {
    ...
    pipeline.use(errorHandlerMiddleware());
    pipeline.use(
      makeMiddleware('handler', 50, async (ctx, next) => {
        try {
          await next();
        } catch {
  ...
  it('passes through when no error occurs', async () => {
    ...
    pipeline.use(errorHandlerMiddleware());
    pipeline.use(
      makeMiddleware('ok', 100, async (ctx) => {
...
describe('requestLogMiddleware', () => {
  it('allows requests to pass through', async () => {
    ...
    pipeline.use(
      makeMiddleware('handler', 100, async (ctx) => {
        ctx.response = new Response('OK', { status: 200 });
      }),
    );
  ...
  it('runs upstream phase after downstream completes', async () => {
    ...
    pipeline.use(
      makeMiddleware('log', ORDER.REQUEST_LOG, async (_ctx, next) => {
        log.push('log:before');
        await next();
        log.push('log:after');
      }),
...
describe('rateLimitMiddleware', () => {
  ...
  it('allows requests within rate limit', async () => {
    ...
    pipeline.use(
      makeMiddleware('handler', 200, async (ctx) => {
        ctx.response = new Response('OK', { status: 200 });
      }),
    );
...
describe('full pipeline integration', () => {
  ...
  it('runs full pipeline: cors → log → error → rateLimit → auth → role → handler', async () => {
    ...
    // Route handler
    pipeline.use(
      makeMiddleware('handler', 300, async (ctx) => {
        log.push('handler');
  ...
  it('error handler catches route handler errors', async () => {
    ...
    pipeline.use(
      makeMiddleware('handler', 300, async () => {
  ...
  it('ordering is deterministic regardless of registration order', async () => {
    ...
    pipeline.use(
      makeMiddleware('auth', 110, async (ctx, next) => {
        log.push('auth');
        ctx.requestContext.authenticated = true;
        await next();
      }),
    ...
    pipeline.use(
      makeMiddleware('rate-limit', 100, async (_ctx, next) => {
        log.push('rate-limit');
        await next();
      }),
    );
    pipeline.use(
      makeMiddleware('cors', 10, async (_ctx, next) => {
        log.push('cors');
        await next();
      }),
    );
