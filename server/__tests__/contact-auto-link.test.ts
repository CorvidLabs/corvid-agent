...
describe('Discord contact auto-link', () => {
  ...
  test('returns cached contact on subsequent messages', () => {
    ...
    const rows = db.query("SELECT COUNT(*) as cnt FROM contacts WHERE display_name = 'testuser'").get() as {
      cnt: number;
    };
  ...
  test('handles different Discord users independently', () => {
    ...
    const rows = db.query('SELECT COUNT(*) as cnt FROM contacts').get() as { cnt: number };
  ...
  test('does not create duplicate contacts for same Discord user', () => {
    ...
    const linkRows = db
      .query("SELECT COUNT(*) as cnt FROM contact_platform_links WHERE platform = 'discord' AND platform_id = '123456'")
      .get() as { cnt: number };
