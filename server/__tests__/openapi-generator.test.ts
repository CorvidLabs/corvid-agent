...
describe('Route Registry', () => {
  ...
  test('POST routes with schemas have requestSchema defined', () => {
    ...
    const createRoutes = routes.filter(
      (r) => r.method === 'POST' && ['/api/projects', '/api/agents', '/api/sessions', '/api/councils'].includes(r.path),
    );
...
describe('OpenAPI Generator', () => {
  ...
  test('path parameters have correct format', () => {
    ...
    const agentGet = (spec.paths['/api/agents/{id}'] as Record<string, unknown>)?.get as
