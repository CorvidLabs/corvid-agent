...
describe('Migration 094 — asa_id column', () => {
  ...
  test('idx_agent_memories_asa index exists', () => {
    const indexes = db
      .query("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_agent_memories_asa'")
      .all();
  ...
  test('asa_id can be set and queried', () => {
    ...
    const row = db.query('SELECT * FROM agent_memories WHERE agent_id = ? AND asa_id = 42').get(agentId) as {
      key: string;
      asa_id: number;
    };
