...
import { getSpendingLimits, recordAlgoSpend } from '../db/spending';
  ...
  return {
    ...
    algodClient: {
      getTransactionParams: mock(() => ({
        do: () =>
          Promise.resolve({ flatFee: true, fee: 1000, firstRound: 1, lastRound: 1000, genesisHash: '', genesisID: '' }),
      })),
...
describe('OnChainTransactor with null service', () => {
  ...
  test('sendToAddress returns null when service is null', async () => {
    ...
    } as unknown as import('@corvidlabs/ts-algochat').ChatAccount;
