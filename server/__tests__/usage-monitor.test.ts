...
describe('UsageMonitor', () => {
    ...
    describe('checkCostSpike', () => {
        it('sends alert when session cost exceeds 2x rolling average', async () => {
            ...
            const callArgs = (mockNotify.mock.calls[0] as unknown as [{ title: string; message: string; level: string; agentId: string }])[0];
    ...
    describe('sendAlert', () => {
        it('calls notify with correct parameters', async () => {
            ...
            expect(mockNotify).toHaveBeenCalledTimes(1);
            const args = (mockNotify.mock.calls[0] as unknown as [{ agentId: string; title: string; message: string; level: string }])[0];
