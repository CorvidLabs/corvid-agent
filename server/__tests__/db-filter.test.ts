...
describe('validateTenantOwnership', () => {
  ...
  test('rejects table not in TENANT_SCOPED_TABLES', () => {
    expect(() => validateTenantOwnership(db, 'evil_table', 'p1', 'tenant-a')).toThrow(/not in TENANT_SCOPED_TABLES/);
  });

  test('rejects invalid idColumn to prevent SQL injection', () => {
    expect(() => validateTenantOwnership(db, 'projects', 'p1', 'tenant-a', 'id; DROP TABLE projects')).toThrow(
      /invalid idColumn/,
    );
  });
  ...
  test('throws for DEFAULT_TENANT_ID when guard is enabled', () => {
    enableMultiTenantGuard();
    expect(() => validateTenantOwnership(db, 'projects', 'p1', DEFAULT_TENANT_ID)).toThrow(
      /DEFAULT_TENANT_ID is not allowed/,
    );
  });
});
