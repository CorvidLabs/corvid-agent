...
beforeEach(() => {
    ...
    db.query(`INSERT INTO agents (id, name, model, system_prompt) VALUES (?, 'WebhookAgent', 'test', 'test')`).run(AGENT_ID);
...
describe('processEvent — session creation', () => {
    it('creates session with descriptive name for issue_comment', async () => {
        ...
        const session = db.query("SELECT name FROM sessions WHERE name LIKE 'Webhook:%' LIMIT 1").get() as { name: string } | null;
...
describe('processEvent — event-based schedules', () => {
    it('fires matching event-based schedules', async () => {
        ...
        `).run(scheduleId, AGENT_ID, JSON.stringify([{ source: 'github_webhook', event: 'issue_comment', repo: 'owner/repo' }]));
