...
import { enqueueRequest, expireOldRequests, getPendingRequests, resolveRequest } from '../db/escalation-queue';
...
describe('expireOldRequests', () => {
  ...
  test('expires requests older than the specified age', () => {
    ...
    db.exec(`UPDATE escalation_queue SET created_at = datetime('now', '-48 hours') WHERE session_id = 's1'`);
  ...
  test('does not expire already resolved requests', () => {
    ...
    // Backdate it
    db.exec(`UPDATE escalation_queue SET created_at = datetime('now', '-48 hours') WHERE id = ${req.id}`);
