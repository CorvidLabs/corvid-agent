...
describe('shouldSkipByHealthGate', () => {
  ...
  test('returns false when system is healthy', () => {
    insertExecution('exec-1');
    const systemState: SystemStateResult = {
      states: ['healthy'],
      details: {},
...
describe('handleApprovalIfNeeded', () => {
  ...
  test('returns false for auto-approval schedules', () => {
    ...
    const result = handleApprovalIfNeeded(
      db,
      makeSchedule({ approvalPolicy: 'auto' }),
      makeExecution(),
      makeAction({ type: 'work_task' }),
      null,
      noopEmit,
  ...
  test('sets execution to awaiting_approval for owner_approve + destructive action', () => {
    ...
    const result = handleApprovalIfNeeded(
      db,
      makeSchedule({ approvalPolicy: 'owner_approve' }),
      makeExecution(),
      makeAction({ type: 'work_task' }),
      null,
      (e) => emitted.push(e as { type: string; data: unknown }),
    );
    expect(result).toBe(true);
  ...
  test('sends notification when notificationService is provided', () => {
    ...
    handleApprovalIfNeeded(
      db,
      makeSchedule({ approvalPolicy: 'council_approve' }),
      makeExecution(),
...
describe('handleRepoLocking', () => {
  ...
  test('returns false when action has no repos', () => {
    const result = handleRepoLocking(db, makeSchedule(), makeExecution(), makeAction({ repos: [] }), noopEmit);
    expect(result).toBe(false);
  });
  ...
  test('releases partially acquired locks on failure', () => {
    ...
    handleRepoLocking(
      ...
      makeSchedule(),
      makeExecution({ id: 'exec-2' }),
      makeAction({ repos: ['org/repo-a', 'org/repo-b'] }),
      noopEmit,
    );
  ...
  test('skips repo locking for work_task actions (uses service-level queuing)', () => {
    ...
    handleRepoLocking(
      ...
      makeSchedule(),
      makeExecution({ id: 'exec-1' }),
      makeAction({ type: 'review_prs', repos: [], projectId: 'proj-1' }),
      noopEmit,
    );
    ...
    const result = handleRepoLocking(
      ...
      makeSchedule(),
      makeExecution({ id: 'exec-2' }),
      makeAction({ type: 'work_task', repos: [], projectId: 'proj-1' }),
      noopEmit,
    );
    expect(result).toBe(false);
  ...
  test('uses projectId fallback when repos is empty', () => {
    ...
    const result = handleRepoLocking(
      ...
      makeSchedule(),
      makeExecution(),
      makeAction({ repos: [], projectId: 'proj-1' }),
      noopEmit,
    );
    expect(result).toBe(false);
