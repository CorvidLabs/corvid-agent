...
function createMockIndexerClient(transactions: Array<{ sender: string; note?: string }> = []) {
  return {
    searchForTransactions: mock(() => ({
      address: mock(function (this: unknown) {
        return this;
      }),
      addressRole: mock(function (this: unknown) {
        return this;
      }),
...
describe('DiscoveryService', () => {
  describe('seedConversations', () => {
    test('should seed SyncManager with conversations from DB', () => {
      ...
      const calledAddrs = syncManager.getOrCreateConversation.mock.calls.map((c: unknown[]) => c[0]);
  ...
  describe('discoverNewSenders', () => {
    ...
    test('should skip already-known participants', async () => {
      ...
      const newCalls = syncManager.getOrCreateConversation.mock.calls.filter((c: unknown[]) => c[0] === SENDER_B);
