...
async function callRoute(...args: Parameters<typeof handleFlockDirectoryRoutes>): Promise<Response | null> {
...
describe('Flock Directory Routes', () => {
  ...
  it('GET /api/flock-directory/search passes query params', async () => {
    ...
    const { req, url } = fakeReq(
      'GET',
      '/api/flock-directory/search?q=test&status=active&capability=code&minReputation=5&limit=10&offset=2',
    );
    const res = await callRoute(req, url, db, svc);
    expect(res).not.toBeNull();
    expect(res!.status).toBe(200);
