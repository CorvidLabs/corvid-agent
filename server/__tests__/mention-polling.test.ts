...
): Promise<Record<string, unknown> & { configs?: Array<Record<string, unknown>> }> {
...
describe('Mention Polling DB Operations', () => {
  ...
  describe('listMentionPollingConfigs', () => {
    ...
    test('returns configs in descending creation order', () => {
      ...
      db.query(`UPDATE mention_polling_configs SET created_at = datetime('now', '-60 seconds') WHERE id = ?`).run(
        c1.id,
      );
