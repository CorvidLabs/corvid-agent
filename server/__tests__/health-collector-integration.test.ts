...
beforeAll(() => {
    ...
    writeFileSync(join(TMP_DIR, 'server/example.ts'), [
        '// TODO: fix this later',
        '// FIXME: also broken',
        'export const x = 1;',
    ].join('\n'));
    ...
    writeFileSync(join(TMP_DIR, 'node_modules/@fake/index.ts'), '// TODO: this should not be counted\nexport const y = 2;\n');
    ...
    writeFileSync(join(TMP_DIR, 'server/node_modules/fake-pkg/index.ts'), Array.from({ length: 800 }, (_, i) => `// line ${i}`).join('\n'));
