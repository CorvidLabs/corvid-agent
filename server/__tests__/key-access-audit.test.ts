...
import { assertProductionReady, EnvKeyProvider, type KeyProvider } from '../lib/key-provider';
...
describe('assertProductionReady', () => {
  ...
  it('throws when no KeyProvider on testnet', async () => {
    ...
    await expect(assertProductionReady(null, 'testnet')).rejects.toThrow('KeyProvider is required');
  });

  it('throws when no KeyProvider on mainnet', async () => {
    process.env.WALLET_ENCRYPTION_KEY = TEST_PASSPHRASE;
    await expect(assertProductionReady(null, 'mainnet')).rejects.toThrow('KeyProvider is required');
  });
  ...
  it('throws when WALLET_ENCRYPTION_KEY not set on testnet', async () => {
    ...
    await expect(assertProductionReady(provider, 'testnet')).rejects.toThrow(
      'WALLET_ENCRYPTION_KEY must be explicitly set',
    );
  });
  ...
  it('throws when WALLET_ENCRYPTION_KEY too short on mainnet', async () => {
    ...
    await expect(assertProductionReady(provider, 'mainnet')).rejects.toThrow('too short');
  });
  ...
  it('rejects provider that returns short passphrase', async () => {
    ...
    await expect(assertProductionReady(badProvider, 'mainnet')).rejects.toThrow('shorter than 32');
  });
  ...
  it('rejects provider that throws', async () => {
    ...
    await expect(assertProductionReady(failingProvider, 'mainnet')).rejects.toThrow('vault unavailable');
  });
});
...
describe('Key access audit actions', () => {
  ...
  it('key rotation creates key_access entries for each agent', async () => {
    ...
    const mnemonic =
      'abandon ability able about above absent absorb abstract absurd abuse access accident account accuse achieve acid acoustic acquire across act action actor actress actual about';
    ...
    db.prepare('UPDATE agents SET wallet_mnemonic_encrypted = ?, wallet_address = ? WHERE id = ?').run(
      enc,
      'ADDR1',
      'a1',
    );
...
describe('Encrypted in-memory cache pattern', () => {
  ...
  it('ephemeral key cannot decrypt data encrypted with different ephemeral key', async () => {
    ...
    await expect(decryptMnemonicWithPassphrase(enc, key2)).rejects.toThrow();
  });
