...
function createMockRunAction(db: Database) {
    ...
    const fn = mock(async (_deps: RunActionDeps, _hctx: HandlerContext, executionId: string) => {
        ...
        db.query(`UPDATE schedule_executions SET status = ?, result = ? WHERE id = ?`)
            .run(outcome.status, outcome.result, executionId);
...
describe('executePipeline', () => {
    it('executes all steps sequentially and returns context with results', async () => {
        ...
        const ctx = await executePipeline(makeDeps(db, emitFn), hctx, schedule, steps, emitFn, mockRun as unknown as RunActionFn);
    ...
    it('sets hasFailure when a step fails', async () => {
        ...
        const steps: PipelineStep[] = [
            ...
            { label: 'report', action: { type: 'discord_post', channelId: '123', message: 'audit done' }, condition: 'always' },
        ];
    ...
    it('runs on_failure steps only when prior step fails', async () => {
        ...
        const steps: PipelineStep[] = [
            ...
            { label: 'alert', action: { type: 'discord_post', channelId: '1', message: 'Build broke!' }, condition: 'on_failure' },
        ];
    ...
    it('interpolates {{pipeline.steps.<label>.result}} in message', async () => {
        ...
        const steps: PipelineStep[] = [
            ...
            { label: 'post', action: { type: 'discord_post', channelId: '1', message: 'Summary: {{pipeline.steps.review.result}}' } },
        ];
    ...
    it('emits events for each step execution', async () => {
        ...
        const steps: PipelineStep[] = [
            { label: 'only', action: { type: 'daily_review' } },
        ];
    ...
    it('handles empty pipeline (no steps)', async () => {
        ...
        const ctx = await executePipeline(makeDeps(db, emitFn), hctx, schedule, [], emitFn, mockRun as unknown as RunActionFn);
    ...
    it('tracks durationMs for each step', async () => {
        ...
        const steps: PipelineStep[] = [
            { label: 'slow', action: { type: 'daily_review' } },
        ];
    ...
    it('three-step pipeline with mixed conditions', async () => {
        ...
        const steps: PipelineStep[] = [
            ...
            { label: 'report', action: { type: 'discord_post', channelId: '1', message: '{{pipeline.summary}}' }, condition: 'always' },
        ];
    ...
    it('records audit entries for each executed step', async () => {
        ...
        const steps: PipelineStep[] = [
            { label: 'audit-step', action: { type: 'dependency_audit' } },
        ];
