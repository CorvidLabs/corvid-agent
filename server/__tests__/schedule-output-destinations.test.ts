...
import type { AgentSchedule, ScheduleExecution, ScheduleOutputDestination } from '../../shared/types';
import { deliverToOutputDestinations, formatResult, type RunActionDeps, shouldDeliver } from '../scheduler/execution';
...
describe('deliverToOutputDestinations', () => {
  ...
  test('skips on_error_only destinations when completed', () => {
    const deps = makeDeps();
    const schedule = makeSchedule([{ type: 'discord_channel', target: '123', format: 'on_error_only' }]);
    const execution = makeExecution('completed', 'done');
  ...
  test('discord delivery skipped without DISCORD_BOT_TOKEN', () => {
    const deps = makeDeps();
    const schedule = makeSchedule([{ type: 'discord_channel', target: '123456789' }]);
  ...
  test('algochat_agent skipped when agentMessenger is null', () => {
    ...
    const schedule = makeSchedule([{ type: 'algochat_agent', target: 'agent-xyz' }]);
  ...
  test('algochat_address skipped when agentMessenger is null', () => {
    const deps = makeDeps({ agentMessenger: null });
    const schedule = makeSchedule([{ type: 'algochat_address', target: 'ALGO_ADDR' }]);
  ...
  test('algochat_agent calls sendOnChainToSelf when messenger available', async () => {
    ...
    const schedule = makeSchedule([{ type: 'algochat_agent', target: 'agent-target' }]);
  ...
  test('algochat_address calls sendNotificationToAddress when messenger available', async () => {
    ...
    const deps = makeDeps({ agentMessenger: messenger });
    const schedule = makeSchedule([{ type: 'algochat_address', target: 'ALGO_ADDR_456' }]);
  ...
  test('handles null result gracefully', () => {
    const deps = makeDeps();
    const schedule = makeSchedule([{ type: 'discord_channel', target: '123' }]);
  ...
  test('delivery errors are caught and do not throw', async () => {
    ...
    const messenger = { sendOnChainToSelf } as unknown as RunActionDeps['agentMessenger'];
    const deps = makeDeps({ agentMessenger: messenger });
    const schedule = makeSchedule([{ type: 'algochat_agent', target: 'agent-fail' }]);
    const execution = makeExecution('completed', 'result');
