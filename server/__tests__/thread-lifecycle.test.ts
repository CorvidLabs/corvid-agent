...
// Mock only buildActionRow and assertSnowflake; preserve all other embeds exports.
...
import { archiveStaleThreads, archiveThread, createStandaloneThread } from '../discord/thread-lifecycle';
import type { ThreadCallbackInfo, ThreadSessionInfo } from '../discord/thread-session-map';
...
describe('createStandaloneThread', () => {
  ...
  test('sets type to GUILD_PUBLIC_THREAD (11) and auto_archive_duration to 1440', async () => {
    const createThread = mock(async (_channelId: string, _data: { type: number; auto_archive_duration: number }) => ({
    ...
    const callArgs = (createThread as ReturnType<typeof mock>).mock.calls[0] as [
      string,
      { type: number; auto_archive_duration: number },
    ];
...
describe('archiveStaleThreads', () => {
  ...
  test('handles empty maps without error', async () => {
    const pm = makeProcessManager();
    await expect(archiveStaleThreads(pm, new Map(), new Map(), new Map(), 5_000)).resolves.toBeUndefined();
  });
});
