...
function mockFetchFail(status = 403, body = 'Forbidden') {
  globalThis.fetch = mock(() => Promise.resolve(new Response(body, { status }))) as unknown as typeof globalThis.fetch;
}
...
describe('archiveStaleThreads', () => {
  ...
  test('handles empty maps without error', async () => {
    ...
    const delivery = makeDelivery();
    await expect(
      archiveStaleThreads(pm, delivery, BOT_TOKEN, new Map(), new Map(), new Map(), 5_000),
    ).resolves.toBeUndefined();
  });
});
