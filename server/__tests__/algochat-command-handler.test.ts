...
function createMockProcessManager(overrides: Partial<Record<string, unknown>> = {}): ProcessManager {
  return {
    ...
    approvalManager: {
      ...
      resolveQueuedRequest: mock((_id: number, _approved: boolean) => overrides.resolveResult ?? false),
...
function createMockResponseFormatter(): ResponseFormatter & {
  lastResponse: { participant: string; content: string } | null;
} {
  const formatter = {
...
function createMockContext(overrides: MockContextOptions = {}): CommandHandlerContext {
  return {
    ...
    extendSession: mock((_sessionId: string, _minutes: number) => overrides.extendResult ?? true) as (
      sessionId: string,
      minutes: number,
    ) => boolean,
  };
...
describe('dependency injection', () => {
  test('setWorkCommandRouter enables /work', () => {
    ...
    // After injection
    const mockRouter = {
      handleSlashCommand: mock((_p: string, _d: string, respond: (t: string) => void) => respond('Task created')),
    } as unknown as WorkCommandRouter;
    handler.setWorkCommandRouter(mockRouter);
    handler.handleCommand(OWNER_ADDR, '/work test', responseFn);
