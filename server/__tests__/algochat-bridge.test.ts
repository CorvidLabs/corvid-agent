...
describe('psk_contacts DB operations', () => {
  ...
  test('insert and query a PSK contact', () => {
    ...
    const rows = db
      .prepare('SELECT id, nickname, network, active FROM psk_contacts WHERE network = ?')
  ...
  test('stores and retrieves initial_psk as BLOB', () => {
    ...
    const row = db.prepare('SELECT initial_psk FROM psk_contacts WHERE id = ?').get('c-blob') as {
  ...
  test('query by network filters correctly', () => {
    ...
    const testnetRows = db
      .prepare('SELECT id FROM psk_contacts WHERE network = ? ORDER BY created_at ASC')
      .all('testnet') as Array<{ id: string }>;

    expect(testnetRows.length).toBe(2);
    expect(testnetRows.map((r) => r.id)).toEqual(['c1', 'c3']);

    const mainnetRows = db.prepare('SELECT id FROM psk_contacts WHERE network = ?').all('mainnet') as Array<{
      id: string;
    }>;
  ...
  test('update nickname', () => {
    ...
    const result = db
      .prepare("UPDATE psk_contacts SET nickname = ?, updated_at = datetime('now') WHERE id = ?")
  ...
  test('update mobile_address on discovery', () => {
    ...
    db.prepare("UPDATE psk_contacts SET mobile_address = ?, updated_at = datetime('now') WHERE id = ?").run(
      'ALGO_ADDR_ABC123',
      'c1',
    );

    const row = db.prepare('SELECT mobile_address FROM psk_contacts WHERE id = ?').get('c1') as {
  ...
  test('query only active contacts with no mobile address (unmatched)', () => {
    ...
    const count = queryCount(
      db,
      'SELECT COUNT(*) as cnt FROM psk_contacts WHERE network = ? AND active = 1 AND mobile_address IS NULL',
      NETWORK,
    );
  ...
  test('list contacts ordered by created_at ASC', () => {
    ...
    const rows = db
      .prepare('SELECT id, nickname FROM psk_contacts WHERE network = ? ORDER BY created_at ASC')
...
describe('algochat_psk_state DB operations', () => {
  ...
  test('insert and query PSK state', () => {
    ...
    const row = db
      .prepare('SELECT address, network, label, send_counter FROM algochat_psk_state WHERE address = ? AND network = ?')
  ...
  test('migrate PSK state address (discovery flow)', () => {
    ...
    db.prepare('DELETE FROM algochat_psk_state WHERE address = ? AND network = ?').run('REAL_MOBILE_ADDR', NETWORK);

    db.prepare('UPDATE algochat_psk_state SET address = ? WHERE address = ? AND network = ?').run(
      'REAL_MOBILE_ADDR',
      'contact-uuid',
      NETWORK,
    );
    ...
    const oldRow = db
      .prepare('SELECT * FROM algochat_psk_state WHERE address = ? AND network = ?')
      .get('contact-uuid', NETWORK);
    expect(oldRow).toBeNull();

    // New address should have the migrated state
    const newRow = db
      .prepare('SELECT send_counter, label FROM algochat_psk_state WHERE address = ? AND network = ?')
...
describe('reassembleGroupMessage', () => {
  test('reassembles chunks in order', () => {
    const chunks = ['[GRP:1/3]Hello ', '[GRP:2/3]beautiful ', '[GRP:3/3]world!'];
    expect(reassembleGroupMessage(chunks)).toBe('Hello beautiful world!');
  });

  test('reassembles chunks regardless of input order', () => {
    const chunks = ['[GRP:3/3]world!', '[GRP:1/3]Hello ', '[GRP:2/3]beautiful '];
    expect(reassembleGroupMessage(chunks)).toBe('Hello beautiful world!');
  });
  ...
  test('returns null when indices are not sequential', () => {
    const chunks = ['[GRP:1/3]Hello ', '[GRP:1/3]duplicate ', '[GRP:3/3]world!'];
