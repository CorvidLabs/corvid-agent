...
import { handleConfigureNotifications, handleNotifyOwner, type McpToolContext } from '../mcp/tool-handlers';
...
describe('NotificationService', () => {
  ...
  test('notify skips disabled channels', async () => {
    ...
    const deliveries = db
      .query('SELECT * FROM notification_deliveries WHERE notification_id = ?')
  ...
  test('missing config produces graceful failure, not throw', async () => {
    ...
    const deliveries = db
      .query(`SELECT * FROM notification_deliveries WHERE notification_id = ? AND status = 'failed'`)
      .all(result.notificationId) as Record<string, unknown>[];
    expect(deliveries.length).toBe(1);
  });
});
