...
function createMockProcessManager(overrides: Partial<Record<string, unknown>> = {}): ProcessManager {
  return {
    ...
    isRunning: mock((id: string) =>
      overrides.runningSessions ? (overrides.runningSessions as string[]).includes(id) : false,
    ),
    startProcess: mock(() => {}),
    stopProcess: mock(() => {}),
    sendMessage: mock((_sessionId: string, _content: string) => overrides.sendMessageResult ?? true),
    resumeProcess: mock(() => {}),
    subscribe: mock(() => {}),
    unsubscribe: mock(() => {}),
    subscribeAll: mock(() => {}),
...
function createMockResponseFormatter(): ResponseFormatter & {
  ...
  return {
    calls,
    sendResponse: mock((participant: string, content: string) => {
...
function createMockCommandHandler(
  overrides: { isOwnerResult?: boolean | ((addr: string) => boolean); handleCommandResult?: boolean } = {},
): CommandHandler {
  const isOwnerFn =
    typeof overrides.isOwnerResult === 'function'
...
function createMockDiscoveryService(
  overrides: { agentWalletAddresses?: Set<string>; findAgentResult?: string | null; defaultProjectId?: string } = {},
): DiscoveryService {
  return {
...
function createMockPSKContactManager(overrides: { pskContacts?: Set<string> } = {}): PSKContactManager {
  return {
...
function createMockAlgoChatService(): AlgoChatService & {
  messageHandler?: (participant: string, messages: unknown[]) => void;
} {
...
function createMockAgentDirectory(overrides: { agentAddresses?: Map<string, string> } = {}): AgentDirectory {
  return {
    findAgentByAddress: mock((addr: string) => overrides.agentAddresses?.get(addr) ?? null),
  } as unknown as AgentDirectory;
}

function createMockApprovalManager(overrides: { resolveByShortIdResult?: boolean } = {}): ApprovalManager {
  return {
...
function createMockOwnerQuestionManager(
  overrides: { findByShortIdResult?: { options?: string[] } | null; resolveByShortIdResult?: boolean } = {},
...
function seedAgentAndProject(db: Database, agentId = AGENT_ID, projectId = PROJECT_ID): void {
  ...
  db.query("INSERT INTO agents (id, name, algochat_enabled, default_project_id) VALUES (?, 'TestAgent', 1, ?)").run(
    agentId,
    projectId,
  );
}
...
describe('handleIncomingMessage', () => {
  ...
  describe('session creation for new conversations', () => {
    ...
    test('returns early when no agent is found for new conversation', async () => {
      ...
      const freshRouter = new MessageRouter(
        freshDb,
        freshPm,
        config,
        svc as unknown as AlgoChatService,
        rf,
        ownerCh,
        sm,
        noDisco,
...
describe('handleLocalMessage', () => {
  ...
  test('echoes inbound message via sendFn', async () => {
    ...
    const inboundCalls = sendFn.mock.calls.filter((c: string[]) => c[2] === 'inbound');
  ...
  test('emits inbound event via eventFn', async () => {
    ...
    const inboundEvents = eventFn.mock.calls.filter(
      (c: unknown[]) =>
        (c[0] as Record<string, unknown>).type === 'message' &&
        (c[0] as Record<string, unknown>).direction === 'inbound',
    );
...
describe('setupMessageHandler', () => {
  ...
  test('filters out messages from agent wallet addresses', () => {
    ...
    const messages = [{ content: 'From agent', direction: 'received', confirmedRound: 100, sender: AGENT_ADDR }];
...
describe('setupSessionNotifications', () => {
  ...
  test('forwards approval request events for AlgoChat sessions', async () => {
    ...
    const callback = (pm.subscribeAll as ReturnType<typeof mock>).mock.calls[0][0] as (
      sessionId: string,
