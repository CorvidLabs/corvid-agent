...
describe('SubscriptionService.subscribe', () => {
  ...
  test('records credit transactions for both subscriber and seller', () => {
    ...
    const subTxns = db
      .query("SELECT * FROM credit_transactions WHERE wallet_address = ? AND type = 'deduction'")
    ...
    const sellerTxns = db
      .query("SELECT * FROM credit_transactions WHERE wallet_address = ? AND type = 'grant'")
  ...
  test('records audit entry on subscribe', () => {
    ...
    const audits = db
      .query("SELECT * FROM audit_log WHERE action = 'credit_deduction' AND actor = ?")
