...
describe('sweepStaleAgents', () => {
  ...
  test('sweeps multiple stale agents at once', async () => {
    ...
    // Backdate two agents
    db.query(`UPDATE flock_agents SET last_heartbeat = datetime('now', '-25 hours') WHERE id IN (?, ?)`).run(
      a1.id,
      a2.id,
    );
...
describe('full lifecycle', () => {
  ...
  test('stats reflect active/inactive counts after sweep', async () => {
    ...
    db.query(`UPDATE flock_agents SET last_heartbeat = datetime('now', '-25 hours') WHERE id = ?`).run(staleAgent.id);
