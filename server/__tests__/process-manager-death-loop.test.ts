...
function countMessages(db: Database, sessionId: string): number {
  const row = db.query('SELECT COUNT(*) as cnt FROM session_messages WHERE session_id = ?').get(sessionId) as {
    cnt: number;
  };
  return row.cnt;
}
...
describe('zero-turn death loop circuit breaker', () => {
  ...
  test('full purge — user, assistant, and system messages are all deleted when circuit breaker fires', () => {
    ...
    const byRole = db
      .query('SELECT role, COUNT(*) as cnt FROM session_messages WHERE session_id = ? GROUP BY role')
