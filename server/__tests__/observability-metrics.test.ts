...
describe('Histogram (via httpRequestDuration)', () => {
  ...
  it('produces cumulative bucket counts in Prometheus output', () => {
    ...
    const bucketLines = lines.filter((l) => l.includes('_bucket') && l.includes('/hist-cumulative'));
  ...
  it('tracks count correctly', () => {
    ...
    expect(output).toContain(
      'http_request_duration_seconds_count{method="GET",route="/hist-count",status_code="200"} 3',
    );
  });
});
