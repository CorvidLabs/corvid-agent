...
import { createMcpServerConfig, getMcpServerConfig, updateMcpServerConfig } from '../db/mcp-servers';
import { createProject, getProject, updateProject } from '../db/projects';
import { runMigrations } from '../db/schema';
import { decryptEnvVars, ENCRYPTED_PREFIX, encryptEnvVars, isEncrypted } from '../lib/env-encryption';
...
describe('migration 073: encrypt existing env_vars', () => {
  ...
  test('newly created records are encrypted after migration', () => {
    ...
    db.query(
      `INSERT INTO projects (id, name, working_dir, env_vars, tenant_id) VALUES (?, 'P1', '/tmp', '{"SECRET":"abc"}', 'default')`,
    ).run(projectId);
    db.query(
      `INSERT INTO mcp_server_configs (id, name, command, args, env_vars, tenant_id) VALUES (?, 'M1', 'echo', '[]', '{"TOKEN":"xyz"}', 'default')`,
    ).run(mcpId);
