...
import { type RotationResult, rotateWalletEncryptionKey } from '../lib/key-rotation';
...
describe('rotateWalletEncryptionKey — input validation', () => {
  ...
  it('rejects same old and new passphrase', async () => {
    ...
    const result: RotationResult = await rotateWalletEncryptionKey(db, passphrase, passphrase, 'testnet');
  ...
  it('accepts new passphrase of exactly 32 characters', async () => {
    ...
    const result = await rotateWalletEncryptionKey(db, 'old-passphrase-that-is-different', 'x'.repeat(32), 'testnet');
...
describe('rotateWalletEncryptionKey — with mock agents', () => {
  ...
  function makeMockDbWithAgents(agents: Array<{ id: string; name: string; wallet_mnemonic_encrypted: string }>) {
    ...
    return {
      db: {
        ...
        prepare: () => ({
          run: (...args: any[]) => {
            updates.push({ encrypted: args[0], id: args[1] });
          },
        }),
        transaction: (fn: () => void) => fn,
      } as any,
  ...
  it('fails when old passphrase is wrong (cannot decrypt)', async () => {
    const encrypted = await encrypt(MNEMONIC, OLD_PASS);
    const { db } = makeMockDbWithAgents([{ id: 'agent-1', name: 'TestAgent', wallet_mnemonic_encrypted: encrypted }]);
