...
import { pruneTable, RETENTION_POLICIES, runRetentionCleanup } from '../db/retention';
...
describe('pruneTable', () => {
  ...
  test('rejects table name with special characters', () => {
    expect(() => pruneTable(db, { table: 'audit-log', timestampColumn: 'timestamp', retentionDays: 90 })).toThrow(
      'invalid table name',
    );
  });
