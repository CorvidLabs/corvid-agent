...
import { type HealthCheckDeps, resetHealthCache } from '../health/service';
...
describe('health-snapshots DB module', () => {
  ...
  test('listHealthSnapshots filters by since', () => {
    ...
    db.exec(
      `INSERT INTO server_health_snapshots (timestamp, status, source) VALUES ('2020-01-01T00:00:00Z', 'unhealthy', 'internal')`,
    );
    insertHealthSnapshot(db, { status: 'healthy' });
  ...
  test('pruneHealthSnapshots deletes old records', () => {
    ...
    db.exec(
      `INSERT INTO server_health_snapshots (timestamp, status, source) VALUES ('2020-01-01T00:00:00Z', 'healthy', 'internal')`,
    );
