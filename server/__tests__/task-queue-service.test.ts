...
function seedFixtures(db: Database): void {
  db.query(`INSERT INTO agents (id, name, model, system_prompt) VALUES (?, 'QueueAgent', 'test', 'test')`).run(
    AGENT_ID,
  );
  db.query(`INSERT INTO projects (id, name, working_dir) VALUES (?, 'QueueProject', '/tmp/queue-test')`).run(
    PROJECT_ID,
  );
}
...
describe('TaskQueueService', () => {
  ...
  it('tick does nothing when active count >= maxConcurrency', async () => {
    ...
    db.query(`INSERT INTO projects (id, name, working_dir) VALUES (?, 'QueueProject2', '/tmp/queue-test-2')`).run(
      PROJECT_ID_2,
    );
    const task2 = createWorkTask(db, {
      agentId: AGENT_ID,
  ...
  it('tick dispatches multiple candidates up to available slots', async () => {
    ...
    db.query(`INSERT INTO projects (id, name, working_dir) VALUES (?, 'TickA', '/tmp/tick-a')`).run(PROJECT_ID_A);
    db.query(`INSERT INTO projects (id, name, working_dir) VALUES (?, 'TickB', '/tmp/tick-b')`).run(PROJECT_ID_B);
  ...
  it('executePromoted marks task as failed when agent does not exist', async () => {
    ...
    const row = db.query(`SELECT status, error FROM work_tasks WHERE id = ?`).get(task.id) as {
      status: string;
      error: string | null;
    } | null;
