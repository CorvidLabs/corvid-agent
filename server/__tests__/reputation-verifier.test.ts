...
function makeIndexerResponse(
  transactions: Array<{ id: string; note?: string; 'confirmed-round'?: number; 'round-time'?: number }>,
) {
...
describe('scanAttestations', () => {
  ...
  test('skips transactions without notes', async () => {
    global.fetch = mock(() =>
      Promise.resolve(
        makeIndexerResponse([{ id: 'txn-1' }, { id: 'txn-2', note: makeAttestation('agent-1', 'abc123') }]),
      ),
    ) as unknown as typeof global.fetch;
