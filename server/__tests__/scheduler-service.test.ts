...
import { SchedulerService, validateScheduleFrequency } from '../scheduler/service';
...
function createTestAgentAndProject() {
  ...
  const project = createProject(db, {
    name: `TestProject-${_testCounter}-${Date.now()}`,
    workingDir: '/tmp/test-project',
  });
  const agent = createAgent(db, {
    name: `TestAgent-${_testCounter}`,
...
describe('SchedulerService', () => {
  ...
  describe('approval policy', () => {
    ...
    it('owner_approve requires approval for destructive actions', async () => {
      ...
      const execs = db
        .query(`SELECT * FROM schedule_executions WHERE schedule_id = ? AND status = 'awaiting_approval'`)
        .all(schedule.id);
  ...
  describe('resolveApproval', () => {
    ...
    it('approves an awaiting execution and sets status to approved', async () => {
      ...
      const awaitingExecs = db
        .query(`SELECT id FROM schedule_executions WHERE schedule_id = ? AND status = 'awaiting_approval'`)
  ...
  describe('execution lifecycle', () => {
    it('creates execution records with config snapshots', async () => {
      ...
      const execs = db.query('SELECT * FROM schedule_executions WHERE schedule_id = ?').all(schedule.id) as Array<
    ...
    it('records audit log entries for executions', async () => {
      ...
      const audits = db
        .query(`SELECT * FROM audit_log WHERE action = 'schedule_execute' AND actor = ?`)
  ...
  describe('action: star_repo', () => {
    it('fails when no repos specified', async () => {
      ...
      const execs = db
        .query(`SELECT * FROM schedule_executions WHERE schedule_id = ? AND status = 'failed'`)
        .all(schedule.id);
      expect(execs.length).toBe(1);
    });
  });
  ...
  describe('action: work_task', () => {
    ...
    it('fails when no description provided', async () => {
      ...
      const schedulerWithWork = new SchedulerService(db, mockPM, mockWorkTaskService as never);
  ...
  describe('notifications', () => {
    it('sends notification when notifyAddress is set and messenger available', async () => {
      ...
      expect(
        (mockMessenger.sendNotificationToAddress as ReturnType<typeof mock>).mock.calls.length,
  ...
  describe('calculateNextRun (via start initialization)', () => {
    it('sets next_run_at from cron expression', () => {
      ...
      const schedules = db.query(`SELECT next_run_at FROM agent_schedules WHERE agent_id = ?`).all(agent.id) as Array<{
        next_run_at: string;
      }>;
