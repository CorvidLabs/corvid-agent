...
function cleanup(): void {
  try {
    unlinkSync(TEST_KEYSTORE_PATH);
  } catch {
    /* ignore */
  }
  try {
    unlinkSync(`${TEST_KEYSTORE_PATH}.rotation-tmp`);
  } catch {
    /* ignore */
  }
}
...
describe('Wallet Encryption Key Rotation', () => {
  ...
  it('old key is invalid after rotation', async () => {
    ...
    const row = db.query('SELECT wallet_mnemonic_encrypted FROM agents WHERE id = ?').get('agent-1') as {
  ...
  it('rotation failure leaves system in consistent state', async () => {
    ...
    const result = await rotateWalletEncryptionKey(
      db,
      'wrong-passphrase-that-is-long-enough-32chars',
      NEW_PASSPHRASE,
      'testnet',
    );
    expect(result.success).toBe(false);
