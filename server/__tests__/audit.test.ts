...
import { queryAuditLog, recordAudit } from '../db/audit';
...
describe('queryAuditLog filtering', () => {
  ...
  test('filter by actor', () => {
    ...
    for (const e of result.entries) expect(e.actor).toBe('agent-1');
  });

  test('filter by resource type', () => {
    const result = queryAuditLog(db, { resourceType: 'credits' });
    expect(result.total).toBe(2);
    for (const e of result.entries) expect(e.resourceType).toBe('credits');
  });
