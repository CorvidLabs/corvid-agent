...
describe('SSRF Protection', () => {
  ...
  it('blocks 127.0.0.1', () => {
    expect(() => federation.registerInstance('http://127.0.0.1:3000', 'loopback')).toThrow('private or loopback');
  });
  ...
  it('blocks 0.0.0.0', () => {
    expect(() => federation.registerInstance('http://0.0.0.0:3000', 'all-ifaces')).toThrow('private or loopback');
  });

  it('blocks 10.x.x.x private IPs', () => {
    expect(() => federation.registerInstance('http://10.0.0.1:3000', 'private10')).toThrow('private or loopback');
  });

  it('blocks 192.168.x.x private IPs', () => {
    expect(() => federation.registerInstance('http://192.168.1.1:3000', 'private192')).toThrow('private or loopback');
  });

  it('blocks 172.16-31.x.x private IPs', () => {
    expect(() => federation.registerInstance('http://172.16.0.1:3000', 'private172-16')).toThrow('private or loopback');
  ...
  it('blocks 169.254.x.x link-local', () => {
    expect(() => federation.registerInstance('http://169.254.1.1:3000', 'link-local')).toThrow('private or loopback');
  });
...
describe('Sync', () => {
  ...
  it('syncInstance throws on private URL', async () => {
    ...
    await expect(federation.syncInstance('http://localhost:3000')).rejects.toThrow('private or loopback');
  });
