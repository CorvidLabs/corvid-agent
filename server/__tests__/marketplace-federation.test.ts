...
describe('SSRF Protection', () => {
  ...
  it('blocks 127.0.0.1', () => {
    expect(() => federation.registerInstance('http://127.0.0.1:3000', 'loopback')).toThrow('Blocked URL');
  });

  it('blocks IPv6 loopback [::1]', () => {
    expect(() => federation.registerInstance('http://[::1]:3000', 'ipv6loop')).toThrow('Blocked URL');
  });

  it('blocks 0.0.0.0', () => {
    expect(() => federation.registerInstance('http://0.0.0.0:3000', 'all-ifaces')).toThrow('Blocked URL');
  });

  it('blocks 10.x.x.x private IPs', () => {
    expect(() => federation.registerInstance('http://10.0.0.1:3000', 'private10')).toThrow('Blocked URL');
  });

  it('blocks 192.168.x.x private IPs', () => {
    expect(() => federation.registerInstance('http://192.168.1.1:3000', 'private192')).toThrow('Blocked URL');
  });

  it('blocks 172.16-31.x.x private IPs', () => {
    expect(() => federation.registerInstance('http://172.16.0.1:3000', 'private172-16')).toThrow('Blocked URL');
    expect(() => federation.registerInstance('http://172.31.255.255:3000', 'private172-31')).toThrow('Blocked URL');
  });

  it('blocks 169.254.x.x link-local', () => {
    expect(() => federation.registerInstance('http://169.254.1.1:3000', 'link-local')).toThrow('Blocked URL');
  });

  it('blocks .local domains', () => {
    expect(() => federation.registerInstance('http://myhost.local:3000', 'local-domain')).toThrow('Blocked URL');
  });
...
describe('Sync', () => {
  ...
  it('syncInstance throws on private URL', async () => {
    ...
    await expect(federation.syncInstance('http://localhost:3000')).rejects.toThrow('Blocked URL');
  });
