...
import { braveMultiSearch, braveWebSearch } from '../lib/web-search';
import { handleDeepResearch, handleWebSearch, type McpToolContext } from '../mcp/tool-handlers';
  ...
  test('returns organized sections with headers', async () => {
    ...
    mockFetch(async (input) => {
      ...
      return braveResponse([
        {
          title: 'Result',
          url: `https://r-${encodeURIComponent(new URL(url).searchParams.get('q') ?? '')}.com`,
          description: 'Desc',
        },
      ]);
    });
