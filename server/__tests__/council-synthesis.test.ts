...
import type { BroadcastStageChangeFn, EmitLogFn, WatchAutoAdvanceFn } from '../councils/synthesis';
import {
...
function seedCouncil(opts: { agentCount?: number; stage?: string; prompt?: string; chairmanAgentId?: string | null }) {
...
describe('triggerReview', () => {
  ...
  it('logs error when startProcess throws', () => {
    ...
    const errorLogs = (emitLog as ReturnType<typeof mock>).mock.calls.filter((c: unknown[]) => c[2] === 'error');
