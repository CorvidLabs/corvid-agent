...
const mockFindContactByPlatformId = mock((...args: any[]) =>
  (_realContacts.findContactByPlatformId as (...args: any[]) => any)(...args),
);
const mockCreateContact = mock((...args: any[]) => (_realContacts.createContact as (...args: any[]) => any)(...args));
const mockAddPlatformLink = mock((...args: any[]) =>
  (_realContacts.addPlatformLink as (...args: any[]) => any)(...args),
);
...
import { CONTACT_CACHE_TTL, contactCache, resolveDiscordContact } from '../discord/contact-linker';
...
beforeEach(() => {
  ...
  mockCreateContact.mockImplementation((_db: unknown, _tenantId: string, name: string) => ({
    id: `contact-${name}`,
    name,
  }));
...
afterEach(() => {
  ...
  mockFindContactByPlatformId.mockImplementation((...args: any[]) =>
    (_realContacts.findContactByPlatformId as (...args: any[]) => any)(...args),
  );
  mockCreateContact.mockImplementation((...args: any[]) =>
    (_realContacts.createContact as (...args: any[]) => any)(...args),
  );
  mockAddPlatformLink.mockImplementation((...args: any[]) =>
    (_realContacts.addPlatformLink as (...args: any[]) => any)(...args),
  );
});
...
describe('resolveDiscordContact', () => {
  ...
  test('addPlatformLink is called with correct discord platform and authorId', () => {
    ...
    expect(mockAddPlatformLink).toHaveBeenCalledWith(db, 'default', 'c-999', 'discord', 'discord-user-999');
  });
});
