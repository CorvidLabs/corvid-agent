...
import { clearTaskStore, handleTaskGet, handleTaskSend } from '../a2a/task-handler';
...
describe('A2A Task Handler', () => {
  ...
  it('throws when no agent with a default project is available', () => {
    ...
    expect(() => handleTaskSend(deps, { message: 'test' })).toThrow('Agent with default project not found');
  });
