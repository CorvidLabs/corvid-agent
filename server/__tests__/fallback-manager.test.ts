...
import type { LlmProviderRegistry } from '../providers/registry';
import type { LlmCompletionParams, LlmCompletionResult, LlmProvider, LlmProviderType } from '../providers/types';
...
describe('FallbackManager', () => {
  ...
  describe('completeWithFallback', () => {
    ...
    it('falls back to second provider when primary fails', async () => {
      const anthropic = createMockProvider('anthropic', () => Promise.reject(new Error('rate limit exceeded')));
      const openai = createMockProvider('openai', () => Promise.resolve(makeResult('fallback response', 'gpt-4')));
    ...
    it('falls back through multiple providers', async () => {
      const anthropic = createMockProvider('anthropic', () => Promise.reject(new Error('503 service unavailable')));
      const openai = createMockProvider('openai', () => Promise.reject(new Error('timeout waiting for response')));
      const ollama = createMockProvider('ollama', () => Promise.resolve(makeResult('local response', 'llama3')));
    ...
    it('skips providers not in registry', async () => {
      const ollama = createMockProvider('ollama', () => Promise.resolve(makeResult('ollama response', 'llama3')));
  ...
  describe('chain exhaustion', () => {
    it('throws ExternalServiceError when all providers fail', async () => {
      const anthropic = createMockProvider('anthropic', () => Promise.reject(new Error('rate limit exceeded')));
      const openai = createMockProvider('openai', () => Promise.reject(new Error('502 bad gateway')));
      const registry = createMockRegistry([anthropic, openai]);
      manager = new FallbackManager(registry);
  ...
  describe('transient error detection', () => {
    it('marks provider failure for rate limit errors', async () => {
      const anthropic = createMockProvider('anthropic', () => Promise.reject(new Error('rate limit exceeded')));
      const openai = createMockProvider('openai', () => Promise.resolve(makeResult('ok', 'gpt-4')));
      const registry = createMockRegistry([anthropic, openai]);
      manager = new FallbackManager(registry);
    ...
    it('marks provider failure for 429 errors', async () => {
      const anthropic = createMockProvider('anthropic', () => Promise.reject(new Error('HTTP 429 Too Many Requests')));
      const openai = createMockProvider('openai', () => Promise.resolve(makeResult('ok', 'gpt-4')));
      const registry = createMockRegistry([anthropic, openai]);
      manager = new FallbackManager(registry);
    ...
    it('marks provider failure for timeout and connection errors', async () => {
      ...
      for (const errorMsg of providers) {
        const anthropic = createMockProvider('anthropic', () => Promise.reject(new Error(errorMsg)));
        const openai = createMockProvider('openai', () => Promise.resolve(makeResult('ok', 'gpt-4')));
        const registry = createMockRegistry([anthropic, openai]);
    ...
    it('still tries next provider on non-transient errors', async () => {
      ...
      const openai = createMockProvider('openai', () => Promise.resolve(makeResult('fallback', 'gpt-4')));
      const registry = createMockRegistry([anthropic, openai]);
      manager = new FallbackManager(registry);

      const chain = makeChain({ provider: 'anthropic', model: 'claude-ultra' }, { provider: 'openai', model: 'gpt-4' });
  ...
  describe('cooldown and exponential backoff', () => {
    it('skips provider during cooldown period', async () => {
      ...
      const openai = createMockProvider('openai', () => Promise.resolve(makeResult('openai ok', 'gpt-4')));
      const registry = createMockRegistry([anthropic, openai]);
      manager = new FallbackManager(registry);
