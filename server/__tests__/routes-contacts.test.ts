...
describe('Contact Routes', () => {
  ...
  it('GET /api/contacts respects limit param', async () => {
    ...
    for (const id of ids) {
      await handleContactRoutes(
        fakeReq('DELETE', `/api/contacts/${id}`).req,
        fakeReq('DELETE', `/api/contacts/${id}`).url,
        db,
        ctx,
      );
    }
  ...
  it('GET /api/contacts respects offset param', async () => {
    ...
    const first = await (await handleContactRoutes(
      fakeReq('GET', '/api/contacts?limit=1').req,
      fakeReq('GET', '/api/contacts?limit=1').url,
      db,
      ctx,
    ))!.json();
    const offset1 = await (await handleContactRoutes(
      fakeReq('GET', '/api/contacts?limit=1&offset=1').req,
      fakeReq('GET', '/api/contacts?limit=1&offset=1').url,
      db,
      ctx,
    ))!.json();
