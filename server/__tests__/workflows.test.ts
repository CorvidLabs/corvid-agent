...
describe('Update Workflow', () => {
  ...
  test('update nodes replaces JSON', () => {
    ...
    const newNodes: WorkflowNode[] = [{ id: 'new-start', type: 'start', label: 'New Start', config: {} }];
  ...
  test('update edges replaces JSON', () => {
    ...
    const newEdges: WorkflowEdge[] = [{ id: 'new-e1', sourceNodeId: 'a', targetNodeId: 'b' }];
...
describe('Workflow Parallel Execution', () => {
  ...
  test('parallel node runs respect MAX_CONCURRENT_NODES (4)', () => {
    ...
    const splitEdges = run.workflowSnapshot.edges.filter((e) => e.sourceNodeId === 'parallel-split');
    expect(splitEdges).toHaveLength(5);

    // Verify the join node has 5 incoming edges
    const joinEdges = run.workflowSnapshot.edges.filter((e) => e.targetNodeId === 'join');
