...
describe('buildImprovementPrompt', () => {
  ...
  test('includes reputation context', () => {
    const prompt = buildImprovementPrompt(makeHealth(), [], makeReputation({ overallScore: 90, trustLevel: 'high' }), {
      maxTasks: 5,
    });
  ...
  test('includes focus area when provided', () => {
    const prompt = buildImprovementPrompt(makeHealth(), [], makeReputation(), {
      maxTasks: 3,
      focusArea: 'security hardening',
    });
    ...
    const memories: ScoredMemory[] = [
      {
        memory: {
          id: 'm-1',
          agentId: 'agent-1',
          key: 'improvement_loop:outcome:2026-01-01',
          content: 'Fixed 3 type errors in billing module',
          txid: null,
          asaId: null,
          status: 'confirmed',
          createdAt: '',
  ...
  test('trust level descriptions are accurate', () => {
    for (const [level, expected] of [
      ...
      const prompt = buildImprovementPrompt(makeHealth(), [], makeReputation({ trustLevel: level }), { maxTasks: 1 });
