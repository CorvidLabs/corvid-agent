...
describe('MemoryManager.save', () => {
  ...
  test('stores category in database', () => {
    ...
    const row = db.query('SELECT * FROM memory_categories WHERE memory_id = ?').get(result.id) as {
      category: string;
      confidence: number;
    } | null;
  ...
  test('stores embedding in database', () => {
    ...
    const row = db.query('SELECT * FROM memory_embeddings WHERE memory_id = ?').get(result.id) as {
      vector: string;
      vocabulary: string;
    } | null;
