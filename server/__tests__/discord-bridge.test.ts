...
describe('DiscordBridge mention-reply resume', () => {
  test('reply to bot message resumes existing session', async () => {
    ...
    const tsm = (
      bridge as unknown as {
        tsm: { mentionSessions: Map<string, import('../discord/message-handler').MentionSessionInfo> };
      }
    ).tsm;
  ...
  test('/quickstart responds with welcome embed listing agents', async () => {
    ...
    try {
      ...
      const body = fetchBodies[0] as {
        data: { embeds: Array<{ title: string; description: string; fields: Array<{ value: string }> }> };
      };
...
describe('withAuthorContext', () => {
  ...
  test('includes channel ID when provided with full author info', () => {
    expect(withAuthorContext('hello', '12345', 'Alice', '99999')).toBe(
      '[From Discord user: Alice (Discord ID: 12345) in channel 99999]\nhello',
    );
  });

  test('includes channel ID with only Discord ID', () => {
    expect(withAuthorContext('hello', '12345', undefined, '99999')).toBe(
      '[From Discord user ID: 12345 in channel 99999]\nhello',
    );
  });

  test('includes channel ID with only username', () => {
    expect(withAuthorContext('hello', undefined, 'Alice', '99999')).toBe(
      '[From Discord user: Alice in channel 99999]\nhello',
    );
  });
});
...
describe('DiscordBridge expired thread session resume', () => {
  ...
  test('falls back to dead-end embed when no agents are configured', async () => {
    ...
    try {
      ...
      const threadMessages = fetchUrls.filter((u) => u.includes('/channels/700000000000000001/messages'));
