...
describe('MarketplaceService', () => {
  ...
  test('search returns only published listings', () => {
    svc.createListing({
      agentId: 'a1',
      name: 'Draft Listing Not Published',
      description: 'Should remain in draft status always',
      category: 'coding',
      tags: ['draft'],
    });
    const pub = svc.createListing({
      agentId: 'a1',
      name: 'Published Listing Agent',
      description: 'This one should be published successfully',
      category: 'coding',
      tags: ['published'],
    });
  ...
  test('search filters by category', () => {
    const l1 = svc.createListing({
      agentId: 'a1',
      name: 'Coder Agent For Testing',
      description: 'An automated coding assistant agent',
      category: 'coding',
      tags: ['coder'],
    });
    const l2 = svc.createListing({
      agentId: 'a1',
      name: 'Writer Agent For Testing',
      description: 'An automated writing assistant agent',
      category: 'writing',
      tags: ['writer'],
    });
  ...
  test('recordUse creates credit_transactions and escrow_transactions records', () => {
    ...
    const buyerTxns = db
      .query("SELECT * FROM credit_transactions WHERE wallet_address = ? AND type = 'marketplace_use'")
    ...
    const escrowTxns = db.query('SELECT * FROM escrow_transactions WHERE listing_id = ?').all(listing.id) as {
      state: string;
