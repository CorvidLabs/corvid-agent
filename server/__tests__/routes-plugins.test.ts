...
function createMockRegistry(overrides?: Partial<PluginRegistry>): PluginRegistry {
  return {
    getLoadedPlugins: mock(() => [{ name: 'test-plugin', version: '1.0.0', description: 'A test plugin' }]),
    listAllPlugins: mock(() => [
...
beforeAll(() => {
  ...
  db.query("INSERT INTO plugins (name, package_name, version) VALUES ('test-plugin', 'test-plugin', '1.0.0')").run();
});
