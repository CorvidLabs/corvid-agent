...
describe('work-tasks-db invariants', () => {
    ...
    it('spec: updateWorkTaskStatus — completed_at set when status is "completed"', () => {
        ...
        const row = db.query(`SELECT completed_at FROM work_tasks WHERE id = ?`).get(task.id) as { completed_at: string | null };
    ...
    it('spec: valid work task statuses include all lifecycle states', () => {
        ...
        for (const status of validStatuses) {
            ...
            expect(() => updateWorkTaskStatus(db, task.id, status as Parameters<typeof updateWorkTaskStatus>[2])).not.toThrow();
        }
    });
});
