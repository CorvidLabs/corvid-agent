...
describe('SessionLifecycleManager — paused sessions in limit enforcement', () => {
    ...
    function insertSession(id: string, projectId: string, status: string, ageHours: number = 48) {
        const updatedAt = new Date(Date.now() - ageHours * 3_600_000).toISOString().replace('T', ' ').replace(/\.\d+Z$/, '');
        db.query(`
    ...
    beforeEach(() => {
        ...
        db.exec('PRAGMA foreign_keys = ON');
        runMigrations(db);
        db.query(`INSERT INTO agents (id, name, model, system_prompt) VALUES ('agent-1', 'TestAgent', 'claude-sonnet-4-6', 'test')`).run();
    ...
    it('spec: paused sessions are subject to per-project limit enforcement (only running is fully protected)', async () => {
        ...
        const remaining = (db.query(`SELECT COUNT(*) as cnt FROM sessions WHERE status = 'paused'`).get() as { cnt: number }).cnt;
    ...
    it('spec: running sessions are NEVER removed by limit enforcement', async () => {
        ...
        const remaining = (db.query(`SELECT COUNT(*) as cnt FROM sessions WHERE status = 'running'`).get() as { cnt: number }).cnt;
