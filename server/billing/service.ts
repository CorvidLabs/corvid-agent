...
export class BillingService {
  ...
  getSubscription(tenantId: string): Subscription | null {
    const row = this.db
      .query('SELECT * FROM subscriptions WHERE tenant_id = ? ORDER BY rowid DESC LIMIT 1')
   ...
   */
  addSubscriptionItem(subscriptionId: string, stripeItemId: string, stripePriceId?: string): void {
    const id = crypto.randomUUID();
    this.db
      .query(`
   ...
   */
  recordUsage(tenantId: string, credits: number, apiCalls: number = 0, sessions: number = 0): void {
  ...
  getInvoice(id: string): Invoice | null {
    const row = this.db.query('SELECT * FROM invoices WHERE id = ?').get(id) as InvoiceRecord | null;
