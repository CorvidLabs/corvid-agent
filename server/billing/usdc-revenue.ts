...
export class UsdcRevenueService {
    ...
    private async forwardUsdc(agentId: string, amountMicro: number): Promise<string | null> {
        ...
        try {
            ...
            const algodClient = (this.config.agentWalletService as unknown as { service: { algodClient: import('algosdk').Algodv2 } }).service?.algodClient;
            if (!algodClient) {
