...
export class PerformanceCollector {
  ...
  recordSlowQuery(operation: string, durationMs: number): void {
    ...
    try {
      this.db
        .query(`INSERT INTO performance_metrics (metric, labels, value, unit) VALUES (?, ?, ?, ?)`)
  ...
  detectRegressions(thresholdPercent: number = 25): Regression[] {
    ...
    const slowThisWeekCount = queryCount(
      this.db,
      `SELECT COUNT(*) as cnt FROM performance_metrics WHERE metric = 'db_slow_query' AND timestamp >= datetime('now', '-7 days')`,
    );
    const slowLastWeekCount = queryCount(
      this.db,
      `SELECT COUNT(*) as cnt FROM performance_metrics WHERE metric = 'db_slow_query' AND timestamp >= datetime('now', '-14 days') AND timestamp < datetime('now', '-7 days')`,
    );
  ...
  getStatusReportSection(): PerformanceReport {
    ...
    const slowToday = queryCount(
      this.db,
      "SELECT COUNT(*) as cnt FROM performance_metrics WHERE metric = 'db_slow_query' AND timestamp >= datetime('now', 'start of day')",
    );
  ...
  private persistSnapshot(snapshot: PerformanceSnapshot): void {
    try {
      const insert = this.db.query(`INSERT INTO performance_metrics (metric, value, unit) VALUES (?, ?, ?)`);
  ...
  private pruneOldMetrics(): void {
    try {
      const result = this.db
        .query(`DELETE FROM performance_metrics WHERE timestamp < datetime('now', '-' || ? || ' days')`)
        .run(RETENTION_DAYS);
      if (result.changes > 0) {
