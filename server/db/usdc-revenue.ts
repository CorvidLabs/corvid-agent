 ...
 */
export function markForwarded(db: Database, revenueId: number, forwardTxid: string): void {
  db.query(`UPDATE agent_usdc_revenue SET forward_txid = ?, forward_status = 'forwarded' WHERE id = ?`).run(
    forwardTxid,
    revenueId,
  );
}
 ...
 */
export function markForwardFailed(db: Database, revenueId: number): void {
  db.query(`UPDATE agent_usdc_revenue SET forward_status = 'failed' WHERE id = ?`).run(revenueId);
}
...
export function getPendingRevenue(db: Database): UsdcRevenueRow[] {
  return db
    .query(`SELECT * FROM agent_usdc_revenue WHERE forward_status = 'pending' ORDER BY created_at ASC`)
...
export function getAgentRevenue(db: Database, agentId: string): UsdcRevenueRow[] {
  return db
    .query(`SELECT * FROM agent_usdc_revenue WHERE agent_id = ? ORDER BY created_at DESC`)
