...
export function listChannelsForAgent(db: Database, agentId: string): NotificationChannel[] {
  const rows = db
    .query(`SELECT * FROM notification_channels WHERE agent_id = ? ORDER BY channel_type`)
...
export function upsertChannel(
  ...
  const row = db
    .query(`SELECT * FROM notification_channels WHERE agent_id = ? AND channel_type = ?`)
...
export function updateChannelEnabled(db: Database, id: string, enabled: boolean): boolean {
  const result = db
    .query(`UPDATE notification_channels SET enabled = ?, updated_at = datetime('now') WHERE id = ?`)
    .run(enabled ? 1 : 0, id);
...
export function listNotifications(db: Database, agentId?: string, limit: number = 50): OwnerNotification[] {
  if (agentId) {
    const rows = db
      .query(`SELECT * FROM owner_notifications WHERE agent_id = ? ORDER BY created_at DESC LIMIT ?`)
      .all(agentId, limit) as Record<string, unknown>[];
    return rows.map(rowToNotification);
  }
  const rows = db.query(`SELECT * FROM owner_notifications ORDER BY created_at DESC LIMIT ?`).all(limit) as Record<
    string,
    unknown
  >[];
  return rows.map(rowToNotification);
...
export function createDelivery(db: Database, notificationId: string, channelType: string): NotificationDelivery {
  db.query(
...
export function listFailedDeliveries(db: Database, maxAttempts: number = 3, limit: number = 50): FailedDeliveryRow[] {
  const rows = db
    .query(
      `SELECT
            d.*,
...
export function listActiveQuestionDispatches(db: Database): QuestionDispatchRow[] {
  const rows = db
    .query(`SELECT * FROM owner_question_dispatches WHERE status = 'sent' ORDER BY created_at ASC`)
    .all() as Record<string, unknown>[];
  return rows.map(rowToQuestionDispatch);
}

export function updateQuestionDispatchStatus(db: Database, id: number, status: 'sent' | 'answered' | 'expired'): void {
  db.query(`UPDATE owner_question_dispatches SET status = ? WHERE id = ?`).run(status, id);
}
...
export function getQuestionDispatchesByQuestionId(db: Database, questionId: string): QuestionDispatchRow[] {
  const rows = db
    .query(`SELECT * FROM owner_question_dispatches WHERE question_id = ? ORDER BY id ASC`)
