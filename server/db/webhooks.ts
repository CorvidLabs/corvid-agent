...
export function getWebhookRegistration(
  db: Database,
  id: string,
  tenantId: string = DEFAULT_TENANT_ID,
): WebhookRegistration | null {
  if (tenantId !== DEFAULT_TENANT_ID && !validateTenantOwnership(db, 'webhook_registrations', id, tenantId))
    return null;
...
export function listWebhookRegistrations(
  db: Database,
  agentId?: string,
  tenantId: string = DEFAULT_TENANT_ID,
): WebhookRegistration[] {
  if (agentId) {
    const { query, bindings } = withTenantFilter(
      'SELECT * FROM webhook_registrations WHERE agent_id = ? ORDER BY created_at DESC',
      tenantId,
    );
...
export function findRegistrationsForRepo(db: Database, repo: string): WebhookRegistration[] {
  const rows = db
    .query(`SELECT * FROM webhook_registrations WHERE repo = ? AND status = 'active' ORDER BY created_at ASC`)
    .all(repo);
  return (rows as Record<string, unknown>[]).map(rowToRegistration);
}

export function updateWebhookRegistration(
  db: Database,
  id: string,
  input: UpdateWebhookRegistrationInput,
  tenantId: string = DEFAULT_TENANT_ID,
): WebhookRegistration | null {
...
export function incrementTriggerCount(db: Database, id: string): void {
  db.query(
    `UPDATE webhook_registrations SET trigger_count = trigger_count + 1, updated_at = datetime('now') WHERE id = ?`,
  ).run(id);
}
...
export function listDeliveries(db: Database, registrationId?: string, limit: number = 50): WebhookDelivery[] {
  const rows = registrationId
    ? db
        .query('SELECT * FROM webhook_deliveries WHERE registration_id = ? ORDER BY created_at DESC LIMIT ?')
        .all(registrationId, limit)
