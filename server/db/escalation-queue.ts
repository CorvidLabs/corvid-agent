...
export function enqueueRequest(
  ...
  const result = db
    .query(`INSERT INTO escalation_queue (session_id, tool_name, tool_input) VALUES (?, ?, ?)`)
    .run(sessionId, toolName, JSON.stringify(toolInput));

  const row = db.query(`SELECT * FROM escalation_queue WHERE id = ?`).get(result.lastInsertRowid) as EscalationRow;

  return rowToEscalation(row);
}

export function resolveRequest(db: Database, id: number, resolution: 'approved' | 'denied'): EscalationRequest | null {
  db.query(
    `UPDATE escalation_queue SET status = ?, resolved_at = datetime('now') WHERE id = ? AND status = 'pending'`,
  ).run(resolution, id);

  const row = db.query(`SELECT * FROM escalation_queue WHERE id = ?`).get(id) as EscalationRow | null;
...
export function getPendingRequests(db: Database): EscalationRequest[] {
  const rows = db
    .query(`SELECT * FROM escalation_queue WHERE status = 'pending' ORDER BY created_at ASC`)
