...
import type { PluginCapabilityRecord, PluginRecord } from '../plugins/types';
...
export function getPluginCapabilities(db: Database, pluginName: string): PluginCapabilityRecord[] {
  return db
    .query('SELECT plugin_name, capability, granted, granted_at FROM plugin_capabilities WHERE plugin_name = ?')
