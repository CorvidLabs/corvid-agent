/**
 * Squashed baseline migration — captures the full schema at v78 (v0.21.0 boundary).
 *
 * This replaces 29 incremental migration files (001–078) with a single
 * idempotent baseline. For fresh databases, this creates the entire schema
 * in one pass. For existing databases already at version 78, this migration
 * is skipped by the runner (getCurrentVersion() returns 78, which is >= 78).
 *
 * All CREATE TABLE statements include every column that existed at v78,
 * including those originally added via ALTER TABLE ADD COLUMN in earlier
 * incremental migrations. No ALTER TABLE statements are needed since this
 * is a clean baseline.
 */
...
const FTS = `CREATE VIRTUAL TABLE IF NOT EXISTS agent_memories_fts USING fts5(
    key,
    content,
