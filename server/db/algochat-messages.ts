...
export function listRecentAlgoChatMessages(
  ...
  const rows = db
    .query(`SELECT * FROM algochat_messages ORDER BY created_at DESC LIMIT ? OFFSET ?`)
...
export function searchAlgoChatMessages(
  ...
  const rows = db
    .query(`SELECT * FROM algochat_messages ${where} ORDER BY created_at DESC LIMIT ? OFFSET ?`)
 ...
 */
export function getWalletSummaries(db: Database, options?: { search?: string }): WalletSummary[] {
  let where = '';
  const params: string[] = [];
  ...
  return rows.map((row) => {
    const allowlistRow = db.query('SELECT label FROM algochat_allowlist WHERE address = ?').get(row.participant) as {
      label: string;
    } | null;

    const creditRow = db
      .query('SELECT credits, total_purchased FROM credit_ledger WHERE wallet_address = ?')
...
export function getWalletMessages(
  ...
  const rows = db
    .query(`SELECT * FROM algochat_messages WHERE participant = ? ORDER BY created_at DESC LIMIT ? OFFSET ?`)
