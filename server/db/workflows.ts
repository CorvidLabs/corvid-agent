...
export function createWorkflowRun(
  ...
  db.query(`
        ...
        VALUES (?, ?, ?, 'running', ?, ?, '[]', ?)
    `).run(id, workflowId, agentId, JSON.stringify(input), JSON.stringify(workflowSnapshot), now);
...
export function listActiveRuns(db: Database): WorkflowRun[] {
  const rows = db
    .query(`SELECT * FROM workflow_runs WHERE status IN ('running', 'paused') ORDER BY started_at ASC`)
    .all();
...
export function listNodeRuns(db: Database, runId: string): WorkflowNodeRun[] {
  const rows = db
    .query('SELECT * FROM workflow_node_runs WHERE run_id = ? ORDER BY started_at ASC NULLS LAST')
    .all(runId);
...
export function getNodeRunByNodeId(db: Database, runId: string, nodeId: string): WorkflowNodeRun | null {
  const row = db
    .query('SELECT * FROM workflow_node_runs WHERE run_id = ? AND node_id = ?')
