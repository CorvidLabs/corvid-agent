...
export function releaseRepoLock(db: Database, repo: string, executionId: string): boolean {
  const result = db.query(`DELETE FROM repo_locks WHERE repo = ? AND execution_id = ?`).run(repo, executionId);
...
export function releaseAllLocks(db: Database, executionId: string): number {
  const result = db.query(`DELETE FROM repo_locks WHERE execution_id = ?`).run(executionId);
...
export function cleanExpiredLocks(db: Database): number {
  const result = db.query(`DELETE FROM repo_locks WHERE expires_at < datetime('now')`).run();
