...
import type { ReputationEventRecord, ReputationRecord } from '../reputation/types';

export function getReputationRecord(db: Database, agentId: string): ReputationRecord | null {
  return db.query('SELECT * FROM agent_reputation WHERE agent_id = ?').get(agentId) as ReputationRecord | null;
}

export function listReputationRecords(db: Database): ReputationRecord[] {
  return db.query('SELECT * FROM agent_reputation ORDER BY overall_score DESC').all() as ReputationRecord[];
}

export function getReputationEvents(db: Database, agentId: string, limit: number = 50): ReputationEventRecord[] {
  return db
    .query('SELECT * FROM reputation_events WHERE agent_id = ? ORDER BY created_at DESC LIMIT ?')
