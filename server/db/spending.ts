...
function ensureRow(db: Database, date: string): void {
  db.query(`INSERT OR IGNORE INTO daily_spending (date, algo_micro, api_cost_usd) VALUES (?, 0, 0.0)`).run(date);
}
...
export function recordAlgoSpend(db: Database, microAlgos: number): void {
  ...
  writeTransaction(db, (db) => {
    ensureRow(db, date);
    db.query(`UPDATE daily_spending SET algo_micro = algo_micro + ? WHERE date = ?`).run(microAlgos, date);
  });
}
...
export function recordApiCost(db: Database, usd: number): void {
  ...
  writeTransaction(db, (db) => {
    ensureRow(db, date);
    db.query(`UPDATE daily_spending SET api_cost_usd = api_cost_usd + ? WHERE date = ?`).run(usd, date);
  });
}
...
export function getDailyTotals(db: Database): DailyTotals {
  const date = today();
  ensureRow(db, date);
  const row = db.query(`SELECT date, algo_micro, api_cost_usd FROM daily_spending WHERE date = ?`).get(date) as {
    date: string;
    algo_micro: number;
    api_cost_usd: number;
  };
...
export function recordAgentAlgoSpend(db: Database, agentId: string, microAlgos: number): void {
  ...
  writeTransaction(db, (db) => {
    ...
    db.query(`UPDATE agent_daily_spending SET algo_micro = algo_micro + ? WHERE agent_id = ? AND date = ?`).run(
      microAlgos,
      agentId,
      date,
    );
...
export function checkAgentAlgoLimit(db: Database, agentId: string, additionalMicro: number): void {
  writeTransaction(db, (db) => {
    ...
    if (globalProjected > DAILY_ALGO_LIMIT_MICRO) {
      const limitAlgo = (DAILY_ALGO_LIMIT_MICRO / 1_000_000).toFixed(6);
      const currentAlgo = (totals.algoMicro / 1_000_000).toFixed(6);
      throw new RateLimitError(`Daily global ALGO spending limit reached: ${currentAlgo}/${limitAlgo} ALGO`);
    }
    ...
    if (agentProjected > agentLimit) {
      ...
      throw new RateLimitError(`Agent daily ALGO spending limit reached: ${currentAlgo}/${limitAlgo} ALGO`);
    }
  });
}
...
export function getAgentDailySpending(db: Database, agentId: string): AgentDailySpending {
  ...
  ensureAgentRow(db, agentId, date);
  const row = db
    .query(`SELECT agent_id, date, algo_micro, usdc_micro FROM agent_daily_spending WHERE agent_id = ? AND date = ?`)
    .get(agentId, date) as {
    agent_id: string;
    date: string;
    algo_micro: number;
