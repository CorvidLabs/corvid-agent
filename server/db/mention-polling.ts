...
export function createMentionPollingConfig(
  db: Database,
  input: CreateMentionPollingInput,
...
export function getMentionPollingConfig(
  db: Database,
  id: string,
  tenantId: string = DEFAULT_TENANT_ID,
): MentionPollingConfig | null {
  if (tenantId !== DEFAULT_TENANT_ID && !validateTenantOwnership(db, 'mention_polling_configs', id, tenantId))
    return null;
  const row = db.query('SELECT * FROM mention_polling_configs WHERE id = ?').get(id) as Record<string, unknown> | null;
  return row ? rowToConfig(row) : null;
}

export function listMentionPollingConfigs(
  db: Database,
  agentId?: string,
  tenantId: string = DEFAULT_TENANT_ID,
): MentionPollingConfig[] {
  if (agentId) {
    const { query, bindings } = withTenantFilter(
      'SELECT * FROM mention_polling_configs WHERE agent_id = ? ORDER BY created_at DESC',
      tenantId,
    );
    return (db.query(query).all(agentId, ...bindings) as Record<string, unknown>[]).map(rowToConfig);
  }
  const { query, bindings } = withTenantFilter(
    'SELECT * FROM mention_polling_configs ORDER BY created_at DESC',
    tenantId,
  );
...
export function updateMentionPollingConfig(
  db: Database,
  id: string,
  input: UpdateMentionPollingInput,
  tenantId: string = DEFAULT_TENANT_ID,
): MentionPollingConfig | null {
  ...
  if (input.mentionUsername !== undefined) {
    fields.push('mention_username = ?');
    values.push(input.mentionUsername);
  }
  ...
  if (input.intervalSeconds !== undefined) {
    ...
    values.push(input.intervalSeconds);
  }
  ...
  if (input.eventFilter !== undefined) {
    ...
    values.push(JSON.stringify(input.eventFilter));
  }
  if (input.allowedUsers !== undefined) {
    fields.push('allowed_users = ?');
    values.push(JSON.stringify(input.allowedUsers));
  }
...
export function updatePollState(db: Database, id: string, lastSeenId?: string): void {
  if (lastSeenId !== undefined) {
    db.query(
      `UPDATE mention_polling_configs SET last_poll_at = datetime('now'), last_seen_id = ?, updated_at = datetime('now') WHERE id = ?`,
    ).run(lastSeenId, id);
  } else {
    db.query(
      `UPDATE mention_polling_configs SET last_poll_at = datetime('now'), updated_at = datetime('now') WHERE id = ?`,
    ).run(id);
  }
}
...
export function incrementPollingTriggerCount(db: Database, id: string): void {
  db.query(
    `UPDATE mention_polling_configs SET trigger_count = trigger_count + 1, updated_at = datetime('now') WHERE id = ?`,
  ).run(id);
}
...
export function updateProcessedIds(db: Database, id: string, processedIds: string[]): void {
  ...
  const capped = processedIds.length > MAX_PROCESSED_IDS ? processedIds.slice(-MAX_PROCESSED_IDS) : processedIds;
