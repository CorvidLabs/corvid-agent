...
export function insertHealthSnapshot(
  ...
  const stmt = db.prepare(`
        INSERT INTO server_health_snapshots (status, response_time_ms, dependencies, source)
        VALUES (?, ?, ?, ?)
    `);
  stmt.run(
    snapshot.status,
  ...
  const row = db.query(`SELECT * FROM server_health_snapshots ORDER BY id DESC LIMIT 1`).get() as SnapshotRow;
  return rowToSnapshot(row);
}

export function listHealthSnapshots(db: Database, opts?: { limit?: number; since?: string }): HealthSnapshot[] {
  const limit = opts?.limit ?? 100;
  if (opts?.since) {
    const rows = db
      .query(`SELECT * FROM server_health_snapshots WHERE timestamp >= ? ORDER BY timestamp DESC LIMIT ?`)
      .all(opts.since, limit) as SnapshotRow[];
    return rows.map(rowToSnapshot);
  }
  const rows = db
    .query(`SELECT * FROM server_health_snapshots ORDER BY timestamp DESC LIMIT ?`)
...
export function pruneHealthSnapshots(db: Database, olderThanDays: number): number {
  ...
  const result = db.query(`DELETE FROM server_health_snapshots WHERE timestamp < ?`).run(cutoff);
