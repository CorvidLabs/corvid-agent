...
export function insertHealthSnapshot(
    ...
    const row = db.query(
        `SELECT * FROM server_health_snapshots ORDER BY id DESC LIMIT 1`,
    ).get() as SnapshotRow;
...
export function listHealthSnapshots(
    ...
    if (opts?.since) {
        const rows = db.query(
            `SELECT * FROM server_health_snapshots WHERE timestamp >= ? ORDER BY timestamp DESC LIMIT ?`,
        ).all(opts.since, limit) as SnapshotRow[];
        return rows.map(rowToSnapshot);
    }
    const rows = db.query(
        `SELECT * FROM server_health_snapshots ORDER BY timestamp DESC LIMIT ?`,
