...
export function listBuddyPairings(db: Database, agentId: string): BuddyPairing[] {
  const rows = db
    .prepare('SELECT * FROM buddy_pairings WHERE agent_id = ? ORDER BY created_at')
    .all(agentId) as PairingRow[];
  return rows.map(rowToPairing);
}

export function getDefaultBuddyForAgent(db: Database, agentId: string): BuddyPairing | null {
  const row = db
    .prepare('SELECT * FROM buddy_pairings WHERE agent_id = ? AND enabled = 1 ORDER BY created_at LIMIT 1')
...
export function updateBuddyPairing(
  ...
  if (updates.buddyRole !== undefined) {
    sets.push('buddy_role = ?');
    values.push(updates.buddyRole);
  }
...
export function listBuddySessions(
  db: Database,
  opts?: {
    leadAgentId?: string;
    buddyAgentId?: string;
  ...
  const values: (string | number)[] = [];
  if (opts?.leadAgentId) {
    conditions.push('lead_agent_id = ?');
    values.push(opts.leadAgentId);
  }
  if (opts?.buddyAgentId) {
    conditions.push('buddy_agent_id = ?');
    values.push(opts.buddyAgentId);
  }
  if (opts?.workTaskId) {
    conditions.push('work_task_id = ?');
    values.push(opts.workTaskId);
  }
  if (opts?.status) {
  ...
  values.push(limit);
  const rows = db
    .prepare(`SELECT * FROM buddy_sessions ${where} ORDER BY created_at DESC LIMIT ?`)
    .all(...values) as SessionRow[];
  return rows.map(rowToSession);
}

export function updateBuddySessionStatus(db: Database, id: string, status: BuddySessionStatus, round?: number): void {
  if (status === 'completed' || status === 'failed') {
    ...
    db.prepare('UPDATE buddy_sessions SET status = ?, current_round = COALESCE(?, current_round) WHERE id = ?').run(
      status,
      round ?? null,
      id,
    );
...
export function listBuddyMessages(db: Database, buddySessionId: string): BuddyMessage[] {
  const rows = db
    .prepare('SELECT * FROM buddy_messages WHERE buddy_session_id = ? ORDER BY round ASC, created_at ASC')
