 ...
 */
export function purchaseCredits(db: Database, walletAddress: string, microAlgos: number, txid?: string): number {
 ...
 */
export function grantCredits(db: Database, walletAddress: string, amount: number, reference?: string): void {
  if (!Number.isInteger(amount) || amount <= 0) {
    throw new Error(`Cannot grant non-positive or fractional credit amount: ${amount}`);
  }
  ...
  recordAudit(
    ...
    'credit_ledger',
    walletAddress,
    `Granted ${amount} credits (${reference ?? 'no reference'})`,
  );
}
...
export function deductTurnCredits(
  ...
  const result = writeTransaction(db, (db) => {
    ...
    if (sessionId) {
      db.query(`UPDATE sessions SET credits_consumed = credits_consumed + ? WHERE id = ?`).run(
  ...
  if (result.success) {
    ...
    recordAudit(
      ...
      'credit_ledger',
      walletAddress,
      `Deducted ${config.creditsPerTurn} credits (turn), remaining: ${result.creditsRemaining}`,
    );
  }
...
export function reserveGroupCredits(
  ...
  return writeTransaction(db, (db) => {
    ...
    const updated = db
      ...
      .run(reserveAmount, walletAddress, reserveAmount);
 ...
 */
export function consumeReservedCredits(db: Database, walletAddress: string, amount: number, sessionId?: string): void {
 ...
 */
export function releaseReservedCredits(db: Database, walletAddress: string, amount: number): void {
...
export function canStartSession(
  db: Database,
  walletAddress: string,
): { allowed: boolean; credits: number; reason?: string } {
  const balance = getBalance(db, walletAddress);
 ...
 */
export function getTransactionHistory(db: Database, walletAddress: string, limit = 20): CreditTransaction[] {
  const rows = db
    .query(
...
export function isFirstTimeWallet(db: Database, walletAddress: string): boolean {
  const row = db.query(`SELECT total_purchased FROM credit_ledger WHERE wallet_address = ?`).get(walletAddress) as {
    total_purchased: number;
  } | null;
 ...
 * @returns The number of credits added
 */
export function depositUsdc(db: Database, walletAddress: string, usdcMicro: number, txid: string): number {
  const config = getCreditConfig(db);
  ...
  const existing = db
    .query(`SELECT id FROM credit_transactions WHERE txid = ? AND type = 'usdc_deposit'`)
 ...
 */
export function getUsdcDepositHistory(db: Database, walletAddress: string, limit = 20): CreditTransaction[] {
  const rows = db
    .query(
      `SELECT id, wallet_address, type, amount, balance_after, reference, txid, session_id, created_at
         FROM credit_transactions
