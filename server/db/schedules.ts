...
function rowToSchedule(row: Record<string, unknown>): AgentSchedule {
  return {
    ...
    outputDestinations: row.output_destinations
      ? (JSON.parse(row.output_destinations as string) as ScheduleOutputDestination[])
      : null,
...
export function listSchedules(db: Database, agentId?: string, tenantId: string = DEFAULT_TENANT_ID): AgentSchedule[] {
  ...
  if (agentId) {
    const { query, bindings } = withTenantFilter(
      `SELECT * FROM agent_schedules WHERE agent_id = ? ${orderBy}`,
      tenantId,
    );
...
export function listActiveSchedules(db: Database): AgentSchedule[] {
  const rows = db.query(`SELECT * FROM agent_schedules WHERE status = 'active' ORDER BY next_run_at ASC`).all();
...
export function updateSchedule(
  ...
  if (input.maxExecutions !== undefined) {
    fields.push('max_executions = ?');
    values.push(input.maxExecutions);
  }
  if (input.maxBudgetPerRun !== undefined) {
    fields.push('max_budget_per_run = ?');
    values.push(input.maxBudgetPerRun);
  }
  if (input.notifyAddress !== undefined) {
    fields.push('notify_address = ?');
    values.push(input.notifyAddress);
  }
  if (input.triggerEvents !== undefined) {
    fields.push('trigger_events = ?');
    values.push(input.triggerEvents ? JSON.stringify(input.triggerEvents) : null);
  }
  if (input.outputDestinations !== undefined) {
    fields.push('output_destinations = ?');
    values.push(input.outputDestinations ? JSON.stringify(input.outputDestinations) : null);
  }
  if (input.executionMode !== undefined) {
    fields.push('execution_mode = ?');
    values.push(input.executionMode);
  }
  if (input.pipelineSteps !== undefined) {
    fields.push('pipeline_steps = ?');
    values.push(input.pipelineSteps ? JSON.stringify(input.pipelineSteps) : null);
  }
...
export function findSchedulesForEvent(
  ...
  const rows = db
    .query(`SELECT * FROM agent_schedules WHERE status = 'active' AND trigger_events IS NOT NULL`)
    .all() as Record<string, unknown>[];

  return rows.map(rowToSchedule).filter((schedule) => {
...
export function listExecutionsFiltered(
  ...
  const rows = db
    .query(`SELECT * FROM schedule_executions ${where} ORDER BY started_at DESC LIMIT ? OFFSET ?`)
...
export function updateExecutionStatus(
  ...
  if (extras?.sessionId !== undefined) {
    ...
    values.push(extras.sessionId);
  }
  if (extras?.workTaskId !== undefined) {
    fields.push('work_task_id = ?');
    values.push(extras.workTaskId);
  }
  if (extras?.costUsd !== undefined) {
    fields.push('cost_usd = ?');
    values.push(extras.costUsd);
  }
