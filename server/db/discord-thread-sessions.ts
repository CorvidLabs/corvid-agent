 ...
 */
export function saveThreadSession(db: Database, threadId: string, info: ThreadSessionInfo): void {
  db.query(
 ...
 */
export function getThreadSession(db: Database, threadId: string): ThreadSessionInfo | null {
  const row = db
    .query(`SELECT * FROM discord_thread_sessions WHERE thread_id = ?`)
 ...
 */
export function updateThreadSessionActivity(db: Database, threadId: string): void {
  db.query(`UPDATE discord_thread_sessions SET last_activity_at = datetime('now') WHERE thread_id = ?`).run(threadId);
}
...
export function getRecentThreadSessions(
  ...
  return rows.map((row) => ({
    ...
    lastActivityAt: new Date(
      row.last_activity_at.endsWith('Z') ? row.last_activity_at : `${row.last_activity_at}Z`,
    ).getTime(),
  }));
}
 ...
 */
export function pruneOldThreadSessions(db: Database, maxAgeDays: number = 14): number {
  const result = db
    .query(`DELETE FROM discord_thread_sessions WHERE last_activity_at < datetime('now', '-' || ? || ' days')`)
    .run(maxAgeDays);
