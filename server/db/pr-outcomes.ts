...
export function getPrOutcomeByWorkTask(db: Database, workTaskId: string): PrOutcome | null {
  const row = db.query('SELECT * FROM pr_outcomes WHERE work_task_id = ?').get(workTaskId) as PrOutcomeRow | null;
  return row ? rowToOutcome(row) : null;
}

export function listOpenPrOutcomes(db: Database): PrOutcome[] {
  const rows = db
    .query(`SELECT * FROM pr_outcomes WHERE pr_state = 'open' ORDER BY created_at ASC`)
    .all() as PrOutcomeRow[];
  return rows.map(rowToOutcome);
}

export function listPrOutcomes(
  db: Database,
  opts?: {
    repo?: string;
    prState?: PrState;
  ...
  const rows = db
    .query(`SELECT * FROM pr_outcomes ${where} ORDER BY created_at DESC LIMIT ?`)
    .all(...params, limit) as PrOutcomeRow[];
  return rows.map(rowToOutcome);
}

export function updatePrOutcomeState(db: Database, id: string, prState: PrState, failureReason?: FailureReason): void {
...
export function markPrChecked(db: Database, id: string): void {
  db.query(`UPDATE pr_outcomes SET checked_at = datetime('now') WHERE id = ?`).run(id);
}
