...
export function searchObservations(db: Database, agentId: string, query: string): MemoryObservation[] {
  // FTS5 search
  try {
...
export function boostObservation(db: Database, id: string, scoreBoost: number = 1.0): void {
  db.query(
...
export function markGraduated(db: Database, id: string, graduatedKey: string): void {
  db.query(
    `UPDATE memory_observations
...
export function dismissObservation(db: Database, id: string): void {
  db.query(`UPDATE memory_observations SET status = 'dismissed' WHERE id = ?`).run(id);
}
