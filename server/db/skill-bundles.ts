...
import type { CreateSkillBundleInput, SkillBundle, UpdateSkillBundleInput } from '../../shared/types';
...
export function createBundle(db: Database, input: CreateSkillBundleInput): SkillBundle {
  ...
  db.query(
    `INSERT INTO skill_bundles (id, name, description, tools, prompt_additions)
         VALUES (?, ?, ?, ?, ?)`,
  ).run(id, input.name, input.description ?? '', JSON.stringify(input.tools ?? []), input.promptAdditions ?? '');
...
export function assignBundle(db: Database, agentId: string, bundleId: string, sortOrder: number = 0): boolean {
  ...
  db.query(`INSERT OR REPLACE INTO agent_skills (agent_id, bundle_id, sort_order) VALUES (?, ?, ?)`).run(
    agentId,
    bundleId,
    sortOrder,
  );
...
export function unassignBundle(db: Database, agentId: string, bundleId: string): boolean {
  const result = db.query('DELETE FROM agent_skills WHERE agent_id = ? AND bundle_id = ?').run(agentId, bundleId);
...
export function resolveAgentPromptAdditions(db: Database, agentId: string): string {
  ...
  const additions = bundles.map((b) => b.promptAdditions).filter(Boolean);
...
export function assignProjectBundle(db: Database, projectId: string, bundleId: string, sortOrder: number = 0): boolean {
  ...
  db.query(`INSERT OR REPLACE INTO project_skills (project_id, bundle_id, sort_order) VALUES (?, ?, ?)`).run(
    projectId,
    bundleId,
    sortOrder,
  );
...
export function unassignProjectBundle(db: Database, projectId: string, bundleId: string): boolean {
  const result = db.query('DELETE FROM project_skills WHERE project_id = ? AND bundle_id = ?').run(projectId, bundleId);
  return result.changes > 0;
}
