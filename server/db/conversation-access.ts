...
export function listAgentAllowlist(db: Database, agentId: string): AgentAllowlistEntry[] {
  const rows = db
    .query('SELECT * FROM agent_conversation_allowlist WHERE agent_id = ? ORDER BY created_at DESC')
...
export function isOnAgentAllowlist(db: Database, agentId: string, address: string): boolean {
  const row = db
    .query('SELECT 1 FROM agent_conversation_allowlist WHERE agent_id = ? AND address = ? LIMIT 1')
    .get(agentId, address);
  return row != null;
}
...
export function addToAgentAllowlist(
  ...
  const row = db
    .query('SELECT * FROM agent_conversation_allowlist WHERE agent_id = ? AND address = ?')
...
export function removeFromAgentAllowlist(db: Database, agentId: string, address: string): boolean {
  const result = db
    .query('DELETE FROM agent_conversation_allowlist WHERE agent_id = ? AND address = ?')
    .run(agentId, address);
...
export function listAgentBlocklist(db: Database, agentId: string): AgentBlocklistEntry[] {
  const rows = db
    .query('SELECT * FROM agent_conversation_blocklist WHERE agent_id = ? ORDER BY created_at DESC')
...
export function isOnAgentBlocklist(db: Database, agentId: string, address: string): boolean {
  const row = db
    .query('SELECT 1 FROM agent_conversation_blocklist WHERE agent_id = ? AND address = ? LIMIT 1')
    .get(agentId, address);
  return row != null;
}
...
export function addToAgentBlocklist(
  ...
  const row = db
    .query('SELECT * FROM agent_conversation_blocklist WHERE agent_id = ? AND address = ?')
...
export function removeFromAgentBlocklist(db: Database, agentId: string, address: string): boolean {
  const result = db
    .query('DELETE FROM agent_conversation_blocklist WHERE agent_id = ? AND address = ?')
    .run(agentId, address);
  return result.changes > 0;
}
