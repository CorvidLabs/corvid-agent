...
function buildLikeClause(column: string): string {
  ...
  return TEST_PATTERNS.flatMap(() => [`lower(${column}) LIKE ?`, `lower(${column}) LIKE ?`]).join(' OR ');
}
...
export function purgeTestData(db: Database, options: { dryRun?: boolean } = {}): PurgeResult {
  ...
  const testCouncils = db.query(`SELECT id, name FROM councils WHERE ${councilWhere}`).all(...likeParams()) as {
    id: string;
    name: string;
  }[];
