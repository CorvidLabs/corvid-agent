...
export function isAgentBlocked(db: Database, agentId: string): boolean {
  const row = db.query('SELECT 1 FROM agent_blocklist WHERE agent_id = ? LIMIT 1').get(agentId);
  return !!row;
}

export function getAgentBlocklistEntry(db: Database, agentId: string): AgentBlocklistEntry | null {
  const row = db.query('SELECT * FROM agent_blocklist WHERE agent_id = ?').get(agentId) as AgentBlocklistRow | null;
...
export function addToAgentBlocklist(
  ...
  ).run(agentId, opts?.reason ?? 'manual', opts?.detail ?? '', opts?.blockedBy ?? 'system');
  return getAgentBlocklistEntry(db, agentId)!;
}

export function removeFromAgentBlocklist(db: Database, agentId: string): boolean {
  const result = db.query('DELETE FROM agent_blocklist WHERE agent_id = ?').run(agentId);
  return result.changes > 0;
}

export function listAgentBlocklist(db: Database): AgentBlocklistEntry[] {
  const rows = db.query('SELECT * FROM agent_blocklist ORDER BY created_at DESC').all() as AgentBlocklistRow[];
