...
export function saveLibraryEntry(
    ...
    const row = db.query(
        'SELECT * FROM agent_library WHERE key = ?'
    ).get(params.key) as LibraryRow | null;
...
export function getLibraryEntry(db: Database, key: string): LibraryEntry | null {
    const row = db.query(
        'SELECT * FROM agent_library WHERE key = ? AND archived = 0'
    ).get(key) as LibraryRow | null;
    return row ? rowToEntry(row) : null;
}

export function getLibraryEntryByAsaId(db: Database, asaId: number): LibraryEntry | null {
    const row = db.query(
        'SELECT * FROM agent_library WHERE asa_id = ?'
    ).get(asaId) as LibraryRow | null;
    return row ? rowToEntry(row) : null;
}
...
export function getBookPages(db: Database, book: string): LibraryEntry[] {
    const rows = db.query(
        'SELECT * FROM agent_library WHERE book = ? AND archived = 0 ORDER BY page ASC'
...
export function updateLibraryEntryTxid(db: Database, key: string, txid: string): void {
    db.query(
        'UPDATE agent_library SET txid = ?, updated_at = datetime(\'now\') WHERE key = ?'
    ).run(txid, key);
}

export function updateLibraryEntryAsaId(db: Database, key: string, asaId: number): void {
    db.query(
        'UPDATE agent_library SET asa_id = ?, updated_at = datetime(\'now\') WHERE key = ?'
    ).run(asaId, key);
}

export function archiveLibraryEntry(db: Database, key: string): boolean {
    const result = db.query(
        "UPDATE agent_library SET archived = 1, updated_at = datetime('now') WHERE key = ?"
    ).run(key);
...
export function deleteLibraryEntryRow(db: Database, key: string): boolean {
    const result = db.query(
        'DELETE FROM agent_library WHERE key = ?'
    ).run(key);
    return (result as unknown as { changes: number }).changes > 0;
}
