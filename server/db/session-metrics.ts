...
export function insertSessionMetrics(db: Database, input: SessionMetricsInput): SessionMetrics {
    ...
    const row = db.query('SELECT * FROM session_metrics WHERE id = ?').get(
        result.lastInsertRowid,
    ) as SessionMetricsRow;
    return rowToMetrics(row);
}

export function getSessionMetrics(db: Database, sessionId: string): SessionMetrics[] {
    const rows = db.query(
        'SELECT * FROM session_metrics WHERE session_id = ? ORDER BY created_at ASC',
    ).all(sessionId) as SessionMetricsRow[];
    return rows.map(rowToMetrics);
}

export function getMetricsAggregate(db: Database, options?: {
    model?: string;
    tier?: string;
    days?: number;
}): {
...
export function listRecentMetrics(db: Database, limit: number = 20): SessionMetrics[] {
    const rows = db.query(
        'SELECT * FROM session_metrics ORDER BY created_at DESC LIMIT ?',
