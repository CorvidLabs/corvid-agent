 ...
 */
export async function migrateUp(db: Database, target?: number, dir?: string): Promise<{ applied: number; to: number }> {
  const current = getCurrentVersion(db);
  const migrations = discoverMigrations(dir);
  const pending = migrations.filter((m) => m.version > current && (target === undefined || m.version <= target));
...
export async function migrateDown(
  ...
  const toRevert = migrations.filter((m) => m.version <= current && m.version > effectiveTarget).reverse();
