...
import type { AgentVariant, AgentVariantAssignment, CreateVariantInput, UpdateVariantInput } from '../../shared/types';
...
export function getAgentVariantAssignment(db: Database, agentId: string): AgentVariantAssignment | null {
  const row = db
    .query('SELECT * FROM agent_variant_assignments WHERE agent_id = ?')
...
export function applyVariant(db: Database, agentId: string, variantId: string): boolean {
  ...
  db.query(`INSERT OR REPLACE INTO agent_variant_assignments (agent_id, variant_id) VALUES (?, ?)`).run(
    agentId,
    variantId,
  );
...
export function removeVariant(db: Database, agentId: string): boolean {
  ...
  const result = db.query('DELETE FROM agent_variant_assignments WHERE agent_id = ?').run(agentId);
  return result.changes > 0;
}
