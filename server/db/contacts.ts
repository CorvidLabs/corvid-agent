...
export function createContact(db: Database, tenantId: string, displayName: string, notes?: string | null): Contact {
...
export function getContact(db: Database, tenantId: string, contactId: string): Contact | null {
  const row = db
    .query('SELECT * FROM contacts WHERE id = ? AND tenant_id = ?')
...
export function listContacts(
  ...
  const countRow = db.query(`SELECT COUNT(*) as cnt FROM contacts ${whereClause}`).get(...params) as { cnt: number };

  const rows = db
    .query(`SELECT * FROM contacts ${whereClause} ORDER BY display_name ASC LIMIT ? OFFSET ?`)
...
export function deleteContact(db: Database, tenantId: string, contactId: string): boolean {
  const result = db.query('DELETE FROM contacts WHERE id = ? AND tenant_id = ?').run(contactId, tenantId);
...
function getLinksForContact(db: Database, tenantId: string, contactId: string): PlatformLink[] {
  const rows = db
    .query('SELECT * FROM contact_platform_links WHERE contact_id = ? AND tenant_id = ? ORDER BY platform ASC')
...
export function addPlatformLink(
  ...
  const row = db.query('SELECT * FROM contact_platform_links WHERE id = ?').get(id) as PlatformLinkRow;
  return rowToLink(row);
}

export function removePlatformLink(db: Database, tenantId: string, linkId: string): boolean {
  const result = db.query('DELETE FROM contact_platform_links WHERE id = ? AND tenant_id = ?').run(linkId, tenantId);
  return result.changes > 0;
}

export function verifyPlatformLink(db: Database, tenantId: string, linkId: string): boolean {
  const result = db
    .query('UPDATE contact_platform_links SET verified = 1 WHERE id = ? AND tenant_id = ?')
    .run(linkId, tenantId);
  return result.changes > 0;
}
...
export function findContactByPlatformId(
  ...
  const linkRow = db
    .query('SELECT contact_id FROM contact_platform_links WHERE tenant_id = ? AND platform = ? AND platform_id = ?')
    .get(tenantId, platform, platformId) as { contact_id: string } | null;
  if (!linkRow) return null;
  return getContact(db, tenantId, linkRow.contact_id);
}

export function findContactByName(db: Database, tenantId: string, name: string): Contact | null {
  const row = db
    .query('SELECT * FROM contacts WHERE tenant_id = ? AND display_name = ? COLLATE NOCASE')
