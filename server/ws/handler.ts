  ...
  return {
    open(ws: ServerWebSocket<WsData>) {
      ...
      ws.data = {
        ...
        walletAddress: ws.data?.walletAddress,
...
function handleClientMessage(
  ...
  switch (msg.type) {
    ...
    case 'chat_send': {
      ...
      log.debug(`chat_send received`, {
        agentId: msg.agentId,
        content: msg.content.slice(0, 50),
        wallet: walletCtx?.slice(0, 8),
      });
          ...
          if (result.sessionId) {
            const invokeCallback = (_sessionId: string, event: { type: string }) => {
              if (event.type === 'result' || event.type === 'session_exited') {
                ...
                import('../db/agent-messages')
                  .then(({ getAgentMessage }) => {
                    const updated = getAgentMessage(messenger.db, result.message.id);
                    if (updated) {
                  ...
                  .catch((err) => {
                    log.warn('Failed to re-fetch agent message', {
...
function sendError(
  ws: ServerWebSocket<WsData>,
  message: string,
  severity?: import('../../shared/ws-protocol').ErrorSeverity,
  errorCode?: string,
): void {
