...
export class PSKDiscoveryPoller {
  ...
  constructor(db: Database, config: AlgoChatConfig, service: AlgoChatService, contactManager: PSKContactManager) {
    this.db = db;
    this.config = config;
  ...
  private async poll(): Promise<void> {
    ...
    if (this.discoveryLastRound === 0) {
      try {
        ...
        log.info('Discovery poller starting', {
          fromRound: this.discoveryLastRound,
          unmatchedContacts: unmatched.length,
        });
      } catch (err) {
        log.error('Failed to get current round for discovery poller', {
    ...
    try {
        ...
        for (const tx of txns) {
          ...
          for (const contact of unmatched) {
            ...
            try {
              ...
              const pskBytes =
                contact.initial_psk instanceof Uint8Array
                  ? contact.initial_psk
              ...
              // Expected: wrong PSK for this contact — try next
              log.debug('PSK trial-decrypt failed', {
                contactId: contact.id,
                txid: tx.id?.slice(0, 12),
      ...
      log.info('Discovery poll complete', {
        totalTxns,
        pskCandidates,
        maxRound,
        prevRound: this.discoveryLastRound,
        discovered: discoveredSenders.size,
      });
    } catch (err) {
