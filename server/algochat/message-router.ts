...
import { generateChatBranchName, resolveAndCreateWorktree } from '../lib/worktree';
...
import type { LocalChatEventFn, LocalChatSendFn, SubscriptionManager } from './subscription-manager';
...
export class MessageRouter {
    ...
    return runWithEventContext(ctx, async () => {
      ...
      if (this.ownerQuestionManager) {
        ...
        if (questionResponse) {
          ...
          const resolved = this.ownerQuestionManager.resolveByShortId(questionResponse.shortId, {
            answer,
            selectedOption,
          });
          if (resolved) {
   ...
   */
  private async createSessionWorktree(
