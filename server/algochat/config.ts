...
export function loadAlgoChatConfig(): AlgoChatConfig {
  ...
  _cachedConfig = {
    ...
    network,
    agentNetwork,
    syncInterval: Number.isNaN(syncInterval) ? 30000 : syncInterval,
...
export function parseOwnerAddresses(network: AlgoChatNetwork): Set<string> {
  ...
  if (addresses.size > 0) {
    ...
    log.warn(
      `No ALGOCHAT_OWNER_ADDRESSES set on ${network} — privileged commands (/stop, /approve, /deny, /mode, /work, /agent, /council) are disabled`,
    );
  }
  return addresses;
}
