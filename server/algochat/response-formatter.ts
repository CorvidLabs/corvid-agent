...
import { getConversationByParticipant } from '../db/sessions';
...
export class ResponseFormatter {
  ...
  constructor(db: Database, _config: AlgoChatConfig, service: AlgoChatService) {
    this.db = db;
  ...
  async sendResponse(participant: string, content: string): Promise<void> {
    ...
    try {
      ...
      if (this.transactor) {
        ...
        const result = await this.transactor.sendToAddress(senderAccount, participant, content, sessionId);
      ...
      const result = await this.service.algorandService.sendMessage(senderAccount, participant, pubKey, sendContent);
   ...
   */
  emitEvent(participant: string, content: string, direction: 'inbound' | 'outbound' | 'status', fee?: number): void {
