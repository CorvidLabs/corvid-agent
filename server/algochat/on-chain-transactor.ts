...
export class OnChainTransactor {
  ...
  async sendMessage(opts: SendMessageOptions): Promise<SendMessageResult> {
    ...
    const sendContent = currentTraceId ? `[trace:${currentTraceId}]\n${content}` : content;
  ...
  async sendToSelf(agentId: string, content: string): Promise<string | null> {
    ...
    try {
      ...
      const result = await sendGroupMessage(this.service, account.account, account.address, pubKey, content);
  ...
  async sendToAddress(
    ...
    try {
      const { sendGroupMessage } = await import('./group-sender');
      const groupResult = await sendGroupMessage(this.service, senderAccount, recipientAddress, pubKey, content);
    ...
    const result = await this.service.algorandService.sendMessage(senderAccount, recipientAddress, pubKey, sendContent);
   ...
   */
  private async sendEncryptedMessage(opts: SendToAddressOptions, messageId?: string): Promise<SendMessageResult> {
