...
export class PSKContactManager {
  ...
  rotatePSKContact(id: string, gracePeriodMs: number = 5 * 60 * 1000): Uint8Array | null {
    ...
    this.db
      .prepare("UPDATE psk_contacts SET initial_psk = ?, updated_at = datetime('now') WHERE id = ?")
      .run(newPSK, id);

    // Update algochat_psk_state with new PSK
    const contact = this.db.prepare('SELECT mobile_address FROM psk_contacts WHERE id = ?').get(id) as {
      mobile_address: string | null;
    } | null;
  ...
  cancelPSKContact(id: string): boolean {
    ...
    if (contact) {
      const stateAddr = contact.mobile_address ?? id;
      this.db
        .prepare('DELETE FROM algochat_psk_state WHERE address = ? AND network = ?')
        .run(stateAddr, this.config.network);
    }
  ...
  getPSKContactURI(id: string): string | null {
    const row = this.db.prepare('SELECT initial_psk, nickname FROM psk_contacts WHERE id = ?').get(id) as {
    ...
    const pskBytes =
      row.initial_psk instanceof Uint8Array ? row.initial_psk : new Uint8Array(row.initial_psk as ArrayBuffer);
  ...
  promoteContact(
    ...
    this.db
      .prepare("UPDATE psk_contacts SET mobile_address = ?, updated_at = datetime('now') WHERE id = ?")
    ...
    this.db
      .prepare('UPDATE algochat_psk_state SET address = ? WHERE address = ? AND network = ?')
  ...
  private setupLegacyPskContact(): void {
    ...
    const existing = this.db
      .prepare('SELECT id FROM psk_contacts WHERE mobile_address = ? AND network = ? AND active = 1')
