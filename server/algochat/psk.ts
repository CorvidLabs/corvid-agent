...
export class PSKManager {
  ...
  constructor(
    ...
    if (restored) {
      ...
      const pskFp = Array.from(pskConfig.psk.slice(0, 8))
        .map((b: number) => b.toString(16).padStart(2, '0'))
        .join('');
      log.info(`Restored state for ${pskConfig.label ?? pskConfig.address.slice(0, 8)}... on ${network}`, {
        network,
        pskFp,
  ...
  rotatePSK(gracePeriodMs: number = PSK_ROTATION_GRACE_MS): Uint8Array {
    ...
    recordAudit(this.db, 'psk_rotation', 'owner', 'psk_contact', this.contactId, JSON.stringify({ gracePeriodMs }));
  ...
  async sendMessage(content: string): Promise<string> {
    ...
    const recipientPubKey =
      this.contactEncryptionKey ?? (await this.service.algorandService.discoverPublicKey(this.contact.address, 1000));
  ...
  private async poll(): Promise<void> {
    ...
    do {
      let query = indexer.searchForTransactions().address(contactAddress).addressRole('sender').limit(50);
        ...
        try {
          ...
          if (!algochat.validateCounter(this.contact.state, envelope.ratchetCounter)) {
            if (this.contact.state.seenCounters.has(envelope.ratchetCounter)) {
              ...
              log.info(`Counter ${envelope.ratchetCounter} reused from new txid (multi-device)`, {
              ...
              log.info(
                `Counter ${envelope.ratchetCounter} out of window (peerLast=${this.contact.state.peerLastCounter}), resyncing`,
                { drift },
              );
  ...
  private loadState(address: string): PSKContactEntry | null {
    const row = this.db
      .query('SELECT * FROM algochat_psk_state WHERE address = ? AND network = ?')
