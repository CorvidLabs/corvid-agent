...
import { getBalance, getCreditConfig, getTransactionHistory } from '../db/credits';
import { getSchedule, listExecutions, listSchedules, updateSchedule, updateScheduleNextRun } from '../db/schedules';
import { createSession, getConversationByParticipant, listConversations } from '../db/sessions';
...
export class CommandHandler {
  ...
  handleCommand(participant: string, content: string, responseFn?: (text: string) => void): boolean {
    ...
    switch (command) {
      ...
      case '/queue': {
        ...
        if (queued.length === 0) {
          ...
          const lines = queued.map(
            (q) => `#${q.id}: [${q.toolName}] session=${q.sessionId.slice(0, 8)} (${q.createdAt})`,
          );
      ...
      case '/approve': {
        ...
        respond(resolved ? `Escalation #${queueId} approved` : `Escalation #${queueId} not found or already resolved`);
        return true;
      }
      ...
      case '/deny': {
        ...
        respond(resolved ? `Escalation #${queueId} denied` : `Escalation #${queueId} not found or already resolved`);
        return true;
      }
      ...
      case '/work': {
        ...
        this.workCommandRouter.handleSlashCommand(participant, description, respond, () =>
  ...
  private handleMessageCommand(participant: string, parts: string[], respond: (text: string) => void): void {
    ...
    if (agentName) {
      ...
      agent = agents.find(
        (a) =>
          a.name.toLowerCase() === target || a.name.toLowerCase().replace(/\s+/g, '') === target.replace(/\s+/g, ''),
      );
      if (!agent) {
        const names = agents.map((a) => a.name).join(', ');
  ...
  private handleScheduleCommand(parts: string[], respond: (text: string) => void): void {
    ...
    switch (sub) {
      ...
      case 'pause': {
        if (!scheduleId) {
          respond('Usage: /schedule pause <schedule-id>');
          return;
        }
        const updated = updateSchedule(this.db, scheduleId, { status: 'paused' });
        if (!updated) {
          respond(`Schedule ${scheduleId} not found`);
          return;
        }
      ...
      case 'resume': {
        if (!scheduleId) {
          respond('Usage: /schedule resume <schedule-id>');
          return;
        }
      ...
      case 'history': {
        if (!scheduleId) {
          respond('Usage: /schedule history <schedule-id>');
          return;
        }
      ...
      case 'run': {
        if (!scheduleId) {
          respond('Usage: /schedule run <schedule-id>');
          return;
        }
        ...
        if (!this.schedulerServiceRef) {
          respond('Scheduler service not available');
          return;
        }
  ...
  private async handleCouncilCommand(
    ...
    if (councilName) {
      ...
      if (agents.length < 2) {
        send(
          `Only 1 agent available (${agents[0].name}). Council requires at least 2 agents.\nEnable more agents for AlgoChat, or create a council in the dashboard.`,
        );
        return;
      }
    ...
    try {
      const result = launchCouncil(this.db, this.processManager, councilId, projectId, prompt, this.agentMessengerRef);

      send(
        `Council launched! (${result.sessionIds.length} agents responding)\nLaunch ID: ${result.launchId.slice(0, 8)}...`,
      );
      ...
      const unsubscribe = onCouncilStageChange((launchId, stage) => {
        ...
        if (stage === 'discussing') {
          ...
          if (launch?.synthesis) {
            ...
            const synthesis =
              launch.synthesis.length > MAX_SYNTHESIS_LENGTH
