 ...
 */
function parseNetworkPreset(preset: {
  algodUrl: string;
  algodToken: string;
  indexerUrl?: string;
  indexerToken?: string;
...
async function fundFromLocalNetDispenser(algodClient: unknown, address: string): Promise<void> {
  ...
  const kmd = new algosdk.Kmd(kmdToken, `${parsed.protocol}//${parsed.hostname}`, parseInt(parsed.port || '4002', 10));
...
export async function initAlgoChatService(config: AlgoChatConfig): Promise<AlgoChatService | null> {
  ...
  try {
    ...
    const indexerClient = serviceConfig.indexerServer
      ? new algosdk.Indexer(serviceConfig.indexerToken, serviceConfig.indexerServer, serviceConfig.indexerPort ?? '')
      : null;
    ...
    if (config.mnemonic) {
      ...
      log.info('Generated new account — set ALGOCHAT_MNEMONIC env var to persist across restarts', {
    ...
    try {
      ...
      log.debug('Failed to publish key (may already exist)', {
