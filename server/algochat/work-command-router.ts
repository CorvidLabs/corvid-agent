...
import { createAgentMessage, getAgentMessage, updateAgentMessageStatus } from '../db/agent-messages';
...
export class WorkCommandRouter {
  ...
  handleSlashCommand(
    ...
    if (projectMatch) {
      ...
      if (!project) {
        const available = listProjects(this.db)
          .map((p) => p.name)
          .join(', ');
  ...
  async handleAgentWorkRequest(params: AgentWorkRequestParams): Promise<AgentWorkRequestResult> {
    const { fromAgentId, fromAgentName, toAgentId, content, paymentMicro, threadId, projectId, emitMessageUpdate } =
      params;
