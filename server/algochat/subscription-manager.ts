 ...
 */
export type LocalChatSendFn = (participant: string, content: string, direction: 'inbound' | 'outbound') => void;
...
export class SubscriptionManager {
  ...
  constructor(processManager: ProcessManager, responseFormatter: ResponseFormatter) {
  ...
  subscribeForResponse(sessionId: string, participant: string): void {
      ...
      this.setSubscriptionTimer(sessionId, () => {
        ...
        if (this.processManager.isRunning(sessionId)) {
          ...
          if (timeoutExtensions < MAX_TIMEOUT_EXTENSIONS) {
            log.info(`Subscription timeout extended — session still running`, {
              sessionId,
              extension: timeoutExtensions,
            });
            const msg = generateProgressSummary();
            this.responseFormatter.sendResponse(participant, `[Status] ${msg}`).catch((err) => {
              log.debug('Failed to send progress update on timeout extension', {
    ...
    const agentsQueried: Set<string> = new Set();
      ...
      if (recentActions.length > 0) {
        ...
        if (textActions.length > 0) {
          ...
          if (lastText.details && lastText.details.length > 0) {
            const preview = lastText.details.length > 60 ? `${lastText.details.slice(0, 60)}...` : lastText.details;
    ...
    const sendAckNow = () => {
      ...
      this.responseFormatter
        .sendResponse(participant, '[Status] Received your message — working on it now.')
        .catch((err) => {
    ...
    const flushTextBlock = () => {
      ...
      if (text.length > 0) {
        ...
        if (text.length > 50) {
          // Only track substantial text blocks
          trackProgress({
            type: 'text_block',
            action: 'reasoning',
            timestamp: Date.now(),
