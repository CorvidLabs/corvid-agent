...
import type { FailureReason, OutcomeStats, PrOutcome } from '../db/pr-outcomes';
import {
  createPrOutcome,
...
export class OutcomeTrackerService {
  ...
  private inferFailureReason(pr: { statusCheckRollup: string | null; reviewDecision: string | null }): FailureReason {
    if (pr.statusCheckRollup?.includes('FAILURE')) {
      return 'ci_fail';
    }
  ...
  private generateInsights(
    ...
    insights.push(
      `PR merge rate: ${(overall.mergeRate * 100).toFixed(0)}% (${overall.merged}/${overall.merged + overall.closed})`,
    );

    if (workTaskStats.total > 0) {
      insights.push(
        `Work task success rate: ${(workTaskStats.successRate * 100).toFixed(0)}% (${workTaskStats.completed}/${workTaskStats.total})`,
      );
    }
    ...
    for (const [repo, stats] of Object.entries(byRepo)) {
      if (stats.total >= 3 && stats.mergeRate < 0.3) {
        insights.push(
          `Low success repo: ${repo} (${(stats.mergeRate * 100).toFixed(0)}% merge rate) — consider reducing contributions.`,
        );
      }
    }
  ...
  private formatAnalysisForMemory(analysis: WeeklyAnalysis): string {
    const lines: string[] = [];
    lines.push(
      `Weekly outcome analysis for ${analysis.period.since.split('T')[0]} to ${analysis.period.until.split('T')[0]}`,
    );
    lines.push(
      `PR merge rate: ${(analysis.overall.mergeRate * 100).toFixed(0)}% (${analysis.overall.merged} merged, ${analysis.overall.closed} closed, ${analysis.overall.open} open)`,
    );
    lines.push(
      `Work tasks: ${analysis.workTaskStats.completed}/${analysis.workTaskStats.total} succeeded (${(analysis.workTaskStats.successRate * 100).toFixed(0)}%)`,
    );
