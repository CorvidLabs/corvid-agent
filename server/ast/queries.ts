...
import type { AstLanguage, AstSymbol, AstSymbolKind } from './types';
...
function extractExport(node: Node, isTS: boolean): AstSymbol | AstSymbol[] | null {
  ...
  // export default ...
  const defaultDecl = findChildByType(node, 'function_declaration') ?? findChildByType(node, 'class_declaration');
  if (defaultDecl) {
