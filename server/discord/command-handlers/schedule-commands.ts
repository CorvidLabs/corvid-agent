...
import { respondEphemeral, respondToInteraction, respondToInteractionEmbed } from '../embeds';
...
export async function handleScheduleCommand(
  ...
  switch (subName) {
    ...
    case 'create':
      ...
      await handleScheduleCreate(ctx, interaction);
      break;
    case 'pause':
      if (permLevel < PermissionLevel.ADMIN) {
      ...
      await handleSchedulePause(ctx, interaction);
      break;
    case 'resume':
      if (permLevel < PermissionLevel.ADMIN) {
      ...
      await handleScheduleResume(ctx, interaction);
      break;
    case 'delete':
      if (permLevel < PermissionLevel.ADMIN) {
...
async function handleScheduleList(ctx: InteractionContext, interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  const lines = schedules.slice(0, 15).map((s) => {
    ...
    const lastRun = s.lastRunAt ? `<t:${Math.floor(new Date(s.lastRunAt).getTime() / 1000)}:R>` : 'never';
...
async function handleScheduleCreate(ctx: InteractionContext, interaction: ChatInputCommandInteraction): Promise<void> {
  const name = interaction.options.getString('name');
  const agentName = interaction.options.getString('agent');
  const cron = interaction.options.getString('cron');
  const actionType = interaction.options.getString('action_type') ?? 'discord_post';
...
async function handleSchedulePause(ctx: InteractionContext, interaction: ChatInputCommandInteraction): Promise<void> {
  const scheduleId = interaction.options.getString('schedule') ?? undefined;
...
async function handleScheduleResume(ctx: InteractionContext, interaction: ChatInputCommandInteraction): Promise<void> {
  const scheduleId = interaction.options.getString('schedule') ?? undefined;
  const schedule = resolveSchedule(ctx, scheduleId);
...
async function handleScheduleDelete(ctx: InteractionContext, interaction: ChatInputCommandInteraction): Promise<void> {
  const scheduleId = interaction.options.getString('schedule') ?? undefined;
  const schedule = resolveSchedule(ctx, scheduleId);
...
async function handleScheduleTemplates(interaction: ChatInputCommandInteraction): Promise<void> {
