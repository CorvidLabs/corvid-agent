...
export class DiscordRestClient {
  ...
  async respondToInteraction(
    ...
    try {
      const result = await this.rest.post(Routes.interactionCallback(interactionId, interactionToken), { body: data });
   ...
   */
  async deferInteraction(interactionId: string, interactionToken: string, ephemeral = false): Promise<void> {
    try {
  ...
  async editDeferredResponse(
    ...
    try {
      const result = await this.rest.patch(Routes.webhookMessage(applicationId, interactionToken, '@original'), {
        body: data,
      });
   ...
   */
  async sendMessage(channelId: string, data: Record<string, unknown>): Promise<APIMessage> {
    try {
   ...
   */
  async editMessage(channelId: string, messageId: string, data: Record<string, unknown>): Promise<APIMessage> {
    try {
      const result = await this.rest.patch(Routes.channelMessage(channelId, messageId), { body: data });
      return result as APIMessage;
    } catch (error) {
  ...
  /**
   * Register (bulk-overwrite) slash commands for an application.
   * Registers guild-scoped commands when guildId is provided, otherwise global commands.
   * Also clears stale global commands when registering guild-scoped ones.
   */
  async putCommands(
    appId: string,
    ...
    try {
      ...
      const registered = (await this.rest.put(route, { body: commands })) as { name: string }[];

      // When using guild commands, clear any stale global commands so they don't shadow guild ones
      if (guildId) {
        try {
          await this.rest.put(Routes.applicationCommands(appId), { body: [] });
