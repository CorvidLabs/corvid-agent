...
import type { DiscordActionRow, DiscordInteractionData } from './types';
import { ButtonStyle, ComponentType, InteractionCallbackType } from './types';
 ...
 */
async function sendPlainMessage(botToken: string, channelId: string, content: string): Promise<void> {
  try {
    const response = await discordFetch(`https://discord.com/api/v10/channels/${channelId}/messages`, {
      method: 'POST',
      headers: {
...
export async function sendTypingIndicator(botToken: string, channelId: string): Promise<void> {
  try {
    const response = await discordFetch(`https://discord.com/api/v10/channels/${channelId}/typing`, {
      method: 'POST',
      headers: {
        Authorization: `Bot ${botToken}`,
      },
 ...
 */
function buildMultipartBody(payload: Record<string, unknown>, files: DiscordFileAttachment[]): FormData {
  const form = new FormData();
  form.append('payload_json', JSON.stringify(payload));
  for (let i = 0; i < files.length; i++) {
    const f = files[i];
    const blob = new Blob([new Uint8Array(f.data) as unknown as ArrayBuffer], {
