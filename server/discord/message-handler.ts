...
async function resumeExpiredThreadSession(
    ...
    ctx.processManager.startProcess(newSession, typeof contextualContent === 'string' ? contextualContent : withAuthorContext(text, authorUsername));
...
async function routeToThread(ctx: MessageHandlerContext, threadId: string, _userId: string, text: string, authorUsername?: string, attachments?: DiscordAttachment[]): Promise<void> {
    ...
    if (!session) {
        ...
        const resumed = await resumeExpiredThreadSession(ctx, threadId, threadInfo, text, authorUsername, attachments);
        if (!resumed) {
