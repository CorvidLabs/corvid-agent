...
import { broadcastAgentError, broadcastGovernanceQuorumReached, broadcastGovernanceVoteResolved } from './discussion';
...
export type EmitLogFn = (
  db: Database,
  launchId: string,
  level: import('../../shared/types').CouncilLogLevel,
  message: string,
  detail?: string,
) => void;
 ...
 */
export function aggregateSessionResponses(
  db: Database,
  allSessions: ReturnType<typeof listSessionsByCouncilLaunch>,
): string[] {
...
export function triggerSynthesis(
  ...
  const labels = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
  const memberResponses = memberSessions
    .map((s, i) => {
  ...
  const reviewSummaries = reviewSessions
    .map((s) => {
  ...
  if (launch.voteType === 'governance' && reputationScorer) {
    ...
    if (govVote) {
      ...
      if (memberVoteRows.length > 0) {
        const voteLines = memberVoteRows
          .map((mv) => {
    ...
    if (event.type === 'session_exited' || event.type === 'session_stopped') {
      ...
      if (lastMsg) {
        ...
        if (effectiveOnChainMode === 'attestation' && agentMessenger && chairmanAgentId) {
          publishSynthesisAttestation(agentMessenger, chairmanAgentId, launchId, lastMsg.content, db).catch((err) => {
...
function resolveGovernanceVote(
  ...
  if (check.passed) {
    ...
    emitLog(
      ...
      launchId,
      'info',
      'Governance vote passed — awaiting human approval',
