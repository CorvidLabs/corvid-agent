 ...
 *
 * Contains council lifecycle orchestration: launch, discussion rounds,
 * abort, follow-up chat, auto-advance, and session waiting utilities.
 * Synthesis/review logic lives in councils/synthesis.ts.
...
export type { BroadcastStageChangeFn, EmitLogFn, WatchAutoAdvanceFn } from './synthesis';
...
export function onCouncilStageChange(cb: StageChangeCallback): () => void {
  ...
  return () => {
    stageChangeListeners.delete(cb);
  };
}
...
export function onCouncilDiscussionMessage(cb: DiscussionMessageCallback): () => void {
  ...
  return () => {
    discussionMessageListeners.delete(cb);
  };
}
...
export function onGovernanceVoteCast(cb: GovernanceVoteCastCallback): () => void {
  ...
  return () => {
    governanceVoteCastListeners.delete(cb);
  };
}
...
export function onGovernanceVoteResolved(cb: GovernanceVoteResolvedCallback): () => void {
  ...
  return () => {
    governanceVoteResolvedListeners.delete(cb);
  };
}
...
export function onGovernanceQuorumReached(cb: GovernanceQuorumReachedCallback): () => void {
  ...
  return () => {
    governanceQuorumReachedListeners.delete(cb);
  };
}
...
/**
 * Trigger peer review — delegates to synthesis module with injected infrastructure.
 */
export function triggerReview(
  ...
  return triggerReviewImpl(db, processManager, launchId, emitLog, broadcastStageChange, (d, pm, lid, sids, role) =>
    watchSessionsForAutoAdvance(d, pm, lid, sids, role),
  );
}

/**
 * Finish with aggregated synthesis — delegates to synthesis module with injected infrastructure.
 */
export function finishWithAggregatedSynthesis(db: Database, launchId: string): void {
  finishWithAggregatedSynthesisImpl(db, launchId, emitLog, broadcastStageChange);
}

/**
 * Trigger chairman synthesis — delegates to synthesis module with injected infrastructure.
 */
...
function triggerDiscussion(
  ...
  const memberSessions = listSessionsByCouncilLaunch(db, launchId).filter((s) => s.councilRole === 'member');
  ...
  emitLog(
    ...
    launchId,
    'stage',
    `Starting discussion stage`,
    `${discussionRounds} rounds, ${council.agentIds.length} agents`,
  );
...
async function runDiscussionRounds(
  ...
  log.info(
    `Discussion timeouts: ${Math.round(perRoundTimeout / 60000)}m/round, ${Math.round(totalTimeout / 60000)}m total (${mode}, ${totalRounds} rounds)`,
  );
  ...
  for (let round = 1; round <= totalRounds; round++) {
    ...
    if (Date.now() - discussionStartTime > totalTimeout) {
      emitLog(
        ...
        launchId,
        'warn',
        `Discussion timed out after ${Math.round((Date.now() - discussionStartTime) / 60000)} minutes, skipping remaining rounds`,
      );
      break;
    }
    ...
    if (discusserSessionIds.length === 0) {
      ...
      if (waitResult.timedOut.length > 0) {
        for (const timedOutSessionId of waitResult.timedOut) {
          ...
          try {
            processManager.stopProcess(timedOutSessionId);
          } catch {
            /* already stopped */
          }
        }
    ...
    for (const agentId of council.agentIds) {
      ...
      if (agentMessenger && councilOnChainMode === 'full') {
        ...
        sendDiscussionOnChain(agentMessenger, agentId, council.agentIds, threadPrefix + content, discMsg.id, db).catch(
          (err) => {
...
export function buildDiscussionPrompt(
  ...
  const responsesText = memberResponses.map((r) => `### ${r.agentName} (${r.label})\n${r.content}`).join('\n\n---\n\n');
...
async function sendDiscussionOnChain(
  ...
  const sends = allAgentIds
    ...
    .map((toAgentId) => agentMessenger.sendOnChainBestEffort(fromAgentId, toAgentId, content));
...
function watchSessionsForAutoAdvance(
  ...
  const watcherTimer = setTimeout(() => {
    ...
    emitLog(
      ...
      launchId,
      'warn',
      `${role} watcher timed out with ${stuck.length} sessions still pending — force-advancing`,
    );
    for (const sid of stuck) {
      try {
        processManager.stopProcess(sid);
      } catch {
        /* already stopped */
      }
    }
