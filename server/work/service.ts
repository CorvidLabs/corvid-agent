...
export class WorkTaskService {
  ...
  async executeTask(
    ...
    if (governanceImpact) {
      ...
      if (governanceImpact.tier === 0) {
        updateWorkTaskStatus(this.db, task.id, 'failed', {
          error: `Blocked: task references Layer 0 (Constitutional) files that require human-only commits — ${governanceImpact.affectedPaths
            .filter((p) => p.tier === 0)
            .map((p) => p.path)
            .join(', ')}`,
        });
        recordAudit(
          ...
          'work_task',
          task.id,
          `Layer 0 governance block: ${governanceImpact.affectedPaths
            .filter((p) => p.tier === 0)
            .map((p) => p.path)
            .join(', ')}`,
        );
    ...
    const prompt = this.buildWorkPrompt(
      branchName,
      task.description,
      repoMap ?? undefined,
  ...
  private extractReferencedPaths(description: string): string[] {
    ...
    let match = pathPattern.exec(description);
    while (match !== null) {
      paths.add(match[1]);
      match = pathPattern.exec(description);
    }
    return [...paths];
  }
