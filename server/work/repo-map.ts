 ...
 */
export async function generateRepoMap(astParserService: AstParserService, projectDir: string): Promise<string | null> {
  try {
    ...
    for (const [filePath, fileIndex] of index.files.entries()) {
      ...
      const exported = fileIndex.symbols.filter((s: AstSymbol) => s.isExported && RELEVANT_KINDS.has(s.kind));
    ...
    for (const entry of fileEntries) {
      const dir = entry.relPath.includes('/') ? entry.relPath.slice(0, entry.relPath.lastIndexOf('/')) : '.';
      ...
      for (const { relPath, fileIndex } of entries) {
        ...
        const symbolList = exported.map((s: AstSymbol) => {
          ...
          if (children && children.length > 0) {
            const methods = children.map((c: AstSymbol) => `${c.name} [${c.startLine}-${c.endLine}]`).join(', ');
