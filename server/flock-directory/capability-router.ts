...
export class CapabilityRouter {
  ...
  constructor(flockService: FlockDirectoryService, conflictResolver?: FlockConflictResolver, selfAgentId?: string) {
  ...
  route(opts: {
    ...
    const required =
      opts.requiredCapabilities ?? (opts.actionType ? ACTION_CAPABILITY_MAP[opts.actionType] : null) ?? [];
    ...
    for (const agent of allAgents) {
      ...
      const score = Math.round(breakdown.reputation + breakdown.workload + breakdown.uptime + breakdown.recency);
