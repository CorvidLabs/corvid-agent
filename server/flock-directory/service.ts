...
import type { FlockAgentRecord, RegisterFlockAgentInput, UpdateFlockAgentInput } from './types';
...
export class FlockDirectoryService {
  ...
  async deregister(id: string): Promise<boolean> {
    ...
    // ── On-chain first ──────────────────────────────────────────────
    if (this.onChainClient && this.signerConfig) {
      await this.onChainClient.deregister(this.signerConfig.senderAddress, this.signerConfig.sk);
    } else {
  ...
  async heartbeat(id: string): Promise<boolean> {
    ...
    // ── On-chain first ──────────────────────────────────────────────
    if (this.onChainClient && this.signerConfig) {
      await this.onChainClient.heartbeat(this.signerConfig.senderAddress, this.signerConfig.sk);
    }
  ...
  update(id: string, input: UpdateFlockAgentInput): FlockAgent | null {
    ...
    if (input.reputationScore !== undefined) {
      ...
      params.push(input.reputationScore);
    }
    if (input.attestationCount !== undefined) {
      sets.push('attestation_count = ?');
      params.push(input.attestationCount);
    }
    if (input.councilParticipations !== undefined) {
      sets.push('council_participations = ?');
      params.push(input.councilParticipations);
    }
    if (input.uptimePct !== undefined) {
      sets.push('uptime_pct = ?');
      params.push(input.uptimePct);
    }
  ...
  recomputeAllReputations(): number {
    const rows = this.db.query(`SELECT id FROM flock_agents WHERE status != 'deregistered'`).all() as { id: string }[];
  ...
  async syncFromChain(address: string): Promise<OnChainAgentRecord | null> {
    ...
    try {
      ...
      if (agent) {
        const score = record.totalMaxScore > 0 ? Math.round((record.totalScore / record.totalMaxScore) * 100) : 0;
