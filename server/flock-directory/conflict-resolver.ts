...
export class FlockConflictResolver {
  ...
  constructor(db: Database, flockService: FlockDirectoryService, config?: Partial<ConflictResolverConfig>) {
    this.db = db;
   ...
   */
  checkAndClaim(opts: { repo: string; issueNumber?: number; branch?: string; description: string }): CheckClaimResult {
   ...
   */
  findConflicts(repo: string, issueNumber: number | null, branch: string | null): ClaimConflict[] {
    const conflicts: ClaimConflict[] = [];
    const now = new Date().toISOString();

    // Get all active claims on this repo
    const activeClaims = this.db
      .query(`
  ...
  listActiveClaims(repo?: string): WorkClaim[] {
    ...
    const rows = (repo ? this.db.query(query).all(repo) : this.db.query(query).all()) as WorkClaimRow[];
  ...
  getClaim(claimId: string): WorkClaim | null {
    const row = this.db.query(`SELECT * FROM work_claims WHERE id = ?`).get(claimId) as WorkClaimRow | null;
  ...
  getStats(): {
    ...
    const active = (
      this.db.query(`SELECT COUNT(*) as cnt FROM work_claims WHERE status = 'active'`).get() as { cnt: number }
    ).cnt;

    const total = (this.db.query(`SELECT COUNT(*) as cnt FROM work_claims`).get() as { cnt: number }).cnt;
    ...
    const overridden = (
      this.db.query(`SELECT COUNT(*) as cnt FROM work_claims WHERE status = 'superseded'`).get() as { cnt: number }
    ).cnt;
  ...
  private createClaim(opts: { repo: string; issueNumber?: number; branch?: string; description: string }): WorkClaim {
