...
import type { WebhookEventType, WebhookRegistration } from '../../shared/types';
import { getAgent } from '../db/agents';
import { isGitHubUserAllowed } from '../db/github-allowlist';
import { findSchedulesForEvent } from '../db/schedules';
import { createSession } from '../db/sessions';
import { createDelivery, findRegistrationsForRepo, incrementTriggerCount, updateDeliveryStatus } from '../db/webhooks';
...
type WebhookEventCallback = (event: { type: 'webhook_delivery'; data: unknown }) => void;
...
export class WebhookService {
  ...
  constructor(db: Database, processManager: ProcessManager, workTaskService?: WorkTaskService | null) {
    this.db = db;
  ...
  async processEvent(
    ...
    return runWithEventContext(ctx, async () => {
      ...
      for (const reg of registrations) {
        ...
        const delivery = createDelivery(this.db, reg.id, event, action, repo, sender, mentionBody, htmlUrl);
  ...
  private buildPrompt(
    ...
    if (event === 'issue_comment' && !payload.issue?.pull_request) {
      ...
      context = [
        ...
        `**Opened by:** @${sender}`,
        `**URL:** ${htmlUrl}`,
        payload.issue?.labels?.length ? `**Labels:** ${payload.issue.labels.map((l) => l.name).join(', ')}` : '',
        ``,
        `### Issue Body`,
        `\`\`\``,
        mentionBody,
