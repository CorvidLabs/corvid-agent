...
import type { ChannelSendResult, NotificationPayload } from './types';
...
export class NotificationService {
  ...
  private async dispatchToChannel(
    ...
    updateDeliveryStatus(this.db, deliveryId, result.success ? 'sent' : 'failed', result.error, result.externalRef);
  ...
  private async retryFailed(): Promise<void> {
    ...
    for (const delivery of failed) {
      ...
      await this.dispatchToChannel(delivery.channelType, delivery.channelConfig, payload, delivery.id);
    }
  }
}
