...
import { createQuestionDispatch, listChannelsForAgent } from '../db/notifications';
...
export class QuestionDispatcher {
  ...
  async dispatch(question: OwnerQuestion): Promise<string[]> {
    ...
    for (const channel of channels) {
      ...
      try {
        const result = await this.dispatchToChannel(channel.channelType, channel.config, question);

        if (result.success) {
          createQuestionDispatch(this.db, question.id, channel.channelType, result.externalRef ?? null);
