...
export class ResponsePollingService {
  ...
  private async pollGitHub(
    dispatches: Array<{ id: number; questionId: string; externalRef: string | null; createdAt: string }>,
  ): Promise<void> {
        ...
        if (resolved) {
          ...
          if (marked) {
            ...
            github.addIssueComment(repo, issueNumber, 'Answer received. Thank you!').catch((err) => {
              log.debug('Failed to add GitHub acknowledgment comment', {
                error: err instanceof Error ? err.message : String(err),
              });
            });
            github.closeIssue(repo, issueNumber).catch((err) => {
              log.debug('Failed to close GitHub issue after answer', {
                error: err instanceof Error ? err.message : String(err),
              });
            });
          }
  ...
  private async pollTelegram(
    dispatches: Array<{ id: number; questionId: string; externalRef: string | null }>,
  ): Promise<void> {
    ...
    for (const qId of questionIds) {
      ...
      const row = this.db.query(`SELECT agent_id FROM owner_questions WHERE id = ?`).get(qId) as {
        agent_id: string;
      } | null;
              ...
              if (optionStr !== 'other') {
                ...
                if (resolved) {
                  ...
                  fetch(`https://api.telegram.org/bot${botToken}/answerCallbackQuery`, {
                    ...
                    log.debug('Failed to answer Telegram callback query', {
                      error: err instanceof Error ? err.message : String(err),
                    });
                  });
                }
          ...
          if (dispatch) {
            ...
            if (resolved) {
              ...
              fetch(`https://api.telegram.org/bot${botToken}/sendMessage`, {
                ...
                log.debug('Failed to send Telegram confirmation message', {
                  error: err instanceof Error ? err.message : String(err),
                });
              });
            }
  ...
  private parseResponse(text: string, questionId: string): { answer: string; selectedOption: number | null } {
    const trimmed = text.trim();

    // Look up the question's options from the DB
    const row = this.db.query(`SELECT options FROM owner_questions WHERE id = ?`).get(questionId) as {
      options: string | null;
    } | null;
