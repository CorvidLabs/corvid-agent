 ...
 */
export async function synthesizeWithCache(db: Database, text: string, voice: VoicePreset): Promise<TTSResult> {
  const textHash = hashText(text);

  // Check cache
  const cached = db
    .query('SELECT audio_data, format, duration_ms FROM voice_cache WHERE text_hash = ? AND voice_preset = ?')
  ...
  if (cacheCount > MAX_VOICE_CACHE_ENTRIES) {
    ...
    db.query('DELETE FROM voice_cache WHERE id IN (SELECT id FROM voice_cache ORDER BY created_at ASC LIMIT ?)').run(
      excess,
    );
