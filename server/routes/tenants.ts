...
async function handleRegister(req: Request, db: Database, tenantService: TenantService | null): Promise<Response> {
  ...
  const tenantPlan =
    plan && validPlans.includes(plan as (typeof validPlans)[number]) ? (plan as (typeof validPlans)[number]) : 'free';
...
function handleGetCurrentTenant(context: RequestContext, tenantService: TenantService | null): Response {
  if (!tenantService) {
...
function handleListMembers(db: Database, context: RequestContext): Response {
  const rows = db
    .query('SELECT tenant_id, key_hash, role, created_at, updated_at FROM tenant_members WHERE tenant_id = ?')
...
async function handleAddMember(req: Request, db: Database, context: RequestContext): Promise<Response> {
  ...
  const memberRole = role && validRoles.includes(role as TenantRole) ? (role as TenantRole) : 'viewer';
...
function handleRemoveMember(req: Request, db: Database, context: RequestContext, keyHash: string): Response {
  const result = db
    .query('DELETE FROM tenant_members WHERE tenant_id = ? AND key_hash = ?')
