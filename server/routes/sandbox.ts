...
import { AssignSandboxSchema, parseBodyOrThrow, SetSandboxPolicySchema, ValidationError } from '../lib/validation';
...
async function handleAssign(req: Request, sandboxManager?: SandboxManager | null): Promise<Response> {
  if (!sandboxManager?.isEnabled()) {
  ...
  try {
    ...
    const containerId = await sandboxManager.assignContainer(data.agentId, data.sessionId, data.workDir);
...
async function handleRelease(sessionId: string, sandboxManager?: SandboxManager | null): Promise<Response> {
  if (!sandboxManager?.isEnabled()) {
    return json({ error: 'Sandboxing is not enabled' }, 503);
  }
