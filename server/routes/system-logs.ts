...
export function handleSystemLogRoutes(
  req: Request,
  url: URL,
  db: Database,
...
function handleLogs(
  ...
  if (type === 'all' || type === 'work-task') {
    ...
    for (const task of tasks) {
      const level = task.status === 'failed' ? 'error' : task.status === 'completed' ? 'info' : 'warn';
      logs.push({
        type: 'work-task',
        id: task.id,
  ...
  if (search) {
    ...
    filtered = filtered.filter(
      (log) => log.message.toLowerCase().includes(lowerSearch) || log.detail?.toLowerCase().includes(lowerSearch),
    );
  }
