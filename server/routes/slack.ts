...
async function handleSlackEvents(req: Request, db: Database, processManager: ProcessManager): Promise<Response> {
  ...
  if (contentType.includes('application/x-www-form-urlencoded')) {
    ...
    if (payload.type === 'block_actions' && payload.actions) {
      for (const action of payload.actions) {
        ...
        const dispatch = dispatches.find((d) => d.channelType === 'slack' && d.questionId.startsWith(shortId));

        if (dispatch) {
          // Look up options from the question
          const row = db.query('SELECT options FROM owner_questions WHERE id = ?').get(dispatch.questionId) as {
            options: string | null;
          } | null;
        ...
        if (resolved) {
          ...
          if (botToken && event.channel) {
            sendSlack(
              ...
              log.debug('Slack answer notification failed', {
