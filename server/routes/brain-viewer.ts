...
function handleSearchMemories(
  ...
  const countRow = safeQuery<CountRow>(db, `SELECT COUNT(*) as count FROM agent_memories m ${whereClause}`, bindings);
  total = countRow?.count ?? 0;
...
function handleStats(url: URL, db: Database): Response {
  ...
  // Total
  const totalRow = db
    .query(`SELECT COUNT(*) as count FROM agent_memories WHERE archived = 0${agentFilter}`)
    .get(...agentBinding) as CountRow;
  const totalMemories = totalRow.count;

  // By status
  const statusRows = db
    .query(`SELECT status, COUNT(*) as count FROM agent_memories WHERE archived = 0${agentFilter} GROUP BY status`)
  ...
  // Date range
  const oldestRow = db
    .query(`SELECT MIN(created_at) as val FROM agent_memories WHERE archived = 0${agentFilter}`)
    .get(...agentBinding) as { val: string | null };
  const newestRow = db
    .query(`SELECT MAX(created_at) as val FROM agent_memories WHERE archived = 0${agentFilter}`)
    .get(...agentBinding) as { val: string | null };

  // Average decay score (computed live)
  let averageDecayScore: number | null = null;
  const decayRows = db
    .query(`SELECT updated_at FROM agent_memories WHERE archived = 0${agentFilter}`)
    .all(...agentBinding) as Array<{ updated_at: string }>;
  if (decayRows.length > 0) {
    const now = new Date();
    const totalDecay = decayRows.reduce((sum, row) => sum + computeDecayMultiplier(row.updated_at, now), 0);
...
function handleMemoryDetail(id: string, db: Database): Response {
  const row = db.query('SELECT * FROM agent_memories WHERE id = ?').get(id) as MemoryRow | null;
...
function handleSyncStatus(url: URL, db: Database): Response {
  ...
  // Pending count
  const pendingRow = db
    .query(`SELECT COUNT(*) as count FROM agent_memories WHERE status = 'pending'${agentFilter}`)
    .get(...agentBinding) as CountRow;

  // Failed count
  const failedRow = db
    .query(`SELECT COUNT(*) as count FROM agent_memories WHERE status = 'failed'${agentFilter}`)
    .get(...agentBinding) as CountRow;
...
function handleObservationList(url: URL, db: Database): Response {
  ...
  if (!agentId) {
    ...
    const totalRow = db
      .query(`SELECT COUNT(*) as count FROM memory_observations ${whereClause}`)
...
function handleObservationStats(url: URL, db: Database): Response {
  ...
  // All agents
  const agentRows = db.query(`SELECT DISTINCT agent_id FROM memory_observations`).all() as { agent_id: string }[];
  ...
  // Totals
  const totalRow = db
    .query(`SELECT COUNT(*) as count FROM memory_observations WHERE status = 'active'`)
    .get() as CountRow;

  const graduationCandidateRow = db
    .query(
...
function enrichMemories(db: Database, rows: MemoryRow[]): Array<Record<string, unknown>> {
  ...
  try {
    ...
    const catRows = db
      .query(`SELECT memory_id, category, confidence FROM memory_categories WHERE memory_id IN (${placeholders})`)
