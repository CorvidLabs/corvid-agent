...
const PRIORITY_RULES: Record<SystemState, PriorityRule> = {
  ...
  ci_broken: {
    skip: ['feature_work'],
    boost: ['maintenance', 'review'],
    reason: 'CI is broken — suppressing feature work, prioritizing fixes and reviews',
  },
  server_degraded: {
    skip: ['feature_work', 'maintenance', 'review', 'communication'],
    boost: ['lightweight'],
    reason: 'Server degraded — only lightweight operations allowed',
  },
  p0_open: {
    skip: ['feature_work'],
    boost: ['maintenance', 'review'],
    reason: 'P0 issues open — suppressing feature work, prioritizing maintenance and reviews',
  },
  disk_pressure: {
    skip: ['feature_work'],
    boost: ['maintenance'],
    reason: 'Disk pressure — suppressing feature work and builds, prioritizing maintenance/cleanup',
  },
};
...
export function evaluateAction(actionType: ScheduleActionType, activeStates: SystemState[]): ActionGateResult {
  ...
  for (const state of activeStates) {
    ...
    if (!rule) continue;
    if (rule.skip.includes(category)) {
      shouldSkip = true;
      reasons.push(rule.reason);
    }
    if (rule.boost.includes(category)) {
      shouldBoost = true;
      reasons.push(rule.reason);
    }
  }
...
export function getRulesForState(state: SystemState): PriorityRule {
  return PRIORITY_RULES[state];
}
export function getAllRules(): Record<SystemState, PriorityRule> {
