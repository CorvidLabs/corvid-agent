...
import { type RunActionDeps, runAction } from './execution';
import type { HandlerContext } from './handlers/types';
import { handleApprovalIfNeeded, handleRepoLocking, shouldSkipByHealthGate } from './orchestration';
import { executePipeline } from './pipeline';
import { getAllRules } from './priority-rules';
import { type SystemStateConfig, SystemStateDetector, type SystemStateResult } from './system-state';
  ...
  if (intervalMs !== undefined && intervalMs !== null) {
    if (intervalMs < MIN_SCHEDULE_INTERVAL_MS) {
      throw new ValidationError(
        `Schedule interval too short: ${intervalMs}ms. Minimum is ${MIN_SCHEDULE_INTERVAL_MS}ms (5 minutes).`,
      );
    }
  }
...
export class SchedulerService {
  ...
  async stop(): Promise<void> {
    ...
    if (this.tickPromise) {
      await this.tickPromise.catch((err) =>
        log.debug('Pending tick error during shutdown', { error: err instanceof Error ? err.message : String(err) }),
      );
  ...
  getStats(): {
    ...
    return {
      ...
      recentFailures: queryCount(
        this.db,
        "SELECT COUNT(*) as cnt FROM schedule_executions WHERE status = 'failed' AND started_at >= datetime('now', '-24 hours')",
      ),
  ...
  async triggerNow(scheduleId: string): Promise<void> {
    ...
    if (schedule.status !== 'active')
      throw new ValidationError('Schedule is not active', { scheduleId, status: schedule.status });
  ...
  private resolveScheduleTenantId(agentId: string): string {
    const row = this.db.query('SELECT tenant_id FROM agents WHERE id = ?').get(agentId) as { tenant_id: string } | null;
    return row?.tenant_id ?? DEFAULT_TENANT_ID;
  }

  private initializeNextRuns(): void {
    const schedules = this.db
      .query(`SELECT * FROM agent_schedules WHERE status = 'active' AND next_run_at IS NULL`)
  ...
  private async executeSchedule(schedule: AgentSchedule): Promise<void> {
    ...
    return runWithEventContext(ctx, async () => {
      ...
      if (schedule.executionMode === 'pipeline' && schedule.pipelineSteps?.length) {
        ...
        executePipeline(
          this.buildRunActionDeps(),
          this.buildHandlerContext(),
          schedule,
          schedule.pipelineSteps,
          emitFn,
        );
        return;
      }
  ...
  private buildHandlerContext(): HandlerContext {
    return {
      ...
      agentMessenger: this.agentMessenger,
      improvementLoopService: this.improvementLoopService,
      reputationScorer: this.reputationScorer,
      reputationAttestation: this.reputationAttestation,
      outcomeTrackerService: this.outcomeTrackerService,
      dailyReviewService: this.dailyReviewService,
      systemStateDetector: this.systemStateDetector,
