...
export type SystemState = 'healthy' | 'ci_broken' | 'server_degraded' | 'p0_open' | 'disk_pressure';
...
export class SystemStateDetector {
  ...
  async evaluate(): Promise<SystemStateResult> {
    ...
    const [ciState, serverState, p0State, diskState] = await Promise.all([
      this.checkCI().catch((err) => {
        log.debug('CI check failed', { error: err instanceof Error ? err.message : String(err) });
        return null;
      }),
      this.checkServerHealth().catch((err) => {
        log.debug('Server health check failed', { error: err instanceof Error ? err.message : String(err) });
        return null;
      }),
      this.checkP0Issues().catch((err) => {
        log.debug('P0 issue check failed', { error: err instanceof Error ? err.message : String(err) });
        return null;
      }),
      this.checkDiskPressure().catch((err) => {
        log.debug('Disk check failed', { error: err instanceof Error ? err.message : String(err) });
        return null;
      }),
    ]);

    if (ciState) {
      states.push('ci_broken');
      details.ci_broken = ciState;
    }
    if (serverState) {
      states.push('server_degraded');
      details.server_degraded = serverState;
    }
    if (p0State) {
      states.push('p0_open');
      details.p0_open = p0State;
    }
    if (diskState) {
      states.push('disk_pressure');
      details.disk_pressure = diskState;
    }
  ...
  private async checkCI(): Promise<string | null> {
    ...
    const resp = await fetch(
      `https://api.github.com/repos/${owner}/${repo}/actions/runs?branch=main&per_page=1&status=completed`,
      {
        headers: {
  ...
  private async checkP0Issues(): Promise<string | null> {
    ...
    const resp = await fetch(
      `https://api.github.com/search/issues?q=repo:${owner}/${repo}+is:issue+is:open+${labelQuery}&per_page=5`,
      {
        headers: {
          Authorization: `token ${token}`,
          'User-Agent': 'corvid-agent',
