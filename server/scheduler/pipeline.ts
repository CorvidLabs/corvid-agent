...
import { createExecution, getExecution } from '../db/schedules';
...
export function shouldStepRun(condition: PipelineStepCondition, ctx: PipelineContext, stepIndex: number): boolean {
    ...
    steps: [
      {
        ...
        action: {
          type: 'custom',
          prompt:
            'Summarize the most recent release: list key changes from the latest tagged commits, group by category (features, fixes, improvements). Keep it concise — 5-10 bullet points max.',
