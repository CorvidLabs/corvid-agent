...
export async function execStarRepos(ctx: HandlerContext, executionId: string, action: ScheduleAction): Promise<void> {
...
export async function execForkRepos(ctx: HandlerContext, executionId: string, action: ScheduleAction): Promise<void> {
  if (!action.repos?.length) {
    updateExecutionStatus(ctx.db, executionId, 'failed', { result: 'No repos specified' });
    return;
  }
...
export async function execReviewPrs(
  ...
  for (const repo of action.repos) {
    ...
    const prompt =
      `You are reviewing open pull requests for ${repo}.\n\n` +
      `## Open PRs\n${prSummary}\n\n` +
      `## Instructions\n` +
...
export async function execGithubSuggest(
  ...
  const prompt =
    `You are analyzing the following repositories for potential improvements: ${repoList}\n\n` +
    `## Instructions\n` +
