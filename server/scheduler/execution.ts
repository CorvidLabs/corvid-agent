...
async function dispatchAction(
  ...
  switch (action.type) {
    case 'star_repo':
      await execStarRepos(hctx, executionId, action);
      break;
    case 'fork_repo':
      await execForkRepos(hctx, executionId, action);
      break;
    case 'review_prs':
      await execReviewPrs(hctx, executionId, schedule, action);
      break;
    case 'work_task':
      await execWorkTask(hctx, executionId, schedule, action);
      break;
    case 'council_launch':
      await execCouncilLaunch(hctx, executionId, schedule, action);
      break;
    case 'send_message':
      await execSendMessage(hctx, executionId, schedule, action);
      break;
    case 'github_suggest':
      await execGithubSuggest(hctx, executionId, schedule, action);
      break;
    case 'codebase_review':
      await execCodebaseReview(hctx, executionId, schedule, action);
      break;
    case 'dependency_audit':
      await execDependencyAudit(hctx, executionId, schedule, action);
      break;
    case 'improvement_loop':
      await execImprovementLoop(hctx, executionId, schedule, action);
      break;
    case 'memory_maintenance':
      await execMemoryMaintenance(hctx, executionId, schedule);
      break;
    case 'reputation_attestation':
      await execReputationAttestation(hctx, executionId, schedule);
      break;
    case 'outcome_analysis':
      await execOutcomeAnalysis(hctx, executionId, schedule);
      break;
    case 'daily_review':
      await execDailyReview(hctx, executionId, schedule);
      break;
    case 'status_checkin':
      await execStatusCheckin(hctx, executionId, schedule);
      break;
    case 'marketplace_billing':
      execMarketplaceBilling(hctx, executionId);
      break;
    case 'flock_testing':
      await execFlockTesting(hctx, executionId, schedule);
      break;
    case 'discord_post':
...
function trackConsecutiveFailures(deps: RunActionDeps, schedule: AgentSchedule, execution: ScheduleExecution): void {
...
function broadcastScheduleResult(
  ...
  agentMessenger
    .sendOnChainToSelf(agentId, msg)
    .catch((err) =>
      log.debug('AlgoChat schedule notification failed', { error: err instanceof Error ? err.message : String(err) }),
    );
}
...
function notifyScheduleEvent(
  ...
  deps.agentMessenger
    .sendNotificationToAddress(schedule.agentId, schedule.notifyAddress, `[schedule:${event}] ${message}`)
    .catch((err) => {
...
export function shouldDeliver(dest: ScheduleOutputDestination, status: string): boolean {
  if (dest.format === 'on_error_only') return status === 'failed';
  return true;
}

/** @internal exported for testing */
export function formatResult(dest: ScheduleOutputDestination, result: string): string {
    ...
    const promise = (async () => {
      switch (dest.type) {
        case 'discord_channel':
          await sendToDiscordChannel(dest.target, schedule.name, execution.actionType, status, formatted);
          break;
        case 'algochat_agent':
