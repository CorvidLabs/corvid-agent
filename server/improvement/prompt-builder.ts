...
import type { HealthMetrics, LargeFile, OutdatedDep, TscError } from './health-collector';
...
function trustLevelDescription(level: TrustLevel): string {
  switch (level) {
    case 'blacklisted':
      return 'BLACKLISTED — all actions blocked';
    case 'untrusted':
      return 'UNTRUSTED — actions severely restricted';
    case 'low':
      return 'LOW — limited to simple, safe fixes only';
    case 'medium':
      return 'MEDIUM — moderate complexity tasks allowed';
    case 'high':
      return 'HIGH — complex improvements allowed';
    case 'verified':
      return 'VERIFIED — full autonomy granted';
  }
}
...
export function buildImprovementPrompt(
  ...
  sections.push(
    `## Instructions\n\n` +
      ...
      `7. Focus on changes that will pass validation (\`fledge lanes run verify\`)\n` +
