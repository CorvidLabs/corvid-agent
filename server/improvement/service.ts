...
import { computeTrends, formatTrendsForPrompt, getRecentSnapshots, saveHealthSnapshot } from './health-store';
...
export class AutonomousLoopService {
  ...
  async run(agentId: string, projectId: string, options: ImprovementLoopOptions = {}): Promise<ImprovementRunResult> {
  ...
  private saveLearnings(agentId: string, sessionId: string, sessionCreatedAt: string): void {
    try {
      ...
      this.memoryManager.save({
        ...
        content:
          `Improvement loop session ${sessionId} completed at ${timestamp}.\n` +
  ...
  private handleTaskCompletion(
    agentId: string,
    task: { id: string; status: string; prUrl: string | null; error: string | null; description?: string },
  ): void {
    try {
