...
export class DailyReviewService {
  ...
  private generateObservations(executions: ExecutionStats, prs: DailyPrStats, health: HealthDelta): string[] {
    ...
    // All green
    if (observations.length === 0 && executions.total > 0) {
  ...
  private formatSummary(
    ...
    const lines: string[] = [`Daily Review — ${date}`, ''];
