...
export function parseTestOutput(
  output: string,
  exitCode: number,
): { passed: boolean; summary: string; failureCount: number } {
...
export function parseTodoOutput(output: string): {
  todoCount: number;
  fixmeCount: number;
  hackCount: number;
  samples: string[];
...
export class CodebaseHealthCollector {
  ...
  private async runTsc(cwd: string): Promise<{ errors: TscError[]; passed: boolean }> {
    const { stdout, exitCode } = await spawnAndCapture(['bun', 'x', 'tsc', '--noEmit', '--skipLibCheck'], cwd);
  ...
  private async runTests(cwd: string): Promise<{ passed: boolean; summary: string; failureCount: number }> {
    const { stdout, exitCode } = await spawnAndCapture(['bun', 'test'], cwd, TEST_TIMEOUT_MS);
    return parseTestOutput(stdout, exitCode);
  }

  private async countTodos(
    cwd: string,
  ): Promise<{ todoCount: number; fixmeCount: number; hackCount: number; samples: string[] }> {
  ...
  private async findLargeFiles(cwd: string): Promise<LargeFile[]> {
    ...
    const { stdout } = await spawnAndCapture(['wc', '-l', ...tsFiles], cwd);
    return parseLargeFiles(stdout);
  }

  private async checkOutdated(cwd: string): Promise<OutdatedDep[]> {
    const { stdout } = await spawnAndCapture(['bun', 'outdated'], cwd);
