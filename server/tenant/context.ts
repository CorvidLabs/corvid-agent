...
import type { Tenant, TenantContext, TenantPlan, TenantRecord } from './types';
...
export class TenantService {
   ...
   * Create a new tenant.
   */
  createTenant(input: { name: string; slug: string; ownerEmail: string; plan?: TenantPlan }): Tenant {
  ...
  getTenant(id: string): Tenant | null {
    const row = this.db.query('SELECT * FROM tenants WHERE id = ?').get(id) as TenantRecord | null;
  ...
  getTenantBySlug(slug: string): Tenant | null {
    const row = this.db.query('SELECT * FROM tenants WHERE slug = ?').get(slug) as TenantRecord | null;
    return row ? recordToTenant(row) : null;
  }
   ...
   */
  listTenants(): Tenant[] {
    const rows = this.db.query('SELECT * FROM tenants ORDER BY created_at DESC').all() as TenantRecord[];
  ...
  updatePlan(tenantId: string, plan: TenantPlan): Tenant | null {
    ...
    this.db
      .query(`
            ...
            WHERE id = ?
        `)
      .run(plan, limits.maxAgents, limits.maxConcurrentSessions, limits.sandboxEnabled ? 1 : 0, tenantId);
  ...
  setStripeCustomerId(tenantId: string, customerId: string): void {
    this.db.query('UPDATE tenants SET stripe_customer_id = ? WHERE id = ?').run(customerId, tenantId);
  }
  ...
  suspendTenant(tenantId: string): void {
    this.db.query("UPDATE tenants SET status = 'suspended', updated_at = datetime('now') WHERE id = ?").run(tenantId);
  ...
  canStartSession(tenantId: string): boolean {
    ...
    return (
      queryCount(
        this.db,
        "SELECT COUNT(*) as cnt FROM sessions WHERE tenant_id = ? AND status IN ('running', 'idle')",
        tenantId,
