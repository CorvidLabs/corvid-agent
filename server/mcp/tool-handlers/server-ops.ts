...
export async function handleRestartServer(ctx: McpToolContext, args: { reason?: string }): Promise<CallToolResult> {
  ...
  db.query(`UPDATE sessions SET server_restart_initiated_at = datetime('now') WHERE id = ?`).run(sessionId);
