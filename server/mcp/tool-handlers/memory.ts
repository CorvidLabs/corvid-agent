 ...
 */
async function searchOnChainFallback(ctx: McpToolContext, search: string): Promise<OnChainMemory[] | null> {
  try {
    const memories = await ctx.agentMessenger.readOnChainMemories(ctx.agentId, ctx.serverMnemonic, ctx.network, {
      limit: 20,
      search,
    });
...
export async function handlePromoteMemory(ctx: McpToolContext, args: { key: string }): Promise<CallToolResult> {
  try {
    ...
    if (isLocalnet) {
      ...
      if (!arc69Ctx) {
        return errorResult('Cannot promote memory: ARC-69 context unavailable (missing indexer or chat account).');
      }
...
export async function handleRecallMemory(
  ...
  try {
      ...
      if (memories.length === 0) {
        ...
        if (onChainResults && onChainResults.length > 0) {
          const lines = onChainResults.map(
            (mem) => `[on-chain] [${mem.key}] ${mem.content}\n  (txid: ${mem.txid} | ${mem.timestamp})`,
          );
          return textResult(
    ...
    return textResult(
      `Your ${memories.length} most recent memor${memories.length === 1 ? 'y' : 'ies'}:\n\n${lines.join('\n')}`,
    );
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
...
export async function handleReadOnChainMemories(
  ...
  try {
    ...
    if (allMemories.length === 0) {
      const msg = args.search ? `No on-chain memories found matching "${args.search}".` : 'No on-chain memories found.';
