    ...
    switch (args.action) {
      case 'list': {
        ...
        const lines = workflows.map(
          (w) => `- ${w.name} [${w.id}] status=${w.status} nodes=${w.nodes.length} edges=${w.edges.length}`,
        );
      ...
      case 'get': {
        ...
        const edgeList = workflow.edges
          .map((e) => `  - ${e.sourceNodeId} → ${e.targetNodeId}${e.condition ? ` (${e.condition})` : ''}`)
          .join('\n');
      ...
      default:
        return errorResult(
          `Unknown action: ${args.action}. Use list, create, get, activate, pause, trigger, runs, or run_status.`,
        );
    }
  } catch (err) {
