...
export async function handleSendMessage(
  ...
  try {
    ...
    const match = available.find(
      (a) => a.agentId === args.to_agent || a.agentName.toLowerCase() === args.to_agent.toLowerCase(),
    );
...
export async function handleListAgents(ctx: McpToolContext): Promise<CallToolResult> {
  try {
    const available = await ctx.agentDirectory.listAvailable();
    ...
    const lines = others.map((a) => {
      ...
      if (flockSvc && a.walletAddress) {
        ...
        if (flockAgent) {
          const caps = flockAgent.capabilities.length > 0 ? ` [${flockAgent.capabilities.join(', ')}]` : '';
          const rep = flockAgent.reputationScore > 0 ? ` rep: ${flockAgent.reputationScore}` : '';
          extra = `${caps}${rep}`;
        }
      }
