    ...
    switch (args.action) {
      ...
      case 'get': {
        ...
        const details = [
          ...
          schedule.outputDestinations
            ? `  Output destinations: ${JSON.stringify(schedule.outputDestinations, null, 2)}`
            : null,
      ...
      case 'update': {
        ...
        if (args.agent_id !== undefined) {
          updateInput.agentId = args.agent_id;
        ...
        if (args.cron_expression !== undefined) {
          updateInput.cronExpression = args.cron_expression;
          changedFields.push('cron_expression');
        }
        ...
        if (changedFields.length === 0) {
          return errorResult(
            'No fields to update. Provide at least one of: agent_id, name, description, cron_expression, interval_minutes, schedule_actions, approval_policy, max_executions',
          );
        }
