...
import { computeTrends, formatTrendsForPrompt, getRecentSnapshots } from '../../improvement/health-store';
...
export async function handleCheckReputation(ctx: McpToolContext, args: { agent_id?: string }): Promise<CallToolResult> {
...
export async function handlePublishAttestation(
  ...
  try {
    ...
    try {
      txid = await ctx.reputationAttestation.publishOnChain(targetId, hash, async (note: string) => {
