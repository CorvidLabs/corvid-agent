...
import { type DiscordFileAttachment, sendDiscordMessage, sendMessageWithFiles } from '../../discord/embeds';
...
export async function handleDiscordSendImage(
  ...
  try {
    ...
    const delivery = getDeliveryTracker();
    const messageId = await sendMessageWithFiles(delivery, botToken, args.channel_id, args.message ?? '', [attachment]);
