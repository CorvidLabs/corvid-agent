...
export async function handleFlockDirectory(
  ...
  try {
    switch (args.action) {
      ...
      case 'lookup': {
        const agent = args.agent_id ? svc.getById(args.agent_id) : args.address ? svc.getByAddress(args.address) : null;
      ...
      case 'list': {
        ...
        const lines = [`${agents.length} active agent(s):`, ...agents.map((a) => formatAgentEntry(a))];
        return textResult(lines.join('\n'));
      }
      ...
      case 'stats': {
        ...
        return textResult(
          `Flock Directory: ${stats.total} registered, ${stats.active} active, ${stats.inactive} inactive${onChainInfo}`,
        );
      }
      ...
      case 'health_overview': {
        ...
        const lines = [`Flock Health Overview — ${allAgents.total} agent(s):`, ''];
        for (const a of allAgents.agents) {
          const age = heartbeatAge(a.lastHeartbeat);
          const caps = a.capabilities.length > 0 ? ` [${a.capabilities.join(', ')}]` : '';
          const uptime = a.uptimePct !== undefined && a.uptimePct !== null ? ` uptime: ${a.uptimePct.toFixed(1)}%` : '';
