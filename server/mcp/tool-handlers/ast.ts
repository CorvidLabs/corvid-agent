...
export async function handleCodeSymbols(
  ...
  if (!projectDir) {
    return errorResult(
      'Could not resolve project directory. Provide project_dir or ensure the agent has a default project.',
    );
  }
  ...
  try {
    ...
    const validKinds = args.kinds?.filter((k): k is AstSymbolKind =>
      ['function', 'class', 'interface', 'type_alias', 'enum', 'import', 'export', 'variable', 'method'].includes(k),
    );
