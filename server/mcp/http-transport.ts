...
function createMcpServer(baseUrl: string, agentId: string): McpServer {
  const server = new McpServer({ name: 'corvid-agent', version: '1.0.0' }, { capabilities: { tools: {} } });
  ...
  server.tool(
    'corvid_save_memory',
    'Save a memory to short-term local storage (SQLite). Use corvid_promote_memory to promote to long-term on-chain storage.',
    {
  ...
  server.tool(
    ...
    'Recall memories. Provide key for exact lookup, query for search, or neither for recent.',
    {
  ...
  server.tool(
    ...
    'Read memories from on-chain storage (Algorand blockchain).',
    {
  ...
  server.tool(
    ...
    'Sync on-chain memories back to local SQLite cache.',
    {
  ...
  server.tool(
    ...
    'Record a short-term observation. High-value observations auto-graduate to long-term ARC-69 memory.',
    {
  ...
  server.tool(
    ...
    'List or search memory observations.',
    {
  ...
  server.tool(
    ...
    "Boost an observation's relevance score (makes graduation more likely).",
    {
  ...
  server.tool(
    ...
    'Dismiss an observation — marks it as not worth keeping.',
    {
      id: z.string().describe('Observation ID'),
    },
    async ({ id }) => {
  ...
  server.tool(
    ...
    'Get statistics about memory observations and graduation candidates.',
    {},
    async () => {
      try {
  ...
  server.tool(
    ...
    'Create a work task that spawns a new agent session on a dedicated branch.',
    {
      ...
      agent_id: z
        .string()
        .optional()
        .describe('Agent ID to execute and be credited for this task. Defaults to the calling agent.'),
    },
  ...
  server.tool(
    ...
    'Get details about a specific work task.',
    {
