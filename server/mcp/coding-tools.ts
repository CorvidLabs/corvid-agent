...
import { isProtectedBashCommand, isProtectedPath } from '../process/protected-paths';
    ...
    // ── edit_file ───────────────────────────────────────────────
    {
      ...
      handler: async (args) => {
        try {
          ...
          // Count occurrences
          let count = 0;
          let idx = content.indexOf(oldStr, 0);
          while (idx !== -1) {
            count++;
          ...
          if (count === 0) {
            return {
              text: `old_string not found in ${filePath}. Read the file first to see its current content.`,
              isError: true,
            };
          }
          if (count > 1) {
            return {
              text: `old_string appears ${count} times in ${filePath}. It must be unique — include more surrounding context.`,
              isError: true,
            };
          }
    ...
    // ── list_files ──────────────────────────────────────────────
    {
      ...
      parameters: {
        ...
        properties: {
          path: {
            type: 'string',
            description: 'Directory path or glob pattern relative to project directory. Defaults to "."',
          },
        },
      },
      handler: async (args) => {
