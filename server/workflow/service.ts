...
import type { WorkflowEdge, WorkflowNode, WorkflowNodeRun, WorkflowRun } from '../../shared/types';
...
export class WorkflowService {
  ...
  async triggerWorkflow(workflowId: string, input: Record<string, unknown> = {}): Promise<WorkflowRun> {
    ...
    return runWithEventContext(ctx, async () => {
      ...
      if (workflow.status !== 'active') {
        throw new ValidationError(`Workflow is not active (status: ${workflow.status}). Activate it first.`, {
          workflowId,
        });
      }
      ...
      const run = createWorkflowRun(this.db, workflowId, workflow.agentId, input, {
  ...
  getStats(): {
    ...
    const activeRuns = queryCount(
      this.db,
      "SELECT COUNT(*) as cnt FROM workflow_runs WHERE status IN ('running', 'paused')",
    );
  ...
  private executeCondition(node: WorkflowNode, input: Record<string, unknown>): Record<string, unknown> {
  ...
  private executeTransform(node: WorkflowNode, input: Record<string, unknown>): Record<string, unknown> {
  ...
  private completeRun(runId: string, _run: WorkflowRun, nodeRunMap: Map<string, WorkflowNodeRun>): void {
