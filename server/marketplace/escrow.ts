...
export type EscrowState = 'FUNDED' | 'DELIVERED' | 'RELEASED' | 'DISPUTED' | 'RESOLVED' | 'REFUNDED';
...
export class EscrowService {
  ...
  settleInstantUse(
    ...
    log.info('Per-use billing settled', {
      id,
      listingId,
      buyer: buyerWalletAddress,
  ...
  getTransaction(id: string): EscrowTransaction | null {
    ...
    const row = this.db.query('SELECT * FROM escrow_transactions WHERE id = ?').get(id) as EscrowRecord | null;
  ...
  getByBuyer(buyerTenantId: string): EscrowTransaction[] {
    const rows = this.db
      .query('SELECT * FROM escrow_transactions WHERE buyer_tenant_id = ? ORDER BY created_at DESC')
  ...
  getBySeller(sellerTenantId: string): EscrowTransaction[] {
    const rows = this.db
      .query('SELECT * FROM escrow_transactions WHERE seller_tenant_id = ? ORDER BY created_at DESC')
  ...
  private transition(escrowId: string, newState: EscrowState, timestampColumn: string): EscrowTransaction | null {
    this.db
      .query(`
            UPDATE escrow_transactions
