...
export class SubscriptionService {
    ...
    const created = this.db.transaction(() => {
      ...
      this.db
        .query(`
                ...
                VALUES (?, ?, ?, ?, ?, ?, 'active', ?, ?)
            `)
        .run(id, listingId, subscriberTenantId, sellerTenantId, priceCredits, billingCycle, now, periodEnd);
  ...
  processRenewals(): { renewed: number; pastDue: number; expired: number } {
    ...
    const expiredCancelled = this.db
      .query(`
            UPDATE marketplace_subscriptions
            SET status = 'expired'
  ...
  getSubscription(id: string): MarketplaceSubscription | null {
    const row = this.db
      .query('SELECT * FROM marketplace_subscriptions WHERE id = ?')
  ...
  getSubscribers(listingId: string): MarketplaceSubscription[] {
    const rows = this.db
      .query('SELECT * FROM marketplace_subscriptions WHERE listing_id = ? ORDER BY created_at DESC')
