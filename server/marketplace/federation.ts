...
export class MarketplaceFederation {
  ...
  removeInstance(url: string): boolean {
    ...
    this.db.query('DELETE FROM marketplace_listings WHERE instance_url = ?').run(normalizedUrl);
    const result = this.db.query('DELETE FROM federated_instances WHERE url = ?').run(normalizedUrl);
  ...
  getInstance(url: string): FederatedInstance | null {
    const row = this.db
      .query('SELECT * FROM federated_instances WHERE url = ?')
  ...
  listInstances(): FederatedInstance[] {
    const rows = this.db.query('SELECT * FROM federated_instances ORDER BY name').all() as FederatedInstanceRecord[];
  ...
  async syncInstance(url: string): Promise<number> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new ExternalServiceError('Federation', `HTTP ${response.status}: ${response.statusText}`, {
          url: normalizedUrl,
        });
      }
      ...
      // Mark unreachable
      this.db.query("UPDATE federated_instances SET status = 'unreachable' WHERE url = ?").run(normalizedUrl);
