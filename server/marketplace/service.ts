...
export class MarketplaceService {
  ...
  getListingBadges(listingId: string): ListingBadges {
    ...
    const scoreRow = this.db
      .query('SELECT overall_score FROM agent_reputation WHERE agent_id = ?')
    ...
    const ownerRow = this.db.query("SELECT value FROM settings WHERE key = 'owner_wallet'").get() as {
      value: string;
    } | null;
  ...
  getListing(id: string): MarketplaceListing | null {
    const row = this.db.query('SELECT * FROM marketplace_listings WHERE id = ?').get(id) as ListingRecord | null;
  ...
  updateListing(id: string, input: UpdateListingInput): MarketplaceListing | null {
    ...
    if (input.pricingModel !== undefined) {
      ...
      values.push(input.pricingModel);
    }
    if (input.priceCredits !== undefined) {
      updates.push('price_credits = ?');
      values.push(input.priceCredits);
    }
    if (input.status !== undefined) {
      updates.push('status = ?');
      values.push(input.status);
    }
    if (input.trialUses !== undefined) {
      updates.push('trial_uses = ?');
      values.push(input.trialUses);
    }
    ...
    this.db.query(`UPDATE marketplace_listings SET ${updates.join(', ')} WHERE id = ?`).run(...values);
  ...
  getListingsByAgent(agentId: string): MarketplaceListing[] {
    const rows = this.db
      .query('SELECT * FROM marketplace_listings WHERE agent_id = ? ORDER BY updated_at DESC')
  ...
  recordUse(listingId: string, buyerWalletAddress?: string): UseResult {
    ...
    if (!listing) {
      ...
      this.db
        .query("UPDATE marketplace_listings SET use_count = use_count + 1, updated_at = datetime('now') WHERE id = ?")
        .run(listingId);
  ...
  search(params: MarketplaceSearchParams): MarketplaceSearchResult {
    ...
    const total = queryCount(
      this.db,
      `SELECT COUNT(*) as cnt FROM marketplace_listings ml${joinClause} ${where}`,
      ...values,
    );
  ...
  getReview(id: string): MarketplaceReview | null {
    const row = this.db.query('SELECT * FROM marketplace_reviews WHERE id = ?').get(id) as ReviewRecord | null;
  ...
  getReviewsForListing(listingId: string): MarketplaceReview[] {
    const rows = this.db
      .query('SELECT * FROM marketplace_reviews WHERE listing_id = ? ORDER BY created_at DESC')
  ...
  getTiersForListing(listingId: string): PricingTier[] {
    const rows = this.db
      .query('SELECT * FROM marketplace_pricing_tiers WHERE listing_id = ? ORDER BY sort_order ASC')
  ...
  getTier(tierId: string): PricingTier | null {
    const row = this.db.query('SELECT * FROM marketplace_pricing_tiers WHERE id = ?').get(tierId) as TierRecord | null;
  ...
  updateTier(tierId: string, input: UpdateTierInput): PricingTier | null {
    ...
    if (input.billingCycle !== undefined) {
      ...
      values.push(input.billingCycle);
    }
    ...
    values.push(tierId);
    this.db.query(`UPDATE marketplace_pricing_tiers SET ${updates.join(', ')} WHERE id = ?`).run(...values);
  ...
  recordTierUse(listingId: string, tierId: string, buyerWalletAddress: string): UseResult {
    ...
    if (tier.priceCredits > 0) {
      const escrowTx = this.escrow.settleInstantUse(listingId, buyerWalletAddress, listing.tenantId, tier.priceCredits);
  ...
  private updateListingRating(listingId: string): void {
    ...
    this.db
      .query(`
            ...
            WHERE id = ?
        `)
      .run(Math.round(stats.avg * 100) / 100, stats.count, listingId);
  }
}
