...
import { type AuditAction, recordAudit } from '../db/audit';
import { createLogger } from '../lib/logger';
import type { GrantOptions, PermissionAction, PermissionCheckResult, PermissionGrant, RevokeOptions } from './types';
...
function getHmacSecret(): string {
  ...
  if (isProduction) {
    log.warn(
      'PERMISSION_HMAC_SECRET not set in production — using ephemeral random key. Permission grants will not survive restarts. Set PERMISSION_HMAC_SECRET to a stable secret.',
    );
  } else {
...
export class PermissionBroker {
  ...
  async grant(options: GrantOptions): Promise<PermissionGrant> {
    ...
    recordAudit(
      this.db,
      'permission_grant' as AuditAction,
      grantedBy,
      'permission',
      String(grantId),
  ...
  revoke(options: RevokeOptions): number {
    ...
    if (affected > 0) {
      recordAudit(
        this.db,
        'permission_revoke' as AuditAction,
        revokedBy,
        'permission',
  ...
  emergencyRevoke(agentId: string, revokedBy: string, reason: string): number {
    ...
    if (count > 0) {
      ...
      recordAudit(
        this.db,
        'permission_emergency_revoke' as AuditAction,
        revokedBy,
        'permission',
        agentId,
