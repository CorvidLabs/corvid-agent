...
export function getAgentPolicy(db: Database, agentId: string): ResourceLimits {
  const row = db.query('SELECT * FROM sandbox_configs WHERE agent_id = ?').get(agentId) as SandboxConfigRecord | null;
 ...
 */
export function setAgentPolicy(db: Database, agentId: string, limits: Partial<ResourceLimits>): void {
  const merged = { ...DEFAULT_RESOURCE_LIMITS, ...limits };
  const existing = db.query('SELECT id FROM sandbox_configs WHERE agent_id = ?').get(agentId) as { id: string } | null;
  ...
  if (existing) {
    db.query(`
            ...
            WHERE agent_id = ?
        `).run(merged.cpuLimit, merged.memoryLimitMb, merged.networkPolicy, merged.timeoutSeconds, agentId);
  } else {
    ...
    db.query(`
        ...
        `).run(id, agentId, merged.cpuLimit, merged.memoryLimitMb, merged.networkPolicy, merged.timeoutSeconds);
  }
...
export function listAgentPolicies(db: Database): SandboxConfigRecord[] {
  return db.query('SELECT * FROM sandbox_configs ORDER BY created_at DESC').all() as SandboxConfigRecord[];
}
