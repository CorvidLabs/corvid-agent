...
export class SandboxManager {
   ...
   */
  async assignContainer(agentId: string, sessionId: string, workDir?: string | null): Promise<string> {
    if (!this.enabled) {
    ...
    for (const [containerId] of this.pool) {
      promises.push(
        stopContainer(containerId)
          ...
          .catch((err) => {
            log.debug('Failed to stop/remove container during shutdown', {
              error: err instanceof Error ? err.message : String(err),
            });
          }),
      );
      ...
      if (!entry.sessionId) {
        ...
        if (!status || status.status === 'error') {
          ...
          await removeContainer(containerId).catch((err) => {
            log.debug('Failed to remove dead warm container', {
              error: err instanceof Error ? err.message : String(err),
            });
          });
          this.pool.delete(containerId);
