...
export interface AlgochatMessageEvent {
  type: 'algochat_message';
  participant: string;
  content: string;
  direction: 'inbound' | 'outbound' | 'status';
}
export interface AgentBalanceMessage {
  type: 'agent_balance';
  agentId: string;
  balance: number;
  funded: number;
...
export interface CouncilStageChangeMessage {
  type: 'council_stage_change';
  launchId: string;
  stage: string;
  sessionIds?: string[];
}
export interface CouncilLogMessage {
  type: 'council_log';
  log: CouncilLaunchLog;
}
export interface CouncilDiscussionMessageEvent {
  type: 'council_discussion_message';
  message: CouncilDiscussionMessage;
}
export interface WorkTaskUpdateMessage {
  type: 'work_task_update';
  task: WorkTask;
}
export interface WorkTaskQueueUpdateMessage {
...
export interface ScheduleExecutionUpdateMessage {
  type: 'schedule_execution_update';
  execution: ScheduleExecution;
}
export interface ScheduleApprovalRequestMessage {
...
export interface OllamaPullProgressMessage {
...
export interface MentionPollingUpdateMessage {
  type: 'mention_polling_update';
...
export interface WorkflowNodeUpdateMessage {
  type: 'workflow_node_update';
  nodeExecution: WorkflowNodeRun;
}
...
export interface AgentQuestionMessage {
  type: 'agent_question';
  question: OwnerQuestionWire;
}
export interface GovernanceVoteCastMessage {
...
export interface GovernanceVoteResolvedMessage {
  type: 'governance_vote_resolved';
  launchId: string;
...
export interface GovernanceQuorumReachedMessage {
  type: 'governance_quorum_reached';
  launchId: string;
  weightedApprovalRatio: number;
  threshold: number;
}
...
export interface CouncilAgentErrorMessage {
  type: 'council_agent_error';
  launchId: string;
  agentId: string;
  agentName: string;
  error: CouncilAgentErrorInfo;
}
...
export type StreamEvent =
  ...
  | (StreamEventBase & {
      ...
      data: StreamEventDataBase & { type: 'message_delta'; delta?: { type?: string; text?: string } };
    })
  ...
  | (StreamEventBase & {
      ...
      data: StreamEventDataBase & { type: 'content_block_delta'; delta?: { type?: string; text?: string } };
    })
  ...
  | (StreamEventBase & {
      eventType: 'result';
      data: StreamEventDataBase & { type: 'result'; result?: string; total_cost_usd: number };
    })
  | (StreamEventBase & {
      eventType: 'error';
      data: StreamEventDataBase & { type: 'error'; error: { message: string; type: string } };
    })
  | (StreamEventBase & {
      eventType: 'tool_status';
      data: StreamEventDataBase & { type: 'tool_status'; statusMessage: string };
    })
  | (StreamEventBase & {
      eventType: 'system';
      data: StreamEventDataBase & { type: 'system'; statusMessage?: string; message?: { content: string } };
    })
  | (StreamEventBase & {
  ...
  | (StreamEventBase & {
      ...
      data: StreamEventDataBase & { type: 'session_exited'; result?: string };
    })
  ...
  | (StreamEventBase & {
      ...
      data: StreamEventDataBase & {
        type: 'session_error';
        error: { message: string; errorType: string; severity: string; recoverable: boolean };
      };
    })
  | (StreamEventBase & {
      eventType: 'queue_status';
      data: StreamEventDataBase & { type: 'queue_status'; statusMessage: string };
    })
  | (StreamEventBase & {
  ...
  | (StreamEventBase & {
      eventType: 'raw';
      data: StreamEventDataBase & { type: 'raw'; message?: { content: string } };
    });
