...
28. **authGuard proxy bypass (TRUST_PROXY=1)**: When `TRUST_PROXY` is `'1'` or `'true'` and the request contains a syntactically valid `X-Forwarded-Email` header, `authGuard` skips API key validation and returns null without setting `context.authenticated`. Identity resolution is completed by `tenantGuard` downstream (multi-tenant: sets `authenticated=true` after email lookup; single-tenant: sets `authenticated=true` for any valid proxy email). Without `TRUST_PROXY` set, or with a malformed/absent email header, `authGuard` enforces API key auth as normal. This allows requests authenticated by an upstream OAuth proxy to pass through without a Bearer token.
...
| 2026-04-05 | corvid-agent | Added invariant 28 (authGuard proxy bypass) — authGuard now skips API key check when TRUST_PROXY=1 and X-Forwarded-Email is valid, enabling end-to-end proxy-authenticated requests. Part of #1549 (Phase 2, item 5). |
