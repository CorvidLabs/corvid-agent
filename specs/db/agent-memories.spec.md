...
| `resolveAsaForKey` | `(db: Database, agentId: string, key: string)` | `number \| null` | Look up the ASA ID for a given memory key from the local DB. Returns null if no matching row or ASA ID is NULL |
