...
| `waitForSessions` | `processManager: ProcessManager, sessionIds: string[], timeoutMs?: number` | `Promise<WaitForSessionsResult>` | Wait for a set of sessions to complete, resolving with partial results on timeout. Uses heartbeat polling to detect sessions that exit without emitting events. |
