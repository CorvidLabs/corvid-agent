...
| `tenantQuery` | `db: Database, query: string, tenantId: string, ...params` | `T[]` | Execute a SELECT with automatic tenant scoping, returning all matching rows (generic) |
| `tenantQueryGet` | `db: Database, query: string, tenantId: string, ...params` | `T \| null` | Execute a SELECT with tenant scoping, returning a single row or null (generic) |
