<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-6215%20unit%20%7C%20360%20E2E-brightgreen" alt="6215 Unit | 360 E2E Tests">
...
| Metric | Value |
|--------|-------|
| Unit tests | **6,215** across 255 files (17,166 assertions) |
...
| DB migrations | **30** (squashed baseline, 88 tables) |
...
|  +-----------------------------------------------------------+  |
|  |                    SQLite (bun:sqlite)                     |  |
|  |  30 migrations | FTS5 search | WAL mode | foreign keys    |  |
|  +-----------------------------------------------------------+  |
+-----------------------------------------------------------------+
```
...
server/          Bun HTTP + WebSocket server
  ...
  db/            SQLite schema (30 migrations) and query modules
...
| Database | [SQLite](https://bun.sh/docs/api/sqlite) — WAL mode, FTS5, 30 migrations |
